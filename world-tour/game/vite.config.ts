...
import { themeCss, tone } from './src/ui/theme';
  ...
  return {
    ...
    transformIndexHtml: {
      ...
      handler(html) {
        ...
        // The theme's custom properties come first: every stylesheet after this mixes from them.
        return html.replace('<!-- startup-shell -->', `<style>${themeCss()}${css}
          #startup { position:fixed; inset:0; z-index:200; display:flex; flex-direction:column;
            ...
            color:${tone('night', 'mist', .98)}; background:${tone('night', 'mist', .02)}; font:16px/1.5 system-ui,sans-serif; overflow:auto; }
          ...
          #startup button { border:1px solid ${tone('accentDeep', 'accentLight', .4)}; padding:12px 24px; border-radius:12px;
            font:inherit; color:inherit; background:${tone('night', 'mist', .08)}; cursor:pointer; }
          ...
          <h1><img src="./brand/slime-race-mark-180.png" alt=""><span>Silicon Slime Rush: World Tour</span></h1><p role="status"></p>
...
/**
 * Landmark models ship meshopt-packed, losslessly, in `dist/` only.
 *
 * The Blender exports under `public/models/landmarks/` stay raw float32 because the map pipeline reads
 * them (trimesh slices each one at car height against the road, and it cannot decode meshopt) and hashes
 * their bytes into every track's build id -- packing them in place would rebuild every route and break the
 * check. So they are packed here, on the way out, by `assets-src/compress_models.mjs`, which refuses to
 * write a model whose triangles, nodes or materials changed. 36 models went from 244 MB to about 100 MB:
 * that is the download a player waits for before a landmark appears, and it was most of a 450 MB build.
 * Packed copies are cached by the raw file's hash, so only a new or changed model costs the ~2 s.
 */
export function packLandmarks(): Plugin {
  let distDir = resolve(__dirname, 'dist');
  return {
    name: 'sr-pack-landmarks',
    apply: 'build',
    configResolved(config) { distDir = resolve(config.root, config.build.outDir); },
    closeBundle() {
      const dir = resolve(distDir, 'models/landmarks');
      if (!existsSync(dir)) return;
      const cache = resolve(__dirname, 'node_modules/.cache/sr-landmarks');
      mkdirSync(cache, { recursive: true });
      for (const name of readdirSync(dir).filter(file => file.endsWith('.glb'))) {
        const file = resolve(dir, name);
        const raw = readFileSync(file);
        const header = raw.subarray(20, 20 + raw.readUInt32LE(12)).toString('utf-8');
        if (header.includes('EXT_meshopt_compression')) continue;
        const packed = resolve(cache, createHash('sha256').update(raw).digest('hex') + '.glb');
        if (!existsSync(packed)) {
          execFileSync(process.execPath, [resolve(__dirname, '../assets-src/compress_models.mjs'), file, packed + '.tmp'], { stdio: 'pipe' });
          copyFileSync(packed + '.tmp', packed);
          rmSync(packed + '.tmp');
        }
        copyFileSync(packed, file);
      }
    },
  };
}
...
export default defineConfig({
  ...
  plugins: [stripSchemaNotes(), startupShell(), packTiles(), packLandmarks(), creditsNotices(credits.notices), stripCopiedNotes(), obfuscateRelease()],
  base: './',
  build: {
