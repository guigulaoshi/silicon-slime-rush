...
test('no GPU program is created for the first time after the loading screen @version', async ({ page }) => {
  ...
  const saved = { language: 'en', quality: 'high', muted: true, best: { lhasa: 120 }, ghosts: { lhasa: ghost() } };
  await page.addInitScript((save) => localStorage.setItem('silicon-rush-world-tour.save.v1', JSON.stringify(save)), saved);
  ...
  const loaded: string[] = await page.evaluate(async () => {
    ...
    await g.startRace({ trackId: 'lhasa', car: 'sedan', playerVehicles: ['micro-hatch'], ai: false, slimeDensity: 'normal', timeOfDay: 'day' }, true);
