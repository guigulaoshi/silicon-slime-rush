...
test('the smallest purple splitter bounces away on every hit, stays on the road and scores each time', async ({ page }) => {
  ...
  // Retargeted from shoreline (deleted, "an open flat campus route") to lhasa: a generic authored
  // slime-splitting check, any flat day track works.
  expect(await page.evaluate(() => window.game.startRace({ trackId: 'lhasa', car: 'sedan', playerVehicles: ['micro-hatch'],
  ...
  const read = () => page.evaluate(() => {
    ...
    // Only this test's slime: moving the car streams in the next tiles, whose own slimes are not the subject.
    return { score: s.race.score, hits: s.race.slimeHits, alive: !!target?.active,
      slicks: layer.lives.filter((l: any) => l.tile === 'smallest' && l.spawn.kind === 'slick').length, splitChildren: layer.stats.splitChildren };
  });
