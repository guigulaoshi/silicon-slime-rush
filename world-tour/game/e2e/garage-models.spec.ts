...
const { vehicles: ALL } = JSON.parse(readFileSync('src/vehicles/catalogue.json', 'utf8')) as { vehicles: VehicleDefinition[] };
// The nine garage slots; on a route with its own car that car stands in its slot (the button keeps the
// slot's id, the picture and the model are the local car's).
const VEHICLES = ALL.filter(vehicle => !vehicle.homeTrack);
const shown = (slot: VehicleDefinition, track: string) => ALL.find(v => v.homeTrack === track && v.replaces === slot.id) ?? slot;
...
for (const viewport of [{ width: 1440, height: 900 }, { width: 844, height: 390 }]) {
  test('production garage models at ' + viewport.width, async ({ page }) => {
    ...
    // Opening loads the home page's sydney scene (game/src/app/showcase.ts), car and all, before the menu exists.
    ...
    for (const vehicle of VEHICLES) await expect(page.locator(
      ...
      .toHaveAttribute('src', `./garage/${shown(vehicle, track).id}.webp`);
    for (const vehicle of VEHICLES) {
      await page.locator('[data-vehicle="' + vehicle.id + '"]').click();
      await expect(hero).toHaveAttribute('data-state', 'ready');
      const body = shown(vehicle, track);
      await expect(hero).toHaveAttribute('data-model-vehicle', body.id);
      await expect(hero).toHaveAttribute('data-bodies',
        [body.id, ...(body.trailer ? [body.trailer.id] : [])].join(','));
