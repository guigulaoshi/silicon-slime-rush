...
test('split screen: the combo clears the nav disc, speedometer and clock, and hides at the finish', async ({ page }) => {
  ...
  for (const width of [1280, 1024]) {
    ...
    // At ten the combo is in its hot tier and wears that tier's colour -- whatever the remix's theme makes it
    // (the original's orange became the passport-stamp red), so the colour is read from the tier's own rule.
    expect(await page.locator('.player-hud[data-player="1"] .hud-combo').evaluate(node => {
      const probe = document.createElement('div'); probe.className = 'hud-combo'; probe.dataset.tier = 'hot';
      node.parentElement!.append(probe); const hot = getComputedStyle(probe).color; probe.remove();
      const base = document.createElement('div'); base.className = 'hud-combo';
      node.parentElement!.append(base); const plain = getComputedStyle(base).color; base.remove();
      return [(node as HTMLElement).dataset.tier, getComputedStyle(node).color === hot, hot !== plain];
    })).toEqual(['hot', true, true]);
