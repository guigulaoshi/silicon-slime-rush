...
test('the engine goes quiet when the race is paused, and comes back when it resumes', async ({ page }) => {
  // A straight launch keeps this sound check off the guardrail while accelerating. The showcase
  // route (sydney) bends onto the bridge within the first 50 m; paris's start runs dead straight
  // instead -- curvature stays under 0.001 for the first 300 m (game/public/tracks/paris/track.json
  // spline.curvature, s <= 300).
  await page.goto('/?track=paris&dev=1');
