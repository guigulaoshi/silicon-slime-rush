...
// Retargeted from goldengate (deleted, "the default/showcase track, a long bridge over water, deck
// and towers") to sydney, the new home showcase track (game/src/app/showcase.ts) with its own
// 1.4 km harbour bridge deck.
const TRACK = process.env.HOLES460_TRACK ?? 'sydney';
...
test(`no sky under the horizon from the race and photo cameras on ${TRACK}`, { tag: '@version' }, async ({ page }) => {
  ...
  if (BEFORE) return;
  // Was measured on goldengate, the worst track: chase camera at most 2,606, photo mode at most 6,087
  // (35 m behind the car, 15 degrees up, facing along the bridge) -- a thin strip under the Marin
  // hills. The home page's hole before 455 was 21,349. Ten thousand separates the two. Sydney's
  // harbour bridge deck is the new track most likely to show the same kind of gap, but the actual
  // pixel counts need a real render to confirm.
  // RETARGET-MEASURE: worst.sky and worst.camera on sydney (chase + all six photo-mode angles), and
  // whether 10,000 still leaves the same safety margin above the measured worst.
