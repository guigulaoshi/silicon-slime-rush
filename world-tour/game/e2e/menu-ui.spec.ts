...
test('B3 route gallery and world choices use real game renders', async ({ page }) => {
  ...
  // goldengate -> sydney (default/showcase), twin-peaks -> zhangjiajie (winding hill road), wolfe-pruneridge -> beijing (loop)
  for (const id of ['sydney', 'zhangjiajie', 'beijing']) {
  ...
  await page.locator('[data-track="sydney"]').click();
  ...
  await expect(shot).toHaveAttribute('src', /menu\/world\/sydney\/day-clear\.webp/);
 
  await page.locator('[data-time="day"]').click();
  await expect(shot).toHaveAttribute('src', /menu\/world\/sydney\/day-clear\.webp/);
  await page.locator('[data-time="night"]').click();
  await expect(shot).toHaveAttribute('src', /menu\/world\/sydney\/night-clear\.webp/);
  for (const weather of ['fog', 'rain', 'snow']) {
    await page.locator(`[data-weather="${weather}"]`).click();
    await expect(shot).toHaveAttribute('src', new RegExp(`menu/world/sydney/night-${weather}\\.webp`));
  }
  for (const density of ['none', 'normal', 'many']) {
    await page.locator(`[data-slime-density="${density}"]`).click();
    await expect(shot).toHaveAttribute('src', new RegExp(`menu/world/sydney/slimes-${density}\\.webp`));
  }
  for (const difficulty of ['relaxed', 'rush']) {
    await page.locator(`[data-ai-mode="${difficulty}"]`).click();
    await expect(shot).toHaveAttribute('src', new RegExp(`menu/world/sydney/ai-${difficulty}\\.webp`));
  }
