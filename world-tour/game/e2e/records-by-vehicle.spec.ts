...
// Retargeted from goldengate (deleted) to beijing: a generic real track for this menu display check.
const seed = (page: Page, vehicles: Record<string, string>) => page.addInitScript(remembered =>
  localStorage.setItem('silicon-rush-world-tour.save.v1', JSON.stringify({ version: 14, language: 'en', muted: true,
    best: { 'beijing@sports-car': 150.5, 'beijing@monster-truck': 222.25, 'synth-p2p@monster-truck': 61 },
    ratings: { 'beijing@sports-car': 5, 'beijing@monster-truck': 3 }, vehicles: remembered })), vehicles);
