...
test('menu remembers different cars per route after reload and migrates invalid ids', async ({ page }) => {
  await page.goto('/'); await page.locator('.home-go').click();
  // sydney (showcase bridge, ex goldengate) and lhasa (flat open route, ex shoreline) replace the
  // deleted Bay Area tracks used here just to exercise per-route saved state.
  await page.evaluate(() => localStorage.setItem('silicon-rush-world-tour.save.v1', JSON.stringify({ version: 3,
    language: 'zh', timeOfDay: 'night', vehicles: { sydney: 'retired-car' }, best: { lhasa: 123 } })));
  ...
  await choose('sydney', 'school-bus');
  await choose('lhasa', 'retro-van');
  for (const [trackId, vehicle] of [['sydney', 'school-bus'], ['lhasa', 'retro-van']]) {
    await page.locator(`.sm-item[data-track="${trackId}"]`).click();
    await page.locator('.sm-go').click(); await step(page, 1);
    await page.locator('.sm-go').click(); await step(page, 2);
  ...
  expect(await page.evaluate(() => JSON.parse(localStorage.getItem('silicon-rush-world-tour.save.v1')!).best))
    .toEqual({ 'lhasa@micro-hatch': 123 });
});
