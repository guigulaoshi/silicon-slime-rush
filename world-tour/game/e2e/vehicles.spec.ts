...
// These drives are on lhasa, where the jeep's slot holds the city's own car: asking for it drives the plateau SUV.
const driving = (vehicle: VehicleDefinition) =>
...
test('production cars drive with aligned models and survive restart and replacement', async ({ page }) => {
  await page.goto('/?track=lhasa&bot=1&dev=1&time=day&vehicle=micro-hatch');
  ...
  for (const vehicle of VEHICLES) {
    await page.evaluate(async id => {
      ...
      await game.autoRun('lhasa', 'day', id);
    }, vehicle.id);
    ...
    expect(facts).toEqual({ id: body.id, sceneCars: body.trailer ? 2 : 1, wheels: 4, half: body.chassisHalf,
...
test('vehicle inspection waits for a real drive and keeps the car visible', async ({ page }) => {
  await page.goto('/?track=lhasa&bot=1&dev=1&time=day&vehicle=school-bus&inspect=vehicle');
