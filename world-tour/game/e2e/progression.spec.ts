...
test('nothing is locked and remembered choices work through the real menu', async ({ page }) => {
  ...
  // Retargeted from shoreline (deleted, "an open flat campus route") to lhasa: a generic menu
  // flow check, any real track works.
  await page.locator('[data-track="lhasa"]').click();
  ...
  await page.locator('.sm-go').click();
  await expect.poll(() => page.evaluate(() => JSON.parse(localStorage.getItem('silicon-rush-world-tour.save.v1') ?? '{}').lastRace))
    .toMatchObject({ trackId: 'lhasa', direction: 'reverse', playerCount: 2,
  ...
  await expect(page.locator('[data-track="lhasa"]')).toHaveClass(/sel/);
  await page.locator('.sm-go').click(); await page.locator('.sm-go').click();
  ...
  writeFileSync(resolve(out, 'facts.json'), JSON.stringify({ lastRace: await page.evaluate(() =>
    JSON.parse(localStorage.getItem('silicon-rush-world-tour.save.v1') ?? '{}').lastRace) }, null, 2));
});
