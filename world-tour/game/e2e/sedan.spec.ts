...
import { taxiLivery } from '../src/vehicles/catalogue';

// One real-road inspection plus two localized garage views.
test.describe.configure({ timeout: 120_000 });
const output = evidencePath('sedan');
// Micro-hatch is the taxi vehicle repainted per city (catalogue.json taxis); the plain-black
// production paint this test guards only shows on a built track that does not repaint it.
const unliveriedTrack = [...BUILT].find(id => !taxiLivery('micro-hatch', id));
if (!unliveriedTrack) throw new Error('every built track now repaints the taxi vehicle -- pick another body for the paint check');
test('Sedan has localized menu identity and a visible curved production body', async ({ page }) => {
  ...
  // Retargeted from shoreline (deleted, "an open flat campus route"): this is a generic
  // vehicle-model inspection, any flat day track without a taxi livery on micro-hatch works.
  await page.goto(`/?track=${unliveriedTrack}&bot=1&dev=1&time=day&vehicle=micro-hatch&inspect=vehicle&perf=1&perfSlimes=none`);
