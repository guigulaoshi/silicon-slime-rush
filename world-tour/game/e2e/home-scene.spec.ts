...
/** The home page keeps its instant picture, then fades in a live orbit of the Sydney Harbour Bridge (showcase.ts). */
...
test('the scene is ready before the home page, and leaving home ends it', async ({ page }) => {
  ...
  expect(facts).toMatchObject({ phase: 'menu', track: 'sydney', raceTime: 0 });
...
test('reduced motion keeps the still picture', async ({ page }) => {
  await page.setViewportSize({ width: 1280, height: 720 });
  await page.addInitScript(() => localStorage.setItem('silicon-rush-world-tour.save.v1', JSON.stringify({ version: 1, language: 'en', volume: 0, muted: true, best: {}, reducedMotion: true })));
  await page.goto('/');
  await expect(page.locator('.home-go')).toBeVisible();
