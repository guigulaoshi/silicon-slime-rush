...
const { vehicles } = JSON.parse(readFileSync(resolve('src/vehicles/catalogue.json'), 'utf8')) as {
  vehicles: { id: string; homeTrack?: string; replaces?: string; handling: { mass: number; maxSpeed: number } }[];
};
// On lhasa the jeep's slot holds the city's own car: asking for the jeep drives the plateau SUV.
const driving = (vehicle: typeof vehicles[number]) =>
  vehicles.find(v => v.homeTrack === 'lhasa' && v.replaces === vehicle.id) ?? vehicle;
...
for (const vehicle of vehicles) test(`${vehicle.id} completes the same real-road course with its own calibration`, async ({ page }) => {
  await page.goto(`/?track=lhasa&bot=1&dev=1&time=day&speed=6&vehicle=${vehicle.id}`);
  ...
  const body = driving(vehicle);
  expect(tuning.mass).toBe(body.handling.mass);
  expect(tuning.speed).toBe(body.handling.maxSpeed);
  expect(tuning.physicalMass).toBeCloseTo(body.handling.mass, 3);
...
test('records clear road-view evidence of all calibrated vehicles', async ({ page }) => {
  ...
  for (const vehicle of vehicles) {
    ...
    await page.goto(`/?track=lhasa&bot=1&dev=1&time=day&vehicle=${vehicle.id}&inspect=vehicle&perf=1&perfSlimes=none`);
...
test('keeps the car out of physics until delayed starting road collisions arrive', async ({ page }) => {
  ...
  let requests = 0;
  await page.route('**/tracks/beijing/tiles.bin', async route => {
    requests++; await held; await route.continue();
  });
  await page.goto('/?track=beijing&bot=1&dev=1&speed=6');
  ...
  // beijing (loop, 1 lap) has spline.length ~2225 m, so progress > 350 is ~16% of the way in --
  // still a meaningful distance moved before physics settles, same as the old wolfe-pruneridge check.
