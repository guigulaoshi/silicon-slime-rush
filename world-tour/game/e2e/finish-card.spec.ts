...
async function finish(page: Page, ai: boolean, language: 'en' | 'zh') {
  return page.evaluate(async ({ ai, language }) => {
    ...
    g.i18n.set(language);
    await g.startRace({ trackId: 'new-york', vehicleId: 'sports-car', timeOfDay: 'day',
...
test('every route rates the finish: stars in the slogan slot, big title, new footer, place with AI', async ({ page }) => {
  ...
  for (const language of ['en', 'zh'] as const) for (const ai of [false, true]) {
    ...
    // Copy updated for the World Tour remix (game/src/ui/locales/en.json / zh.json, key
    // replica.realRoads).
    const footer = language === 'en' ? 'DRIVE REAL STREETS IN 13 CITIES · FREE IN YOUR BROWSER' : '开遍十三座名城的真实街道 · 浏览器免费玩';
