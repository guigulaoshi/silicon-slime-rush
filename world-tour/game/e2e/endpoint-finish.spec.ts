...
// shanghai: the Bund riverside (game/public/tracks/shanghai/track.json), replacing fishermans-wharf
// as a waterfront route now that the Bay Area tracks are gone.
test('shanghai actual finish background', async ({page}) => {
 const out=evidencePath('endpoints/after');mkdirSync(out,{recursive:true});
 const errors:string[]=[];page.on('pageerror',error=>errors.push(error.message));
 await page.goto('/?track=shanghai&bot=1&dev=1&speed=6');
 ...
 writeFileSync(resolve(out,'shanghai-drive.json'),JSON.stringify(report,null,2));
 ...
 await page.screenshot({path:resolve(out,'shanghai-results.png')});
});
