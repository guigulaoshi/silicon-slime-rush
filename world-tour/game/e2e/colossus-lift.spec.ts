...
test('slow and fast crossings float at the same height and fall as soon as they leave',async({page})=>{
  ...
  for(const mode of ['slow','fast']) {
    ...
    await page.evaluate(async({scale,groundFraction,speed})=>{
      ...
      // lhasa: flat, dead straight through s=140-200 (game/public/tracks/lhasa/track.json
      // spline.curvature <= 0.00084 there), replacing shoreline for this synthetic-tile test.
      await g.startRace({trackId:'lhasa',car:'sedan',playerVehicles:['micro-hatch'],ai:false,slimeDensity:'normal',timeOfDay:'day'});
