...
// Five actual collision drives plus a close view; this is the opt-in visual delivery.
test.skip(process.env.SLIME_QA !== '1', 'run with SLIME_QA=1');
test.describe.configure({ timeout: 240_000 });
test.use({ video: { mode: 'on', size: { width: 1280, height: 720 } }, viewport: { width: 1280, height: 720 } });
const OUT = evidencePath('slime-polish');

test('records the resized population, timed wheel trails, centre blast and projected body splats', async ({ page }) => {
  ...
  for (const kind of ['slick', 'burst', 'boost', 'popper'] as const) {
    ...
    const initial = await page.evaluate(kind => {
      ...
      const start = spline.point(spline.indexAt(12));
      const t = spline.tangent(spline.indexAt(12));
      // Put even the 1.75 m slick directly on the current tangent. The spline bends between 12 m
      // and 30 m, so using its later centre lets the car follow the inside line and miss the test.
      const p: [number, number, number] = [start[0] + t[0] * 10, start[1], start[2] + t[2] * 10];
      s.car.reset([start[0], start[1] + .8, start[2]], Math.atan2(-t[0], -t[2]));
      s.car.body.setLinvel({ x: t[0] * 8, y: 0, z: t[2] * 8 }, true);
      const radius = kind === 'slick' ? .875 : kind === 'popper' ? 2 : 2.6;
