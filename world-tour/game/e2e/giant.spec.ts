...
for (const [label, inspect, kind] of [
  ...
  // Shoreline (an open flat campus route) was deleted with the Bay Area tracks; Lhasa is its
  // world-tour replacement (flat high valley, boulevard) -- the inspector spawns a demo slime of
  // `kind` and measures it against the real road width from Lhasa's own spline, same as before.
  await page.goto(`/?track=lhasa&bot=1&dev=1&time=day&inspect=${inspect}`);
