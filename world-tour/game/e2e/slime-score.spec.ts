...
for (const lang of ['en', 'zh']) test(`actual slime collision scores and settles in ${lang}`, async ({ browser }) => {
  ...
  await page.addInitScript(language => localStorage.setItem('silicon-rush-world-tour.save.v1', JSON.stringify({ language, muted: true })), lang);
  ...
  await page.evaluate(() => {
    ...
    // The restart brings back every tile's own slimes; only this test's popper is the subject (a change to
    // the route's placement put another one near the start and it scored too).
    for (const tile of [...s.slimes.tileSpawns.keys()]) if (tile !== g.__scoreTile) s.slimes.removeTile(tile);
