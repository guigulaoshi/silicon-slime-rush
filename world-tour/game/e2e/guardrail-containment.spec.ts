...
test('roadside rails contain vehicles at point-to-point starts', async ({ page }) => {
  ...
  // 475's Golden Gate/Shoreline pair was retargeted after those tracks were deleted (world-tour
  // rebuild): Sydney is the new default/showcase point-to-point track (a long bridge deck, same
  // role Golden Gate played), Lhasa is the new open flat boulevard route (Shoreline's role). Both
  // are point-to-point, which is what this test exercises (start-rail containment), and the
  // contained/breakout thresholds below are computed at runtime from each track's own halfWidth,
  // not hardcoded, so no numbers needed re-deriving.
  const samples: Sample[] = [
    { trackId: 'sydney', vehicleId: 'micro-hatch', label: 'Sydney sedan low-speed direct',
      alongSpeed: 0, acrossSpeed: 4, expected: 'contained' },
    { trackId: 'sydney', vehicleId: 'school-bus', label: 'Sydney school bus low-speed direct',
      alongSpeed: 0, acrossSpeed: 4, expected: 'contained' },
    { trackId: 'lhasa', vehicleId: 'micro-hatch', label: 'Lhasa sedan low-speed direct',
      alongSpeed: 0, acrossSpeed: 4, expected: 'contained' },
    { trackId: 'sydney', vehicleId: 'micro-hatch', label: 'Sydney sedan fast shallow rub',
      alongSpeed: 18, acrossSpeed: 4, expected: 'contained' },
    // The original's own breakout case (11 m/s across) and a 80 km/h, 45 degree strike (15.7 m/s)
    // now bounce back; only a racing-speed, near head-on strike (26 m/s across) goes through.
    { trackId: 'sydney', vehicleId: 'micro-hatch', label: 'Sydney sedan old breakout now contained',
      alongSpeed: 0, acrossSpeed: 11, expected: 'contained' },
    { trackId: 'sydney', vehicleId: 'micro-hatch', label: 'Sydney sedan 80 km/h at 45 degrees',
      alongSpeed: 15.7, acrossSpeed: 15.7, expected: 'contained' },
    { trackId: 'sydney', vehicleId: 'micro-hatch', label: 'Sydney sedan racing-speed head-on breakout',
      alongSpeed: 0, acrossSpeed: 26, expected: 'breakout' },
  ];
  const results = [];
  for (const sample of samples) {
    const result = await launchAcrossStartRail(page, sample);
    results.push({ ...sample, ...result });
    if (sample.label === 'Sydney sedan low-speed direct') {
      await page.evaluate(() => {
  ...
  for (const result of results) {
    ...
    if (result.expected === 'contained') {
      ...
      // Through the rail: either standing beyond it, or -- where the start is raised, as Sydney's is on
      // the Harbour Bridge approach -- fallen past it and put back by the race's recovery.
      expect(result.lateral > result.halfWidth + 2 || result.resets > 0, JSON.stringify(result)).toBe(true);
    }
  }
});
