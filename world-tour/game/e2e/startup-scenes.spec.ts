...
// goldengate (~5400 m) -> sydney (3115.02 m, game/public/tracks/sydney/track.json), the app's own
// home/showcase track (game/src/app/showcase.ts). 950/1450 were ~17.6%/26.9% along the old route,
// both on its bridge deck; 550/850 keep the same proportions on sydney, and sydney's own deck runs
// 0-1368 m (flat y~41 m), so both points sit on the deck here too.
test('capture clean real sydney startup backgrounds',async({page})=>{
 ...
 for(const scene of scenes){
  ...
  await page.evaluate(async v=>{
   const g=window.game as any;await g.startRace({trackId:'sydney',vehicleId:'sports-car',ai:v.ai,aiDifficulty:v.difficulty,timeOfDay:v.time,weather:v.weather,slimeDensity:v.slimes});
   const s=g.session,w=s.world;g.phase='paused';g.autopilot=false;
   const at=v.home?550:850,i=w.spline.indexAt(at),p=w.spline.point(i),t=w.spline.tangent(i);
