...
// Retargeted from wolfe-pruneridge (deleted; "Big Tech Rooftop" was its rooftop-ring flavour name).
// This tests the generic road-edge guardrail (pipeline/sr/roads.py guardrails(): every route with
// enough points gets continuous beams along both edges), not anything unique to a rooftop. beijing
// is the new loop track (matching wolfe-pruneridge's own shape), and s=300 sits well inside its
// 2,225 m length (game/public/tracks/beijing/track.json spline.length), clear of the route ends.
test('a school bus driven into a beijing guardrail is turned back onto the road', async ({ page }) => {
  mkdirSync(out, { recursive: true });
  await page.addInitScript(() => localStorage.setItem('silicon-rush-world-tour.save.v1', JSON.stringify({ version: 13, language: 'en' })));
  await page.goto('/?dev=1');
  await page.waitForFunction(() => window.game);
  await page.evaluate(async () => {
    await (window.game as any).startRace({ trackId: 'beijing', vehicleId: 'school-bus',
