...
test('formal high-low-high quality changes retain nonempty ground and body GPU pixels and fade state',async({page})=>{
 ...
 const result=await page.evaluate(async()=>{
  ...
  // Retargeted from shoreline (deleted, "an open flat campus route") to lhasa, any flat day track works for this GPU-texture probe.
  await g.startRace({trackId:'lhasa',vehicleId:'jeep',slimeDensity:'normal',ai:false,timeOfDay:'day'});g.phase='paused';cancelAnimationFrame(g.raf);
