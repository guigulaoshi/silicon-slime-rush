...
test('every vehicle takes the slime coat on its shell, not inside the cabin', async ({ page }) => {
  ...
  // (deleted with the Bay Area tracks); this test only needs a real race to splash slime on.
  expect(await page.evaluate(() => window.game.startRace({ trackId: 'lhasa', car: 'sedan', playerVehicles: ['lightweight-sports'],
