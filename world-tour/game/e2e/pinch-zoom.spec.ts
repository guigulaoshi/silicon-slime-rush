...
test.describe('phone pinch zoom', () => {
  ...
  // Track retargeted from shoreline (deleted, "an open flat campus route") to lhasa, a flat
  // day track that reaches the results screen quickly at speed=6.
  test('the results screen refuses a pinch', async ({ page }) => {
    await page.goto('/?track=lhasa&bot=1&dev=1&speed=6');
  ...
  test('a page that did get zoomed is back to scale 1 once the race starts', async ({ page }) => {
    ...
    await page.evaluate(() => window.game.startRace({ trackId: 'lhasa', playerVehicles: ['micro-hatch'], slimeDensity: 'none', ai: false }));
