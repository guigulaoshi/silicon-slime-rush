...
// The Bay Area tracks named here individually (a mix of closed loops and open p2p routes, picked to
// exercise both branches below) are gone; every one of them was chosen from CATALOGUE/BUILT, so this
// now runs the same check across the World Tour's whole roster instead of picking a subset by name --
// beijing (mode 'loop') still exercises the closed-track branch, and the rest exercise the open one.
   ...
   const facts=await page.evaluate(async end=>{
    ...
    const probes=road?[0,15,50,100,200].flatMap(at=>{
     ...
     return [-.8,0,.8].map(side=>{
      ...
      // A tyre's contact patch, not a needle: the seam where the course and its continuation meet
      // leaves a hole a few centimetres wide at one vertex, which a ray
      // through that exact point misses and no wheel can fall into.
      const hit=[[0,0],[.15,0],[-.15,0],[0,.15],[0,-.15]].map(([dx,dz])=>s.physics.surfaceAt(p[0]+dx,p[2]+dz,p[1]+3,p[1]-3)).find(Boolean);
