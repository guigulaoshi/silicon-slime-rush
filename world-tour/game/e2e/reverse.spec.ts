...
// CATALOGUE and SYNTHETIC supply whichever tracks currently exist, so this parametrizes itself; no
// route name below needs updating when tracks change.
//
// bayshore-101 (the 64.8 km highway) was deleted along with the rest of the Bay Area
// tracks, so `reverse bayshore-101` no longer runs -- but the incident it hit is kept verbatim below
// because the caution ("never make it green by relaxing the rollover rule") applies to whichever
// track flakes under load next, not to that one route.
//
