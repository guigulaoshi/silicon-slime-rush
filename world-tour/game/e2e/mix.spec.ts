...
test('records one continuous mix, persists separate levels and survives unavailable audio', async ({ page }) => {
  await page.addInitScript(() => {
    ...
    localStorage.setItem('silicon-rush-world-tour.save.v1', JSON.stringify({ version: 5, language: 'en', muted: false, slimeDensity: 'none' }));
  });
  ...
  expect(await page.evaluate(() => JSON.parse(localStorage.getItem('silicon-rush-world-tour.save.v1')!)))
...
test('phone settings keep independent mix and mute after reload', async ({ page }) => {
  ...
  const before = await page.evaluate(() => JSON.parse(localStorage.getItem('silicon-rush-world-tour.save.v1')!));
  ...
  await page.locator('.sm-link').first().click();
  expect(await page.evaluate(() => JSON.parse(localStorage.getItem('silicon-rush-world-tour.save.v1')!))).toEqual(before);
