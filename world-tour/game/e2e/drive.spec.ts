...
// 那是「问浏览器一个问题」的价钱。十七条路线（合成路线 3 条 + CATALOGUE 里除 sydney 外的 14 条），
// 全部六倍 turbo，悉尼那条是实时的：三分钟驾驶加加载。Turbo 每一帧多跑完整的 60 Hz 固定步，不把物理步长改粗。
...
test.describe('the automated driver', () => {
  ...
  for (const track of [...SYNTHETIC, ...CATALOGUE.map((entry) => entry.id)
    .filter((id) => id !== 'sydney')]) {
    test(`finishes ${track}`, {}, async ({ page }) => {
      const errors: string[] = [];
      page.on('console', (m) => { if (m.type() === 'error') errors.push(m.text()); });
      // Six fixed steps per wall step reduces rendering overhead without coarsening physics, for every
      // route now: the ×12 bayshore-101 exception is gone along with that 64.8 km highway (deleted
      // with the rest of the Bay Area tracks -- there is no long route any more).
      const timeScale = 6;
      ...
      // A route that carries giants makes the bot drive through one; a hairpin mountain road may carry
      // none (sr/slimes.py puts giants only on road clear of sharp bends), and then there is none to enter.
      if ((report.slimes?.colossiStreamed ?? 0) > 0) expect(report.slimes?.colossusEntries,
        `the bot never entered the route's mandatory colossus: ${where}`).toBeGreaterThan(0);
      else expect(track, 'only the hairpin route carries no giant').toBe('zhangjiajie');
  ...
  test('finishes the whole of sydney with the scenery loaded', { tag: '@version' }, async ({ page }) => {
    test.skip(process.env.REALTIME_ROUTE_QA !== '1',
      'explicit real-time streaming evidence; turbo route and seven-car sydney gates are recurring');
    const errors: string[] = [];
    page.on('console', (m) => { if (m.type() === 'error') errors.push(m.text()); });
    ...
    const report = await driveToFinish(page, budget, 'sydney');
    ...
    expect(report.resets, 'the real-time sydney run must not use recovery').toBe(0);
    expect(report.tiles?.failed ?? 0).toBe(0);
    expect(report.slimes?.spawned ?? 0, 'the streamed sydney route has no slimes').toBeGreaterThan(0);
    expect(report.renderQuality).toBe('high');
    expect(report.qualityLimits?.slimes).toBe(QUALITY_LIMITS.high.slimes);
    expect(report.slimes?.colossusEntries, 'the sydney bot never entered the route\'s mandatory colossus')
      .toBeGreaterThan(0);
    expect(report.slimes?.colossusExits, 'an accidental sydney giant entry never exited')
      .toBe(report.slimes?.colossusEntries);
    expect(errors, errors.join('\n')).toHaveLength(0);
    console.log(`sydney: ${report.time.toFixed(1)}s, ${report.resets} resets, `
...
test('shows a result and remembers the best time', async ({ page }) => {
  ...
  const saved = await page.evaluate(() => JSON.parse(localStorage.getItem('silicon-rush-world-tour.save.v1') ?? 'null'));
