...
// Retargeted from fishermans-wharf (deleted): dubai's route has treeFill.mix.palm = 1.0
// (pipeline/routes/dubai.json), the only new track whose roadside trees are all palms.
test('Dubai palms have feathered crowns and repeatable varied bends', async ({ page }) => {
  const directory = evidencePath('palms');
  mkdirSync(directory, { recursive: true });
  await page.goto('/?track=dubai&bot=1&dev=1&time=day&perf=1&perfSlimes=none');
  await page.waitForFunction(() => window.game?.report().phase === 'racing');
  await expectWorldLoaded(page, '254 Dubai');
