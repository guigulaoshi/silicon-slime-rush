 ...
 */
// new-york: p2p, 3273 m (game/public/tracks/new-york/track.json) -- a dense city route like
// fishermans-wharf was, before the Bay Area tracks were deleted; paired with ai-pace.
...
test(`AI throttle use on ${TRACK} at ${DIFFICULTY}`, { tag: '@version' }, async ({ page }) => {
  const out = evidencePath('ai-pace-2'); mkdirSync(out, { recursive: true });
  await page.addInitScript(() => localStorage.setItem('silicon-rush-world-tour.save.v1', JSON.stringify({ muted: true })));
  ...
  if (!process.env.AI_PACE_PLAYER_DRIVER) {
    // A solo race ends when the player crosses the line, so an unfinished AI is not news: a school bus can
    // legitimately trail minutes behind a quick field on zhangjiajie's switchback climb (rio's replacement,
    // rio deleted along with machu-picchu) and has done nothing wrong. What would be news
