...
import { SHARE_FILE_STEM } from '../src/app/Publication';
...
test('the share window switches to the text card picture, saves and shares it, and copies the text card', async ({ page }) => {
  ...
  await page.addInitScript(() => {
    if (!localStorage.getItem('silicon-rush-world-tour.save.v1')) localStorage.setItem('silicon-rush-world-tour.save.v1', JSON.stringify({ version: 1, language: 'en', volume: 0, muted: true, best: {}, names: ['Mia', ''] }));
  ...
  for (const lang of ['en', 'zh']) {
    ...
    expect(download.suggestedFilename()).toMatch(new RegExp(`^${SHARE_FILE_STEM}-text-card-synth-loop-\\d{8}-\\d{6}\\.png$`));
