...
test('the nickname is typed in settings, cleaned and remembered', async ({ page }) => {
  ...
  await expect.poll(() => page.evaluate(() => JSON.parse(localStorage.getItem('silicon-rush-world-tour.save.v1')!).names)).toEqual(['Mia the fast', '']);
...
async function finishWith(page: Page, vehicles: string[], times: number[]) {
  ...
  await page.addInitScript(() => {
    if (!localStorage.getItem('silicon-rush-world-tour.save.v1')) localStorage.setItem('silicon-rush-world-tour.save.v1', JSON.stringify({ version: 1, language: 'en', volume: 0, muted: true, best: {}, names: ['Mia', 'Kai'] }));
...
test('the share dialog offers the nickname as an optional press, and the card and dare pick it up', async ({ page }) => {
  await page.setViewportSize({ width: 1280, height: 720 });
  await page.addInitScript(() => { if (!localStorage.getItem('silicon-rush-world-tour.save.v1')) localStorage.setItem('silicon-rush-world-tour.save.v1', JSON.stringify({ version: 1, language: 'en', volume: 0, muted: true, best: {} })); });
  await page.goto('/?dev=1'); await page.waitForFunction(() => window.game);
  ...
  expect(await page.evaluate(() => JSON.parse(localStorage.getItem('silicon-rush-world-tour.save.v1')!).names)).toEqual(['Mia', '']);
