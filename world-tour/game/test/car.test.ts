...
describe('Car', () => {
  ...
  it('does not pull a high normal-speed breach back through the guardrail', () => {
    ...
    const at = car.position;
    physics.setBodyPosition(car.body, { x: 6.85, y: at.y, z: at.z });
    car.body.setLinvel({ x: 26, y: 0, z: -1 }, true);
  ...
  it('keeps low-speed and shallow-angle hits inside but permits a hard direct breakout', () => {
    ...
    // The original broke out at 11 m/s across the rail; remixes put it at 100 km/h, 60 degrees.
    expect(strike(0, 11), 'the original breakout speed now bounces back').toBeLessThan(6);
    expect(strike(15.7, 15.7), '80 km/h at 45 degrees bounces back').toBeLessThan(6);
    expect(strike(0, 26), 'a racing-speed head-on strike may break out').toBeGreaterThan(6);
    // Fast enough across the rail (26 m/s) but only 52 degrees into it: the speed-only rule broke it out.
    expect(strike(20, 26), 'a fast strike shallower than 60 degrees bounces back').toBeLessThan(6);
  });
...
it('finds the ground under any road on Earth without being told how high to look', () => {
  // The default reach was 250 m down to -250 m: every road above it (a remix's Lhasa at 3,650 m,
  // Zhangjiajie, Fuji -- also Machu Picchu and Rio before both were deleted) had no ground
  // at all for a query without bounds, and the falling slimes that land beside the car never appeared there.
  const physics = new PhysicsWorld(api, -9.81);
  ground(physics, 200, 3650);
  physics.world.step();   // publish the new collider to the query index
  expect(physics.surfaceAt(10, 10)?.point.y).toBeCloseTo(3650, 3);
  physics.dispose();
});
