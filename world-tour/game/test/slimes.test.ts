...
import { MIN_SPLIT_RADIUS, isSmallestSplitter, SLIME_BODY_OPACITY, SLIME_COLORS, SLIME_DROPLET_OPACITY,
  SlimeLayer, BURST_MAX_RISE, BURST_REFERENCE_MASS, burstCrossedCentre, burstImpulse, burstSparkScale, pieceVariation, burstSignedDistance, colossusContains,
...
it('launches a car lighter than the reference mass no faster than a car at that mass', () => {
  const centre = new THREE.Vector3(), scale = [1.2, 1, 1.2];
  const speed = (mass: number) => burstImpulse(scale, centre, centre.clone(), 1, mass).y / mass;
  expect(speed(600)).toBeCloseTo(speed(BURST_REFERENCE_MASS), 9);
  expect(speed(680)).toBeCloseTo(speed(BURST_REFERENCE_MASS), 9);
  // Heavier bodies keep the fixed impulse, so the launch still falls off with weight.
  expect(burstImpulse(scale, centre, centre.clone(), 1, 1800).y).toBeCloseTo(burstImpulse(scale, centre, centre.clone()).y, 9);
  expect(speed(1800)).toBeLessThan(speed(BURST_REFERENCE_MASS));
});

it('launches no car faster than the rise cap, however large the bomb', () => {
  const centre = new THREE.Vector3(), large = [3.4, 2.8, 3.4];
  for (const mass of [650, 900, 1180]) expect(burstImpulse(large, centre, centre.clone(), 1, mass).y / mass).toBeCloseTo(BURST_MAX_RISE, 9);
  // A heavy bus never reached the cap, and a small bomb stays below it.
  expect(burstImpulse(large, centre, centre.clone(), 1, 4200).y / 4200).toBeLessThan(BURST_MAX_RISE);
  expect(burstImpulse([.8, .66, .8], centre, centre.clone(), 1, 900).y / 900).toBeLessThan(BURST_MAX_RISE);
});
...
it('shares a rear-only burst across the actual ball-hitch rig without creating impulse', () => {
  ...
  try {
    ...
    const expected = burstImpulse(live.spawn.scale, new THREE.Vector3(...live.spawn.position),
      position, rear.tuning.ramMultiplier, rear.body.mass()).y;
