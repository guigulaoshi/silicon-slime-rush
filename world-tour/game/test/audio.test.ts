import { RACE_PIECES, ROTATION } from '../src/audio/music';
...
describe('music mix and lifecycle', () => {
  ...
  it('moves to the next race piece for each new race, keeps it through pause and restart, and brings the menu loop back', async () => {
    ...
    audio.setScene('menu');
    expect(audio.musicPiece).toBe('menu');
    expect(audible()).toEqual([1, ...RACE_PIECES.map(() => 0)]);
    audio.dispose();
    expect(made[0]!.buffers.every(source => source.stops === 1)).toBe(true);
  });

  it("plays the route's own regional loop every race there, and the rotation again on a route without one", async () => {
    const { audio } = harness();
    await audio.unlock();
    const race = (): string => { audio.setScene('countdown'); audio.setScene('racing'); const id = audio.musicPiece; audio.setScene('menu'); return id; };
    audio.setTrack('lhasa');
    expect([race(), race()]).toEqual(['amalehuo', 'amalehuo']);
    // A second route with its own loop, so a fixed regional loop is told apart from the rotation below.
    audio.setTrack('fuji');
    expect(race()).toBe('sakura');
    audio.setTrack('synth-loop');
    expect(ROTATION).toContain(race());
    audio.dispose();
  });
