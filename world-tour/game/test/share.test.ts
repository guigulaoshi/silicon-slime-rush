...
it('uses the exact bilingual Tiananmen Square name in result sharing', () => {
  // wolfe-pruneridge (Big Tech Rooftop) is gone with the rest of the Bay Area set; beijing's
  // localized name comes straight from src/ui/locales/{en,zh}.json's track.beijing.name.
  const result: ResultFacts = { trackId: 'beijing', time: 83.4,
    score: 12, slimeHits: 3, best: null, isBest: false };
  expect(shareLines(new I18n('en'), result, null)[1]).toBe('Tiananmen Square');
  expect(shareLines(new I18n('zh'), result, null)[1]).toBe('天安门广场');
});
...
it('points the coffee card at the verified game page\'s own itch.io tip page', () => {
  ...
  expect(creatorLinkUrl('homepage', 'https://guigulaoshi.itch.io/silicon-slime-rush-world-tour')).toBe('https://guigulaoshi.itch.io/');
...
it('stamps name, icon and address inside the bottom-right corner of every image size', () => {
  expect(watermarkAddress(null)).toBe('guigulaoshi.itch.io/silicon-slime-rush-world-tour');
  ...
  for (const language of ['zh', 'en'] as const) {
    ...
    expect(texts).toEqual([t.t('app.title'), 'guigulaoshi.itch.io/silicon-slime-rush-world-tour']);
...
it('names every saved picture and clip after its route and local time, so two saves never collide', () => {
  // trackId is an opaque string here, not validated against the catalogue -- sydney and zhangjiajie
  // stand in for lombard/twin-peaks (both deleted with the rest of the Bay Area set), then for
  // rio/twin-peaks.
  const at = new Date(2026, 8, 18, 7, 12, 5);
  // prefix is 'silicon-slime-rush-world-tour' (src/ui/ShareDialog.ts), the product name for this remix.
  expect(shareFileName({trackId: 'sydney', direction: 'forward'}, 'png', '', at)).toBe('silicon-slime-rush-world-tour-sydney-20260918-071205.png');
  expect(shareFileName({trackId: 'zhangjiajie', direction: 'reverse'}, 'mp4', '', at)).toBe('silicon-slime-rush-world-tour-zhangjiajie-reverse-20260918-071205.mp4');
  expect(shareFileName({trackId: 'sydney'}, 'png', 'text-card', at)).toBe('silicon-slime-rush-world-tour-text-card-sydney-20260918-071205.png');
  expect(shareFileName(null, 'png', '', at)).toBe('silicon-slime-rush-world-tour-20260918-071205.png');
  expect(shareFileName({trackId: 'sydney'}, 'webm', '', new Date(2026, 8, 18, 7, 12, 6)))
    .not.toBe(shareFileName({trackId: 'sydney'}, 'webm', '', at));
});
