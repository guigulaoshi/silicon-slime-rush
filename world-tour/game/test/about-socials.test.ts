...
it('takes the addresses the billboards already carry from the billboard manifest', () => {
  ...
  const copies = manifest.faces.flatMap(face => face.kind === 'social' ? [face.zh, face.en] : []);
  const target = (platform: string) => copies.find(copy => copy.platform === platform)!.target;
  ...
  const without = {...manifest, faces: manifest.faces.filter(face => face.en.platform !== 'x' && face.zh.platform !== 'x')};
