...
import { LAMP_STYLES, type LampStyleName, NightScenery, POOL_BEND_GLSL, POOL_BEND_KNOTS, POOL_CLEARANCE, nightLightStations } from '../src/world/NightScenery';
...
describe('night scenery', () => {
  ...
  it.each(Object.keys(LAMP_STYLES) as LampStyleName[])('the %s lamp stands on the ground and shines from inside its own glass', (name) => {
    const style = LAMP_STYLES[name];
    const body = style.body(); body.computeBoundingBox();
    const lens = style.lens(); lens.computeBoundingBox();
    expect(body.boundingBox!.min.y).toBeCloseTo(0, 1);
    const [x, y] = style.head;
    // the cone starts at the head, so the head has to be in the glass or the beam leaves thin air
    expect(lens.boundingBox!.containsPoint(new THREE.Vector3(x, y, 0))
      || lens.boundingBox!.distanceToPoint(new THREE.Vector3(x, y, 0)) < 0.35).toBe(true);
    const lit = new NightScenery(line(1_000), 'night', name);
    expect(lit.drawCalls).toBe(4);
    const built = (lit.root.getObjectByName('night-lamp-bodies') as THREE.InstancedMesh).geometry;
    expect(built.getAttribute('position').count).toBe(body.getAttribute('position').count);
    lit.dispose();
  });

  it('keeps a complete four-draw light chain at night, and by day only a named style\'s unlit posts', () => {
    ...
    // A route that names no lamps keeps the original's empty day.
    expect(day.count).toBe(0);
    expect(day.drawCalls).toBe(0);
    expect(day.root.children).toHaveLength(0);
    // 592: one that names them shows the posts by day -- two draws, no light: no cones, no pools,
    // glass that does not glow.
    const named = new NightScenery(spline, 'day', 'huabiao');
    expect(named.count).toBe(night.count);
    expect(named.drawCalls).toBe(2);
    expect(named.root.children.map(c => c.name)).toEqual(['night-lamp-bodies', 'night-lamp-lenses']);
    const dayGlass = (named.root.getObjectByName('night-lamp-lenses') as THREE.InstancedMesh).material;
    expect(dayGlass).toBeInstanceOf(THREE.MeshStandardMaterial);
    named.dispose();
    night.dispose();
    day.dispose();
  });
