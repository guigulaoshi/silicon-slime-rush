...
const ZH_PLATFORMS = ['bilibili', 'xiaohongshu', 'douyin', 'wechat'];
const EN_PLATFORMS = ['x', 'youtube', 'tiktok'];
...
describe('the shipped manifest', () => {
  it('fills fourteen alternating slots with seven owner ads and seven available spaces', () => {
    ...
    //
    // reversing): the Chinese copies take turns among the four Chinese platforms, the English
    // copies among the three English ones, so every platform of that language shows in a cycle.
    const social = m.faces.filter((f) => f.kind === 'social');
    expect(new Set(social.map((f) => f.zh.platform))).toEqual(new Set(ZH_PLATFORMS));
    expect(new Set(social.map((f) => f.en.platform))).toEqual(new Set(EN_PLATFORMS));
  });
  ...
  it('says something in both languages on every face', () => {
    for (const f of shipped().faces) {
      ...
      if (f.kind === 'social') {
        expect(f.zh.image).toBe(`billboards/social-${f.zh.platform}-zh.png`);
        expect(f.en.image).toBe(`billboards/social-${f.en.platform}-en.png`);
      } else {
...
describe('loading', () => {
  ...
  it('swaps every social picture for that language\'s platforms when the language changes', async () => {
    const materials = new MaterialLibrary();
    const boards = new Billboards(materials);
    const fetcher = vi.fn().mockResolvedValue({ ok: true, json: async () => shipped() });
    ...
    const platforms = () => [...shown.values()].map((name) => /social-([a-z]+)-/.exec(name)?.[1]).filter(Boolean);
    expect(new Set(platforms())).toEqual(new Set(ZH_PLATFORMS));
    boards.setLanguage('en');           // the 中/EN switch, mid-race: no reload
    expect(new Set(platforms())).toEqual(new Set(EN_PLATFORMS));
