...
it('gives the star a run earns exactly on a threshold, not one lost to rounding', () => {
  // 0.9 x 2.4 s is the five-star pace exactly, but 2.16.../2.4... divides to 0.8999999999999999;
  // 3.5 points a metre over 0.9 m likewise divides to 3.4999999999999996.
  const seconds = 2.4000000000000012, reference = 0.9 * seconds;
  expect(raceRating(1, seconds, 6, reference)).toBe(5);
  const distance = 0.8999999999999999;
  expect(raceRating(distance, 10, 3.5 * distance, 7.5)).toBe(4);
});
