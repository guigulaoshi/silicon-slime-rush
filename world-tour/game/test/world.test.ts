 ...
 *
 * `where` here is only a label baked into failure messages by expectWorldLoaded (e2e/world.ts) --
 * it is never matched against a real track id, so 'sydney' (goldengate's replacement, the mapping
 * table's showcase/default track) is just for a reader's sake.
 *
...
describe('the empty-world guard', () => {
  it('passes a track whose tiles are loaded and none of which failed', async () => {
    await expectWorldLoaded(page({ loaded: 37, loading: 2, failed: 0 }), 'sydney');
  });
  ...
  it('fails when every tile 404s, and names the command that fixes it', async () => {
    ...
    await expect(expectWorldLoaded(page({ loaded: 0, loading: 0, failed: 30 }), 'sydney'))
  ...
  it('fails when a track is only half built, not just when it is wholly missing', async () => {
    ...
    await expect(expectWorldLoaded(page({ loaded: 12, loading: 0, failed: 4 }), 'sydney'))
  ...
  it('fails when nothing is resident even though nothing has failed yet', async () => {
    ...
    await expect(expectWorldLoaded(page({ loaded: 0, loading: 3, failed: 0 }), 'sydney'))
      .rejects.toThrow(/empty world/);
  });

  it('fails when the game reports no streamer at all', async () => {
    await expect(expectWorldLoaded(page(null), 'sydney'))
  ...
  it('fails when the tiles arrived but no material is textured', async () => {
    ...
    await expect(expectWorldLoaded(page({ loaded: 37, loading: 0, failed: 0 }, 0), 'sydney'))
      .rejects.toThrow(/no material is textured/);
  });

  it('fails when the game reports no materials at all', async () => {
    await expect(expectWorldLoaded(page({ loaded: 37, loading: 0, failed: 0 }, null), 'sydney'))
