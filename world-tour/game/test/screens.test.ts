...
it('renders the same score in the current language at finish', () => {
  const i18n = new I18n('en');
  const facts = { trackId: 'lhasa', time: 20, best: null, isBest: true,
...
it('renders the exact bilingual Tiananmen Square name at finish', () => {
  const i18n = new I18n('en');
  // wolfe-pruneridge (Big Tech Rooftop) is gone with the Bay Area tracks; beijing is the
  // shipping track whose translated name this same lookup now has to get right.
  const facts = { trackId: 'beijing', time: 20, best: null,
    isBest: false, score: 0, slimeHits: 0 };
  const screen = resultsScreen(i18n, () => facts, new MenuList([]), () => {});
  screen.render();
  expect(screen.node.querySelector('.result-track')!.textContent).toBe('Tiananmen Square');
  i18n.set('zh'); screen.render();
  expect(screen.node.querySelector('.result-track')!.textContent).toBe('天安门广场');
});

it('reveals earned stars in order and skips the animation with reduced motion', () => {
  vi.useFakeTimers();
  const sound = vi.fn();
  const facts = { trackId: 'lhasa', time: 80, best: null, isBest: true,
