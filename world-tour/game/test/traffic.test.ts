...
it('queues through the real Zhangjiajie switchbacks instead of reversing into a parallel passing lane', () => {
  // rio (Corcovado climb) is gone; zhangjiajie is the
  // remaining track with the same property this test needs -- a hairpin tight enough to force single
  // file. Summing |curvature| * 2 over s=742..782 (the 40 m window the loop in planTraffic scans from
  // the car's position) gives about 1.90 rad -- computed directly from spline.curvature in
  // game/public/tracks/zhangjiajie/track.json -- almost twice the singleFile threshold of PI/3
  // (about 1.05), so this is deep in a hairpin rather than near its edge.
  const track = JSON.parse(readFileSync('public/tracks/zhangjiajie/track.json', 'utf8')) as TrackData;
  const spline = new Spline(track);
  const at = { s: 742, lateral: 0, index: spline.indexAt(742) };
  const own = [{ driver: 'me', s: 742, lateral: 0, halfWidth: 1, halfLength: 2, speed: 2 }];
  const leader = { ...own[0]!, driver: 'leader', s: 748, speed: 0 };
...
it('The autopilot brakes for a bend on the grip the weather leaves', () => {
  // rio (Corcovado climb) is gone; zhangjiajie has the
  // same shape this test needs -- a near-straight run into a tight bend -- read directly off
  // game/public/tracks/zhangjiajie/track.json spline.curvature: it stays low out to s=110
  // (0.0004-0.013 across s=80..115) and then climbs sharply into the bend, peaking at 0.061 around
  // s=130 before easing off by s=145. 16 m/s clears it dry (brake 0) but not on the .62 grip snow
  // leaves (brake 1); 15 and 17 m/s both hold too, so this is not a knife-edge pick.
  const track = JSON.parse(readFileSync('public/tracks/zhangjiajie/track.json', 'utf8')) as TrackData;
  const spline = new Spline(track);
  const index = spline.indexAt(120), point = spline.point(index), tangent = spline.tangent(index);
  const at = { s: 120, lateral: 0, index };
