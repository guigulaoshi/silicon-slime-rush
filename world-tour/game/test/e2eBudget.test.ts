...
// Named after the showcase route the baseline drives (game/src/app/showcase.ts).
const BASELINE_DIR = resolve(process.cwd(), '..', 'tools', 'baselines', 'sydney');
// The eight the visual baseline compares against, from baseline.spec.ts's MILESTONES (re-anchored to
// sydney: game/public/tracks/sydney/track.json, the 3115 m harbour-bridge route).
const MILESTONE_SHOTS = ['01-start', '02-bridge-mid', '03-bridge-south-end', '04-off-the-bridge',
                         '05-the-rocks-hairpin', '06-circular-quay', '07-macquarie-street',
                         '08-opera-house-forecourt'].map((n) => `${n}.png`);
