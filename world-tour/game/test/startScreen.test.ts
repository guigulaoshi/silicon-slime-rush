...
describe('the map', () => {
  ...
  it('draws the selected route as a lap, and the coast only as a locator', () => {
    const { q } = mount();
    ...
    // far-south is 150 km outside the mapped region, so it is not drawn there at all -- it is
    // still in the list, which is now the control, so nothing about it is unreachable.
  ...
  it('leaves out a route this build cannot play', () => {
    const { q } = mount({ playable: (id: string) => id !== 'zhangjiajie' });
...
describe('walking the three setup steps', () => {
  it('restores the complete previous start choice', () => {
    const { screen, q, started } = mount({
      initialChoice: () => ({ trackId: 'lhasa', direction: 'reverse', playerCount: 2,
        playerVehicles: ['sports-car', 'jeep'], timeOfDay: 'night', weather: 'rain', slimeDensity: 'many',
        ai: true, aiDifficulty: 'rush' }),
    });
    expect(q('.sm-item.sel')[0]!.getAttribute('data-track')).toBe('lhasa');
    ...
    expect(started[0]).toMatchObject({ trackId: 'lhasa', direction: 'reverse',
  ...
  it('resolves the real route defaults, ignores stale responses, and retains per-route drafts', async () => {
    ...
    pending.get('lhasa')!({ vehicleId: 'school-bus', timeOfDay: 'day' });
    await Promise.resolve();
    pending.get('sydney')!({ vehicleId: 'jeep', timeOfDay: 'night' });
    await Promise.resolve();
    ...
    expect(started[0]).toMatchObject({ trackId: 'lhasa', vehicleId: 'retro-van', timeOfDay: 'day' });
  });
  ...
  it('keeps explicit time and density when going back and changing the route', () => {
    ...
    screen.handle('confirm'); screen.handle('confirm'); screen.handle('confirm');
    expect(started[0]).toMatchObject({ trackId: 'zhangjiajie', timeOfDay: 'night', slimeDensity: 'none' });
  });
  ...
  it('keeps a prepared race confirmable during an input-device render', async () => {
    ...
    expect(started[0]).toMatchObject({ vehicleId: 'jeep', trackId: 'sydney' });
  });
  ...
  it('hands back the route, car, time and independent weather that were picked', () => {
    ...
    (q('.sm-item')[1] as HTMLElement).click();       // lhasa
    go(screen);                                                   // world
    (q('[data-time="night"]')[0] as HTMLElement).click();
    ...
    go(screen);
    expect(started).toEqual([{ direction: 'forward', trackId: 'lhasa', vehicleId: 'pickup-travel-trailer', timeOfDay: 'night',
  ...
  it('goes back a step without losing what was already chosen', () => {
    ...
    expect(started[0]).toMatchObject({ trackId: 'zhangjiajie', vehicleId: 'micro-hatch' });
  });
...
describe('keyboard and gamepad', () => {
  it('gets from the map to the road without a pointer', () => {
    ...
    expect(started).toEqual([{ direction: 'forward', trackId: 'lhasa', vehicleId: 'sports-car', timeOfDay: 'night',
...
describe('a friend\'s challenge', () => {
  it('opens on the challenged route and direction, keeps an open challenged car, and says what to beat', () => {
    let dare: import('../src/app/Challenge').Challenge | null = { trackId: 'zhangjiajie', direction: 'reverse', time: 95.4, rating: 4,
      score: 8200, vehicleId: 'micro-hatch', name: 'Ada' };
    const { screen, q, started } = mount({
      initialChoice: () => ({ trackId: 'lhasa', direction: 'forward', playerCount: 1, playerVehicles: ['jeep'],
    ...
    expect(q('.sm-item.sel')[0]!.getAttribute('data-track')).toBe('zhangjiajie');
    ...
    expect(started[0]).toMatchObject({ trackId: 'zhangjiajie', direction: 'reverse', vehicleId: 'micro-hatch' });
  ...
  it('keeps an open dared car on a new save once the route defaults arrive, and never pulls back after the first time', async () => {
    const dare = { trackId: 'zhangjiajie', direction: 'forward' as const, time: 95.4, rating: 4 as const, score: 8200, vehicleId: 'jeep' };
    const { screen, q, started } = mount({
      challenge: () => dare,
    ...
    expect(started[0], 'the dared jeep, not the route default').toMatchObject({ trackId: 'zhangjiajie', vehicleId: 'jeep' });
    // Choosing another route hides the dare: its button would start that route and never compare.
    const other = q('.sm-item').find(item => item.getAttribute('data-track') !== 'zhangjiajie') as HTMLElement;
  ...
  it('takes the challenged car, since every car is open', () => {
    const { screen, q, started } = mount({
      challenge: () => ({ trackId: 'lhasa', direction: 'forward', time: 80, rating: 2, score: 10, vehicleId: 'sports-car' }),
    });
    screen.reset(); screen.render();
    screen.handle('confirm'); screen.handle('confirm'); screen.handle('confirm');
    expect(started[0]!.trackId).toBe('lhasa');
...
it('selects direction by button or keyboard and reads the matching record', () => {
  ...
  screen.handle('confirm');
  (q('[data-direction=reverse]')[0] as HTMLElement).click();
  expect(bestLabel).toHaveBeenLastCalledWith('sydney','reverse','micro-hatch');
  screen.handle('confirm'); screen.handle('confirm');
...
it('has no join control on a tablet, including when restoring a desktop two-player save',()=>{
 ...
 const {screen,q}=mount({initialChoice:()=>({trackId:'lhasa',direction:'forward',playerCount:2,
...
it('keeps the menu wordmark visible and translates its accessible name with the header switch', () => {
  ...
  // uppercases the English for the aria-label, and the wordmark spans read 'Silicon Slime Rush World Tour'.
  const { screen, i18n, q } = mount();
  expect(q('.sm-name .brand-mark')).toHaveLength(1);
  expect((q('.sm-name')[0] as HTMLElement).textContent).toBe('Silicon Slime Rush World Tour');
  expect(q('.sm-name')[0]!.getAttribute('aria-label')).toBe('史莱姆赛车：环游世界');
  ...
  i18n.set('en'); screen.render();
  expect((q('.sm-name')[0] as HTMLElement).textContent).toBe('Silicon Slime Rush World Tour');
  expect(q('.sm-name')[0]!.getAttribute('aria-label')).toBe('SILICON SLIME RUSH: WORLD TOUR');
...
describe('quick race from the home page', () => {
  it('starts the last race in one press and says what it will start', () => {
    const { screen, q, started, i18n } = mount({
      showHome: true,
      initialChoice: () => ({ trackId: 'zhangjiajie', direction: 'reverse', playerCount: 1, playerVehicles: ['jeep'],
    ...
    quick.click();
    expect(started).toHaveLength(1);
    expect(started[0]).toMatchObject({ trackId: 'zhangjiajie', direction: 'reverse', vehicleId: 'jeep', timeOfDay: 'night',
...
it('shows the route\'s own car on the card of the slot it stands in', () => {
  // A local car keeps its slot's button id, but the card is what the player picks by: on Lhasa the
  // jeep's card showed the jeep's picture and name while the car that drove was the plateau SUV.
  const { screen, i18n, q } = mount({ initialChoice: () => ({ trackId: 'lhasa', direction: 'forward', playerCount: 1,
    playerVehicles: ['sports-car'], timeOfDay: 'day', weather: 'clear', slimeDensity: 'normal', ai: true, aiDifficulty: 'relaxed' }) });
  screen.handle('confirm'); screen.handle('confirm');
  const card = q('[data-vehicle="jeep"]')[0]!;
  expect(card.querySelector('img')!.getAttribute('src')).toBe('./garage/lhasa-plateau-suv.webp');
  expect(card.querySelector('.sm-carnm')!.textContent).toBe(i18n.t('car.lhasa-plateau-suv.name'));
  expect(q('[data-vehicle="sports-car"] img')[0]!.getAttribute('src'), 'other slots stay themselves').toBe('./garage/sports-car.webp');
});

it('keeps the garage car cards when a render changes nothing they show, and rebuilds them when it does', () => {
  // The first World Tour's: `render()` runs whenever the input device changes, and on an
  // iPad the first tap on the car is such a change. Rebuilding the cards every render replaced the
  // row with fresh nodes, which the player saw as the cards popping in again on every touch.
  const { screen, i18n, q } = mount();
  screen.handle('confirm'); screen.handle('confirm');
  const first = q('[data-vehicle]')[0]!;
  screen.render(); screen.render();
  expect(q('[data-vehicle]')[0], 'an input-device render must keep the same card node').toBe(first);
  expect(first.isConnected).toBe(true);
  i18n.set('en'); screen.render();
  const english = q('[data-vehicle]')[0]!;
  expect(english, 'a language change redraws the names and units').not.toBe(first);
  (q('.startup-add-player')[0] as HTMLElement).click();
  expect(q('[data-vehicle]')[0], 'a second player adds the right-hand row').not.toBe(english);
  expect(screen.playerCount).toBe(2);
});
