...
import { ALL_VEHICLES, catalogueErrors, defaultVehicle, modelPath, resolveVehicle, VEHICLES,
...
describe('single vehicle catalogue', () => {
  ...
  it('shares axle ordering and suspension offsets without exposing mutable geometry', () => {
    for (const body of ALL_VEHICLES) {
...
describe('loaded model contract', () => {
  // Local cars drive out of the same slots, so every body in the catalogue answers to the same contract.
  const bodies = ALL_VEHICLES.flatMap(v => [v, ...(v.trailer ? [v.trailer] : [])]);
  ...
  it('reports the combined exported body and garage-card size', () => {
    ...
    expect(bytes).toHaveLength(ALL_VEHICLES.length + 1);
    const menuBytes = ALL_VEHICLES.map(vehicle => readFileSync(resolve('public/garage', `${vehicle.id}.webp`)).byteLength);
    expect(menuBytes).toHaveLength(ALL_VEHICLES.length);
...
it('every catalogue thumbnail was rendered from its current production bodies', () => {
  ...
  expect(Object.keys(records).sort()).toEqual(ALL_VEHICLES.map(vehicle => vehicle.id).sort());
