...
describe('player nicknames', () => {
  ...
  it('labels a card with the nickname, falls back to left/right driver, and leaves an unnamed solo card as it was', () => {
    const t = new I18n('en');
    // goldengate's replacement (mapping table): the showcase/default track.
    const solo: ResultFacts = { trackId: 'sydney', time: 95.4, score: 1200, slimeHits: 3, best: null, isBest: false, rating: 3 };
  ...
  it('travels cleaned in a challenge code', () => {
    ...
    // must be a real CATALOGUE id: decodeChallenge rejects anything not in `known.tracks`, and
    // goldengate is gone with the rest of the Bay Area set.
    const code = encodeChallenge({ trackId: 'sydney', direction: 'forward', time: 90, rating: 2, score: 5, name: 'Mia‮\nX' });
