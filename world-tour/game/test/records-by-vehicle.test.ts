...
// goldengate's replacement (mapping table): sydney. recordKey/record/ghost take opaque strings
// with no catalogue validation, so any id works, but a real current one reads clearer.
const sports = recordKey('sydney', 'sports-car'), truck = recordKey('sydney', 'monster-truck');
...
it('keeps a separate best, rating and ghost for each car on the same route and direction', () => {
  ...
  expect(save.best(recordKey('sydney:reverse', 'sports-car'))).toBeNull();
});

it('files a route-only record from an older save under the car that most likely set it, losing nothing', () => {
  // goldengate/lombard/shoreline here are literal old-save data being migrated (real ids from
  // before the Bay Area set was deleted), not references to a live track -- left as-is on purpose.
