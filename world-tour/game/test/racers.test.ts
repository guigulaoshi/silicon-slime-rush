...
//
// goldengate -> sydney: the new default/showcase track, same role in this
// grid-ordering test (any built route with real width and checkpoints would do).
it('grids AI fastest first ahead of the players, alternating sides of the centre line', () => {
  const track = loadTrack('sydney'), spline = new Spline(track);
  const humans = [vehicleFor('micro-hatch')!, vehicleFor('jeep')!];
  const roster = raceRoster(humans, true);
  // AI_PACE (src/track/aiPace.json) was wiped along with the Bay Area tracks and has no 'sydney' entry
  // yet (only SR_UPDATE_AI_PACE=1 npx vitest run test/autopilot.test.ts, a real run, can fill it back in).
  // aiReferenceSeconds falls back to the same physics-based IdealRace estimate Game.ts uses for any
  // route without a measured table, which still ranks vehicles by real per-vehicle top speed/handling.
  const seconds = (vehicle: VehicleDefinition) => aiReferenceSeconds('sydney', vehicle, spline, track);
  ...
  const ai = byPosition.slice(2).map(item => seconds(item.entry.vehicle));
