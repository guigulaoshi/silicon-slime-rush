...
it('exposes actual installed versions and asset inventory links through both translations', () => {
  ...
  expect(CREDITS.version).toMatch(/^\d+\.\d+\.\d+$/);
  ...
  // every source link in ASSETS.md's table reaches the credits, whatever the remix adds to it
  const tableLinks = readFileSync('../ASSETS.md', 'utf8').split('\n').filter(line => line.startsWith('|'))
    .flatMap(line => [...line.matchAll(/\]\((https:\/\/[^)]+)\)/g)]).length;
  expect(tableLinks).toBeGreaterThanOrEqual(4);
  expect(CREDITS.sourceLinks).toHaveLength(tableLinks);
  for (const lang of ['en', 'zh'] as const) {
    ...
    // The source links, one more link, and the creator's eight accounts.
    expect(view.node.querySelectorAll('a')).toHaveLength(tableLinks + 9);
