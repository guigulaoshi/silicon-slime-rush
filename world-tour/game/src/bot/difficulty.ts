...
export function aiSettingsFor(tuning: CarTuning, difficulty: AiDifficulty, towedMass = 0): AutopilotSettings {
  const skill = SKILL[difficulty];
  const effort = AI_EFFORT[difficulty];
  const mass = tuning.mass + towedMass;       // the caravan is stopped by the tractor's brakes
  return { ...autopilotSettingsFor(tuning, towedMass),
    ...
    brakingAccel: tuning.brakeForce / mass * effort.brakes,
    stopDecel: tuning.brakeForce / mass * .9,
    brakeMargin: 1.015,
