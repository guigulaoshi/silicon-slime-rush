...
const GRAVITY = 9.81;
/** Metres per second a stopped car is allowed to creep at when only geometry holds it back. */
const CRAWL_SPEED = 1.5;
/** Ceilings that mean "stop here": a crawl must never override them. */
const HOLDING_LIMITS = new Set<string>(['stop', 'traffic', 'slime']);

/** Plan below the selected vehicle's physical limits, including avoidance lane changes. */
export function autopilotSettingsFor(tuning: CarTuning, towedMass = 0): AutopilotSettings {
  const avoidSpeed = Math.min(AUTOPILOT.avoidSpeed, Math.sqrt(tuning.maxLateralAccel * 20));
  // A towed caravan has no brakes of its own: the tractor's brakes stop both, so its mass is in the
  // divisor. Planned on the unladen tractor, a pickup with a caravan arrived at a 7.3 m corner at
  // 59 km/h and went up the kerb where the same pickup alone got round.
  const mass = tuning.mass + towedMass;
  return { ...AUTOPILOT,
    ...
    brakingAccel: Math.min(AUTOPILOT.brakingAccel, tuning.brakeForce / mass * .5),
    stopDecel: Math.min(AUTOPILOT.stopDecel, tuning.brakeForce / mass * .5),
...
export class Autopilot {
  ...
  drive(
    ...
    const hereY = this.spline.point(at.index)[1];
    for (let k = 0; k < this.spline.count; k++) {
      ...
      // Braking downhill, gravity takes its share of the brakes: plan on what is left over the
      // average grade to this point, never on less than half. A snowy descent into a right angle
      // otherwise put the AI straight off the road.
      const drop = scanDistance > 1 ? Math.max(0, hereY - this.spline.point(this.spline.wrapIndex(at.index + k))[1]) / scanDistance : 0;
      const braking = Math.max(s.brakingAccel * .5, s.brakingAccel - GRAVITY * drop);
      ...
      steeringCeiling = Math.min(steeringCeiling,
        Math.sqrt(steeringSpeed * steeringSpeed + 2 * braking * scanDistance));
      ...
      const here = Math.sqrt(there * there + 2 * braking * scanDistance);
      cap(here, 'corner');
    }
    ...
    // A geometric ceiling (steering, pursuit, corner, edge, recovery) can reach zero with the car
    // standing still and the geometry unchanged: `speed < ceiling` is then false for ever. One car
    // waited 29 minutes 0.51 m short of the finish. Stopped for no reason to stop, it crawls.
    if (car.speed < CRAWL_SPEED && ceiling < CRAWL_SPEED && !HOLDING_LIMITS.has(limit)
        && !this.waitingForTraffic) ceiling = CRAWL_SPEED;
    this.limit = limit;
