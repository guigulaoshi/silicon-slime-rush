 ...
 * that address and it is read from there (either language's copy: a board shows
 * Bilibili only in Chinese); the Douyin and Xiaohongshu boards lead to a video and a
...
export function creatorSocials(manifest: Manifest = __BILLBOARD_MANIFEST__): CreatorSocial[] {
  const board = (platform: string) =>
    manifest.faces.flatMap(face => face.kind === 'social' ? [face.zh, face.en] : [])
      .find(copy => copy.platform === platform)?.target ?? '';
  return [
