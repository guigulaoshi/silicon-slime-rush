...
import { PUBLIC_GAME_URL, SHARE_FILE_STEM, watermarkAddress } from '../app/Publication';
 ...
 * saves never share a name: silicon-slime-rush-world-tour-rio-reverse-20260918-071230.png. Seconds, not
...
export function shareFileName(result: Pick<ResultFacts, 'trackId' | 'direction'> | null, extension: string,
  ...
  return [SHARE_FILE_STEM, ...(kind ? [kind] : []), ...route, stamp].join('-') + '.' + extension;
}
...
export class ShareDialog {
  ...
  private async renderCard(): Promise<void> {
    ...
    ctx.fillStyle = tone('night', 'mist', 0.06); ctx.fillRect(0, 0, W, H);
    if (this.source) {
    ...
    gradient.addColorStop(0, tone('night', 'mist', 0.06, 0.0)); gradient.addColorStop(.55, tone('night', 'mist', 0.06, 0.702)); gradient.addColorStop(1, tone('night', 'mist', 0.06, 0.902));
    ...
    lines.forEach((line, index) => {
      ctx.fillStyle = index === 0 ? tone('stampDeep', 'stampLight', 0.56) : tone('night', 'mist', 0.98);
