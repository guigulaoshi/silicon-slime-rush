...
export interface MenuMap {
  version: number;
  /**
   * A whole-world map: the ground is sea, `land` is the continents drawn on it, and `water` is the
   * seas they close round, drawn back over the land. The region is shown whole, never trimmed to the
   * panel -- trimming a world to a square cuts off New York or Sydney.
   */
  world?: boolean;
  region: Region;
...
export function parseMenuMap(data: unknown): MenuMap {
  ...
  if (raw.world !== undefined && typeof raw.world !== 'boolean') bad('world is not a boolean');
  return {
    version: 5,
    ...(raw.world ? { world: true } : {}),
