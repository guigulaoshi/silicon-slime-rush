...
export function drawResultImage(canvas: HTMLCanvasElement, t: I18n, result: ResultFacts, source: ShareImage | null): void {
  ...
  ctx.fillStyle = tone('night', 'mist', 0.2); ctx.fillRect(0,0,1000,600);
  if (source) ctx.drawImage(source,0,0,1000,600);
  ctx.fillStyle = tone('night', 'mist', 0.11, 0.749); ctx.fillRect(0,0,1000,600);
  ...
  ctx.font = '20px Arial,"PingFang SC",sans-serif'; ctx.fillStyle = tone('night', 'mist', 0.83); ctx.fillText(t.t('replica.cardPlace'),placeX,130,952 - placeX);
  const people = result.players ?? [result], dual = people.length > 1;
  people.forEach((person,index) => {
    const x = dual ? 48 + index * 466 : 48, width = dual ? 435 : 904;
    ctx.fillStyle = tone('goldDeep', 'goldLight', 0.64); ctx.font = '46px Arial,"PingFang SC",sans-serif';
    ...
    ctx.fillStyle = tone('accentDeep', 'accentLight', 0.67); ctx.font = `${dual ? 72 : 90}px Arial,"PingFang SC",sans-serif`; ctx.fillText(person.score.toLocaleString(t.lang),x,326,width);
  ...
  if (feat) {
    ...
    ctx.fillStyle = feat.kind === 'best' ? tone('accentDeep', 'accentLight', 0.42) : tone('seaDeep', 'seaLight', 0.67);
    // Below the name row: the headline keeps the top line to itself.
    roundRect(ctx, 952 - boxWidth, 152, boxWidth, 54, 12); ctx.fill();
    ctx.fillStyle = tone('night', 'mist', 0.1); ctx.fillText(label, 952 - boxWidth + pad, 189, textWidth);
  }
  drawVisaStamp(ctx, t, result.trackId, 850, 468, new Date());
  ctx.fillStyle='#fff';ctx.font='22px Arial,"PingFang SC",sans-serif';ctx.fillText(t.t('replica.realRoads'),48,536,700);
}

/** The result card's entry stamp (ResultsScreen `visaStamp`) on the shared picture, bottom right. */
function drawVisaStamp(ctx: CanvasRenderingContext2D, t: I18n, trackId: string, cx: number, cy: number, today: Date): void {
  const code = CATALOGUE.find(entry => entry.id === trackId)?.code;
  if (!code) return;
  const date = t.lang === 'zh' ? today.toLocaleDateString('zh-CN', { year: 'numeric', month: 'long', day: 'numeric' })
    : `${String(today.getDate()).padStart(2, '0')} ${today.toLocaleDateString('en-US', { month: 'short' }).toUpperCase()} ${today.getFullYear()}`;
  ctx.save();
  ctx.translate(cx, cy); ctx.rotate(-9 * Math.PI / 180);
  ctx.globalAlpha = .85;
  const ink = tone('stampDeep', 'stampLight', 0.3);
  ctx.strokeStyle = ink; ctx.fillStyle = ink; ctx.textAlign = 'center';
  ctx.lineWidth = 3; roundRect(ctx, -92, -58, 184, 116, 14); ctx.stroke();
  ctx.lineWidth = 1.5; roundRect(ctx, -84, -50, 168, 100, 10); ctx.stroke();
  ctx.font = 'bold 15px Arial,"PingFang SC",sans-serif'; ctx.fillText(t.t('results.stampArrived'), 0, -26);
  ctx.font = '900 44px Arial,"PingFang SC",sans-serif'; ctx.fillText(code, 0, 18);
  ctx.beginPath(); ctx.moveTo(-60, 26); ctx.lineTo(60, 26); ctx.stroke();
  ctx.font = 'bold 14px Arial,"PingFang SC",sans-serif'; ctx.fillText(date, 0, 44);
  ctx.restore();
}
