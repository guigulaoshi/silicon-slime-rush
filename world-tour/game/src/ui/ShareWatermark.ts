...
export function drawWatermark(ctx: CanvasRenderingContext2D, t: I18n, icon: HTMLImageElement | null, address: string): void {
  ...
  ctx.save();
  ctx.fillStyle = tone('night', 'mist', 0.04, 0.722);
  const r = 8 * unit;
  ...
  ctx.fillStyle = tone('accentDeep', 'accentLight', 0.67); ctx.font = `bold ${Math.round(16 * unit)}px Arial,"PingFang SC",sans-serif`;
  ctx.fillText(t.t('app.title'), textX, y + pad + 15 * unit, textWidth);
  ctx.fillStyle = tone('night', 'mist', 0.98); ctx.font = `${Math.round(12 * unit)}px Arial,"PingFang SC",sans-serif`;
