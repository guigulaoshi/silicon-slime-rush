...
const SECTION_COLOURS = { fast: tone('accentDeep', 'accentLight', 0.42), slow: tone('stampDeep', 'stampLight', 0.56), none: tone('night', 'mist', 0.28) } as const;
...
function square(ctx: CanvasRenderingContext2D, kind: string, x: number, y: number, size: number, radius: number): void {
  ...
  if (kind === 'burst') { ctx.strokeStyle = tone('night', 'mist', 0.49); ctx.lineWidth = Math.max(2, size / 28); ctx.stroke(); }
}
...
export function drawTextCardImage(canvas: HTMLCanvasElement, t: I18n, result: ResultFacts): void {
  ...
  ground.addColorStop(0, tone('night', 'mist', 0.13)); ground.addColorStop(1, tone('night', 'mist', 0.04));
  ctx.fillStyle = ground; ctx.fillRect(0, 0, width, height);
  const left = 72, inner = width - left * 2;
  ctx.textBaseline = 'alphabetic';
  ctx.fillStyle = tone('accentDeep', 'accentLight', 0.67); ctx.font = `bold 60px ${FONT}`; ctx.fillText(t.t('app.title'), left, 140, inner);
  ctx.fillStyle = tone('night', 'mist', 0.98); ctx.font = `36px ${FONT}`; ctx.fillText(routeName(t, result.trackId, result.direction), left, 200, inner);
  ...
  if (feat) {
    ...
    ctx.fillStyle = tone('night', 'mist', 0.1); ctx.fillText(label, left + pad, 262, box - pad * 2);
  }
  ...
  players.forEach((player, index) => {
    ...
    if (player.rating) { ctx.fillStyle = tone('goldDeep', 'goldLight', 0.64); ctx.font = `${size.stars}px ${FONT}`; y += size.starGap; ctx.fillText(starGlyphs(player.rating), left, y, inner); }
    // Slimes hit: a coloured square per kind with its count.
    y += size.squareGap;
    if (!player.hits.length) { ctx.fillStyle = tone('night', 'mist', 0.79); ctx.font = `36px ${FONT}`; ctx.fillText(t.t('textCard.noHits'), left, y + size.square * .65, inner); }
    ...
    placed.forEach((spot, k) => {
      ...
      ctx.fillStyle = tone('night', 'mist', 0.98); ctx.fillText(`×${hit.count}`, spot.x + size.square + 12, y + spot.row * (size.square + 14) + size.square * .66);
    });
  ...
  if (result.challengeCode && !duo) {
    ctx.fillStyle = tone('accentDeep', 'accentLight', 0.67); ctx.font = `bold 38px ${FONT}`;
  ...
  for (const kind of ['popper', 'slick', 'burst', 'boost', 'colossus'] as const) {
    ...
    ctx.fillStyle = tone('night', 'mist', 0.87); ctx.fillText(label, x + 38, y); x += 38 + ctx.measureText(label).width + 30;
  }
  y += 46; x = left;
  for (const section of ['fast', 'slow', 'none'] as const) {
    ...
    ctx.fillStyle = tone('night', 'mist', 0.87); ctx.fillText(label, x + 54, y); x += 54 + ctx.measureText(label).width + 30;
  }
}
