...
import { fallbackVehicle, resolveVehicle, vehicleFor, VEHICLES, vehicleTuning, type VehicleDefinition, forTrack, taxiLivery } from '../vehicles/catalogue';
...
import { BILLBOARD_SLOTS, billboardFaceMaterial, type CarKind, type TimeOfDay, type TrackData } from '../track/types';
...
import { type AutoQualitySampler, autoQualitySamplerFor, autoQualityStart, startingQuality, QUALITY_LIMITS, type QualityLimits } from '../world/quality';
import { recordKey, Save } from './Save';
import { SHOWCASE_AT, SHOWCASE_IMAGE, SHOWCASE_SPIN, SHOWCASE_START_ANGLE, SHOWCASE_TRACK } from './showcase';
 ...
 */
/** Extra metres of loaded ground a car held for the road waits for before it is let go. */
export const ROAD_RELEASE_MARGIN = 12;
...
export class Game {
  ...
  private async share(home = false): Promise<void> {
    await this.shareDialog.show(home ? null : this.lastResult, async () => {
      if (home) { const image = new Image(); image.src = SHOWCASE_IMAGE; await image.decode(); return image; }
  ...
  private async load(trackId: string, vehicleId?: string, playerVehicles?: readonly string[], ai = false, difficulty: AiDifficulty = 'rush', direction: RaceDirection = this.choice?.direction ?? 'forward'): Promise<boolean> {
    ...
    const loadModel = (body: Parameters<typeof VehicleModel.load>[0]) => VehicleModel.load(body, request.signal,
      ...
      }).then(loaded => { loaded.applyLivery(taxiLivery(body.id, trackId)); return loaded; });
    try {
      ...
      const vehicle = forTrack(resolveVehicle(vehicleId ?? choice?.vehicleId, { car: choice?.car ?? track.car }), track.id);
      ...
      const humans = playerVehicles?.length
        ? [vehicle, ...playerVehicles.slice(1, isMobileDevice() ? 1 : 2).map(id => forTrack(resolveVehicle(id, track), track.id))] : [vehicle];
      const pace = AI_PACE[raceKey(trackId, direction)];
      const roster = raceRoster(humans, ai, isMobileDevice() ? MOBILE_AI_RIVALS : Infinity, vehicle => pace?.[vehicle.id], track.id);
      ...
      // Keep first-use downloads and GPU preparation behind the loading screen. Tiles use the same
      // window as driving; landmarks place the ones in range and decode the rest (preloadAll).
      ...
      await Promise.all([extra(world.sceneryReady), extra(billboardsReady),
        ...
        extra(landmarks.prepare(positions).then(() => landmarks.preloadAll()))]);
      request.signal.throwIfAborted();
      ...
      // A billboard face first seen in a tile that streams in mid-race compiled its program there
      // (Istanbul: one at 3.8 s). Every face shares it; a hidden probe per slot builds it now.
      const faces = new THREE.Group();
      const probe = new THREE.PlaneGeometry(.01, .01);
      for (const slot of BILLBOARD_SLOTS) faces.add(new THREE.Mesh(probe, world.materials.get(billboardFaceMaterial(slot))));
      world.scene.add(faces);
      const standIns = landmarks.warmGroup();
      world.scene.add(standIns);
      await compileEverything(world.renderer, world.scene, world.camera, world.sceneTarget());
      world.scene.remove(faces, standIns);
      probe.dispose();
      request.signal.throwIfAborted();
      progress(0.96);
      ...
      // Rapier builds its broad phase and the first contact pairs of every collider added so far on
      // the first step. Left to the countdown, that step took 50-75 ms on a desktop and was the hitch
      // the player felt as the race began; take it here, then put every car back on its grid slot.
  ...
  private createSlimes(world: World, physics: PhysicsWorld, racers: readonly Racer[]): SlimeLayer {
    ...
    slimes.farSleep = world.qualityLimits === QUALITY_LIMITS.low;
    return slimes;
  }
  ...
  private warmNow(s: Session): void {
    ...
    compileEverythingNow(s.world.renderer, s.world.scene, s.world.camera, s.world.sceneTarget());
  }
    ...
    if (rolling) {
      ...
      const { steps, alpha } = s.physics.step(dt, (h) => {
        ...
        for (const [index, racer] of s.racers.entries()) {
          const car = racer.car;
          // A car already held for the road is released only once a wider ring has arrived. Without
          // the margin a car parked on the loading edge froze and thawed every step and crept forward
          // a centimetre at a time.
          const margin = racer.isWaitingForRoad ? ROAD_RELEASE_MARGIN : 0;
          const ready = [car, ...(racer.trailer ? [racer.trailer.car] : [])].every(body => {
            const p = body.position, half = body.tuning.chassisHalf;
            return s.world.streamer.collisionReady(p.x, p.z, Math.hypot(half[0], half[2]) + body.speed * h + 2 + margin);
          });
          ...
          if (racingStep && racer.race.state === 'finished') {
            ...
            control = racer.bot.drive(h, {
              ...
              x: p.x, z: p.z, speed: car.speed, forwardSpeed: car.forwardSpeed, gripping: car.gripping, weatherGrip: car.planningGrip,
  ...
  private setSessionQuality(quality: Quality): void {
    ...
    if (this.session.slimes) this.session.slimes.farSleep = quality === 'low';
