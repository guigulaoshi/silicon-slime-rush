...
// Its own key, not the original's: every HTML game on itch.io is served from the same origin, so a
// shared key would hand this game the original's save, full of Bay Area route ids it does not have.
export const SAVE_KEY = 'silicon-rush-world-tour.save.v1';
...
function safeStorage(): Storage | null {
  try {
    ...
    s.setItem('silicon-rush-world-tour.probe', '1');
    s.removeItem('silicon-rush-world-tour.probe');
    return s;
  } catch {
    return null;
  }
