...
/**
 * The lightest body a blast is tuned for. The force is a fixed impulse, so the launch speed is impulse /
 * mass: the original's lightest car (650 kg) already came down on its roof now and then, and the remix's
 * local cars went lighter still (a 600 kg city car, a 680 kg kei truck) and rolled on landing in the
 * robot drives. Below this mass a blast gives the same launch speed as at this mass; heavier cars are
 * unchanged.
 */
export const BURST_REFERENCE_MASS = 900;
/**
 * The fastest a blast launches any car, m/s: about five metres up. A large bomb (radius 3 m) gave a 900 kg
 * car 15 m/s -- seven metres and three seconds in the air -- and where it came down on a slick it slid
 * sideways and rolled, the same station every robot lap (World Tour, Lhasa). The original's cap was 24 m/s.
 */
export const BURST_MAX_RISE = 10;

export function burstImpulse(scale: readonly number[], centre: THREE.Vector3,
  vehicleCentreOfMass: THREE.Vector3, share = 1, mass = Infinity): THREE.Vector3 {
  const radius = Math.cbrt(scale[0]! * scale[1]! * scale[2]!);
  const vertical = Math.min(4200 * radius * share * Math.min(1, mass / BURST_REFERENCE_MASS), mass * BURST_MAX_RISE);
...
export interface SlimeStats {
  ...
  colossi: number;
  /** Giants this run's streamed tiles have carried so far, counted once each: a winding route may have none. */
  colossiStreamed: number;
...
/** Beyond this from every car a moving slime is stopped and put to sleep (sleepFarSlimes). */
const FAR_SLEEP_M = 250;
...
export class SlimeLayer {
  ...
  private readonly giantsStreamed = new Set<string>();
  ...
  private nextFarSleep = 0;
  /** Low quality only (a phone's tier): it is where the physics step is the bottleneck. */
  farSleep = false;
  ...
  addTile(file: string, spawns: readonly TileSlimeSpawn[]): void {
    ...
    for (const spawn of spawns) if (spawn.kind === 'colossus') this.giantsStreamed.add(spawn.position.map(v => v.toFixed(1)).join(','));
    this.syncPopulation();
  }
  ...
  private detonateBurst(spawn: SlimeSpawn, targets: readonly { body: Car; share: number }[],
    ...
    let impulses = targets.map(({ body, share }) => {
      ...
      return { body, impulse: burstImpulse(spawn.scale, centre,
        new THREE.Vector3(massCentre.x, massCentre.y, massCentre.z), share, body.body.mass()) };
    });
  ...
  update(dt: number): void {
    ...
    if (this.farSleep && this.clock >= this.nextFarSleep) { this.nextFarSleep = this.clock + .5; this.sleepFarSlimes(); }
    for (const live of [...this.lives]) {
  ...
  get stats(): SlimeStats {
    ...
    return {
      ...
      colossiStreamed: this.giantsStreamed.size,
  ...
  /**
   * A purple slime has no friction, so one a rival knocked on keeps sliding for ten seconds or more and
   * never falls asleep: on a phone 63 of 97 were awake, 800 m from any car, and their contacts with the
   * road were most of the physics step. Far from every car one is stopped and put to sleep; a car that
   * reaches it wakes it by touching it.
   */
  private sleepFarSlimes(): void {
    const cars = [...this.drivers.keys()].map(car => car.position);
    for (const live of this.lives) {
      const body = live.motion?.body;
      if (!body || body.isSleeping()) continue;
      const p = body.translation();
      if (cars.some(car => (car.x - p.x) ** 2 + (car.z - p.z) ** 2 < FAR_SLEEP_M * FAR_SLEEP_M)) continue;
  ...
  private spawnFalling(car: Car): void {
    ...
    for (let attempt = 0; attempt < 8; attempt++) {
      ...
      // Near the road's own height, so a deck overhead is not taken for the landing (and any altitude works).
      const surface = this.physics.surfaceAt(x, z, p[1] + 8, p[1] - 30);
      if (!surface) continue;
