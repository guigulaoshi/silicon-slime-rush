...
export interface FaceCopy {
  ...
  bg?: string;
  fg?: string;
  /** Where a social face leads.: each language's copy names that language's platform. */
  platform?: string;
  target?: string;
}
 ...
 *
 * Every face has a zh and an en copy and the boards follow the interface language, redrawn the moment
 * it changes. Social faces carry different platforms per language: Chinese players see
 * Bilibili, Xiaohongshu, Douyin and WeChat Channels, English players X, YouTube and TikTok.
 * Available space is bilingual on one face anyway.
 */
