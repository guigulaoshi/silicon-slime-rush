...
/** Bloom and colour grade (PostGrade): desktop high only. Auto quality dropping to medium turns it off. */
export const POST_GRADE: Record<Quality, boolean> = { high: true, medium: false, low: false };
/** Multisampling is fixed when a WebGL context is made, so changing it rebuilds the renderer. */
/** Valley mist sheets (`track.mist`): each is a full-screen transparent layer when you look down into it. */
export const MIST_SHEETS: Record<Quality, number> = { high: 3, medium: 2, low: 1 };
