...
export class PhysicsWorld implements ColliderSink {
  ...
  /** Solids that stop cars but are never drawn: the camera looks through them. */
  private readonly invisibleColliders = new Set<number>();

  unregisterCollider(collider: RAPIER.Collider): void {
    this.colliderRoles.delete(collider.handle);
    this.invisibleColliders.delete(collider.handle);
  ...
  cameraPosition(start: RAPIER.Vector, desired: RAPIER.Vector): RAPIER.Vector {
    ...
    for (const [handle, role] of this.colliderRoles) {
      ...
      if (this.invisibleColliders.has(handle)) continue;
  ...
  // The default reach covers any road on Earth. It was 250 m down to -250 m, which the original's
  // sea-level Bay Area never left; a remix's Lhasa (3,650 m), Machu Picchu, Zhangjiajie, Fuji and Rio
  // all sit above it, so a query without bounds found no ground there at all.
  surfaceAt(x: number, z: number, top = 9000, bottom = -500): WorldSurfaceHit | null {
  ...
  add(key: string, colliders: TileColliders): void {
    ...
    for (const t of colliders.trimeshes) {
      ...
      if (t.invisible) this.invisibleColliders.add(collider.handle);
      bodies.push(body);
    }
  ...
  remove(key: string): void {
    ...
    for (const b of bodies) {
      for (let i = 0; i < b.numColliders(); i++) {
        this.colliderRoles.delete(b.collider(i).handle);
        this.invisibleColliders.delete(b.collider(i).handle);
      }
