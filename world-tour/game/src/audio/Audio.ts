...
import { musicSamples, RACE_MUSIC_RATE, RACE_PIECES, ROTATION } from './music';
...
export class GameAudio {
  ...
  private piece = 0;
  private racePiece = -1;
  /** The regional loop of the route being raced, or null to take the next of ROTATION. */
  private trackPiece: number | null = null;
  ...
  setVehicle(id: string): void {
    // A local car borrows a garage car's voice and may override a few of its numbers.
    const engine = vehicleFor(id)?.engine;
    const borrowed = engine ? { ...ENGINE_VOICES[engine.base]!, ...Object.fromEntries(
      Object.entries(engine).filter(([key]) => key !== 'base')) } as typeof ENGINE_VOICES[string] : undefined;
    this.voice = ENGINE_VOICES[id] ?? borrowed ?? ENGINE_VOICES['micro-hatch']!;
   ...
   */
  /** The route about to be raced: a route with a regional loop plays it; one without takes the next in ROTATION. */
  setTrack(trackId: string | null): void {
    const music = CATALOGUE.find(entry => entry.id === trackId)?.music;
    const index = RACE_PIECES.findIndex(piece => piece.id === music);
    this.trackPiece = index >= 0 ? index : null;
  }
  ...
  setScene(scene: string): void {
    ...
    if (scene === 'countdown' && !this.inRace) {
      if (this.trackPiece !== null) this.racePiece = this.trackPiece;
      else {
        const at = ROTATION.indexOf(RACE_PIECES[this.racePiece]?.id as typeof ROTATION[number]);
        this.racePiece = RACE_PIECES.findIndex(piece => piece.id === ROTATION[(at + 1) % ROTATION.length]);
      }
  ...
  private renderIdlePieces = (): void => {
    // Only what the next race can need: the rotation, and this route's own loop. Rendering all twelve
    // regional loops a player may never hear would cost a phone a few seconds of work for nothing.
    const wanted = (index: number): boolean => ROTATION.includes(RACE_PIECES[index - 1]?.id as typeof ROTATION[number])
      || index - 1 === this.trackPiece;
    const next = this.graph?.loops.findIndex((loop, index) => index > 0 && !loop && wanted(index)) ?? -1;
    if (next < 1) return;
