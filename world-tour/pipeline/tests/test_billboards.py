...
def test_no_board_stands_closer_to_an_open_start_than_its_sight_distance():
    """The sight check puts the driver VIEW_DISTANCE up the road. Nearer an open route's start there is no
    road to stand on: the eye was clamped onto the board, the check passed, and a shortened route put a
    board 18 m from its start that nobody ever drives towards."""
    from sr.billboards import VIEW_DISTANCE
    res = route_with("motorway")
    assert not res.closed
    boards = placements(res, np.full(len(res.P), 10.0), flat(res))
    assert boards
    for board in boards:
        s = float(res.S[int(np.argmin(np.hypot(res.P[:, 0] - board["pos"][0], res.P[:, 2] - board["pos"][2])))])
        assert s >= VIEW_DISTANCE[board["style"]] - 1.0, (board["style"], s)
