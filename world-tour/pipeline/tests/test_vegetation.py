...
from sr.vegetation import (DENSITY, PAVED, ZONED_DENSITY, FOLIAGE_BY_KIND, MAX_TREES,
                           PALM_VARIANT_COUNT, TREE_KINDS, TRUNK, add_to_tiles,
                           kind_for_tags, scatter, tree_mesh)

# sydney is the showcase / bridge track that replaced goldengate.
HAS_CACHE = os.path.exists(os.path.join(CACHE_DIR, "sydney", "landuse.json.gz"))
...
def test_the_same_route_grows_the_same_forest():
    ...
    a = scatter(poly, 100.0, np.random.default_rng(zlib.crc32(b"sydney")))
    b = scatter(poly, 100.0, np.random.default_rng(zlib.crc32(b"sydney")))
    c = scatter(poly, 100.0, np.random.default_rng(zlib.crc32(b"zhangjiajie")))
...
@pytest.mark.skipif(not HAS_CACHE, reason="needs the sydney cache")
def test_sydney_trees_keep_off_the_road():
    """sydney is the new showcase/bridge track (goldengate is gone with the rest of the Bay Area)."""
    import numpy as np
    ...
    res.P[:, 1] = road_y
    trees = positions(res, road_y, dem, "sydney", corridor_polygon(res))
...
def test_sydney_keeps_mapped_palms():
    """fishermans-wharf is gone; sydney's harbour-front is the new track with mapped palms.

    Re-measured directly against this route's own corridor: running `positions()`
    on sydney's real cached OSM trees keeps 22 palm-tagged trees in the corridor -- no Bay Area
    waterfront ever had this many mapped, so 40 does not carry over. The threshold below sits with
    a safety margin under that measured count rather than reusing the old number.
    """
    from sr.dem import DemSampler
    from sr.route import build_route
    from sr.routes import load_route
    ...
    res.P[:, 1] = road_y
    trees = positions(res, road_y, dem, "sydney", corridor_polygon(res))
    assert sum(t[5] == "palm" for t in trees) >= 15
...
def test_tree_fill_plants_the_species_mix_it_declares_and_refuses_one_it_does_not_know():
    """A savanna is acacias *and* bushes. One species repeated reads as an orchard, so the field is
    a mix rather than a species, and a typo in it has to be loud -- a silently ignored kind plants
    the wrong tree everywhere and nothing goes red."""
    from collections import Counter

    from sr import vegetation
    names, weights = vegetation.tree_fill_mix({"mix": {"acacia": 0.62, "scrub": 0.38}})
    counts = Counter(np.random.default_rng(7).choice(names, size=4000, p=weights))
    assert set(counts) == {"acacia", "scrub"}, counts
    assert abs(counts["acacia"] / 4000 - 0.62) < 0.03, counts
    for bad in ({"mix": {"baobab": 1.0}}, {"mix": {"acacia": 0.0}}):
        with pytest.raises(ValueError):
            vegetation.tree_fill_mix(bad)


def test_every_shipped_tree_fill_names_known_kinds_and_amboseli_grows_acacias():
    from sr import vegetation
    from sr.routes import all_route_ids, load_route
    for route_id in all_route_ids():
        fill = load_route(route_id).get("treeFill")
        if fill:
            vegetation.tree_fill_mix(fill)
    assert "acacia" in load_route("amboseli")["treeFill"]["mix"]


def test_the_acacia_silhouette_is_wider_than_the_tree_is_tall():
    """That proportion is the whole point of the species: a flat table on a bare trunk, which is
    what reads as savanna from a car. A round crown at acacia height reads as any tree anywhere."""
    from sr import vegetation
    parts = vegetation.tree_mesh("acacia")
    foliage = parts[vegetation.FOLIAGE_BY_KIND["acacia"]]
    xz = foliage.positions[:, [0, 2]]
    width = float(np.ptp(xz, axis=0).max())
    top = float(max(p[1] for part in parts.values() for p in part.positions))
    lowest_leaf = float(foliage.positions[:, 1].min())
    assert width > top, f"crown {width:.1f} m wide but tree {top:.1f} m tall"
    assert lowest_leaf > 3.0, f"leaves start at {lowest_leaf:.1f} m: an acacia's trunk is bare"


def test_every_crown_material_gets_a_photographed_leaf_texture():
    """Crowns are drawn with a composed leaf mass picked by material name; a crown material missing
    from that table ships as a smooth untextured blob among leafy trees."""
    from sr import photo_textures
    from sr.vegetation import FOLIAGE_BY_KIND
    crowns = {m for kind, m in FOLIAGE_BY_KIND.items() if kind != "palm"}
    assert crowns <= set(photo_textures.FOLIAGE), crowns - set(photo_textures.FOLIAGE)
