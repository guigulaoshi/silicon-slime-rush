...
ZH_PLATFORMS = {"bilibili", "xiaohongshu", "douyin", "wechat"}
EN_PLATFORMS = {"x", "youtube", "tiktok"}


def copies():
    """Every distinct board picture: each platform once, with the language its file is set in."""
    seen = {}
    for channel in load_channels():
        for language in ("zh", "en"):
            content = channel[language]
            seen[content["platform"]] = content
    return [(content, image_language(content["image"].removeprefix("billboards/"))) for content in seen.values()]


def test_each_language_shows_its_own_platforms():
    """
    reversing the one-platform-per-slot-whatever-the-language."""
    channels = load_channels()
    assert len(channels) == 7
    assert {c["zh"]["platform"] for c in channels} == ZH_PLATFORMS
    assert {c["en"]["platform"] for c in channels} == EN_PLATFORMS
    assert ZH_PLATFORMS | EN_PLATFORMS == ALL_PLATFORMS
    for content, language in copies():
...
def test_bilibili_and_xiaohongshu_carry_their_official_marks():
    ...
    faces = {content["platform"]: (content, language) for content, language in copies()}
