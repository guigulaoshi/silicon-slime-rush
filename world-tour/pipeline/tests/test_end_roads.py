...
@pytest.mark.parametrize("route_id", ["shanghai", "istanbul", "rome", "paris", "dubai"])
...
def test_a_timing_line_on_a_bridge_continues_on_the_deck():
    """Sydney starts on the Harbour Bridge. Easing the continuation to the ground under it sank the
    road 2.5 m below the deck within 15 m, and a car reversing off the start line fell through."""
    route = build_route("sydney")
    dem = DemSampler()
    y = road_profile(route, dem, route.route.get("bridgeDeckM"), route.route.get("bridgeMinLengthM", 200.))
    route.P[:, 1] = y
    assert route.bridge[0], "the start line is on the bridge"
    start = continuations(route, y, dem, "sydney")["start"]
    grade = float(route.T[0, 1] * -1)
    near = start.S <= 100
    deck = y[0] + grade * start.S[near]
    assert np.max(np.abs(start.P[near, 1] - deck)) < .5, np.max(np.abs(start.P[near, 1] - deck))
