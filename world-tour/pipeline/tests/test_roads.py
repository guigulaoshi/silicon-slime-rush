...
# sydney is the showcase / bridge track that replaced goldengate.
...
def test_sydney_roads_and_buildings():
    """sydney is the harbour-bridge showcase track that replaced goldengate. Thresholds below are
    the same generic bounds as before (bridge/road/building geometry exists, buildings vary, boxes
    stay in a sane range) -- all comfortably satisfied by sydney's real data (measured:
    road=56126, bridge=28720, sides=23716, barriers=132, facade tri=275338, boxes=780)."""
    ...
    route = load_route("sydney"); res = build_route("sydney"); dem = DemSampler()
    ...
    sides, _paint = side_roads(res, y, dem, "sydney")
    assert sides.triangle_count > 5000

    barriers = barrier_lines(res, y, dem, "sydney")
    ...
    by_facade, boxes = build_buildings(res, y, dem, "sydney", corridor_polygon(res))
...
def test_a_building_below_the_deck_is_not_clipped_by_the_bridge_above_it():
    """An elevated deck may cross a real building far below it -- ``road_footprint``'s own
    docstring records why: Fort Point sat directly under the Golden Gate, and clipping its
    footprint against the bridge's plan shadow deleted 40% of it even though their heights never
    met. Golden Gate is gone with the rest of the Bay Area tracks, and no new track's cached OSM
    data reproduces a real building that sits whole below an elevated deck: Sydney Harbour
    Bridge's own pylons (way:156584774) rise to meet the deck instead of sitting under it, and the
    plan-view overlaps found on fuji/paris are ordinary streetside buildings, not buildings under a
    bridge span (checked directly: none of them sit on a `res.bridge` segment). So this rebuilds
    the same shape synthetically -- a low building whose plan footprint sits under a bridge span --
    which exercises the exact same ``road_footprint(include_bridges=...)`` toggle the production
    code calls."""
    from shapely.geometry import Polygon
    ...
    res = straight_route(length=600.0, bridge_span=(200.0, 400.0))
    # half_width defaults to 4.0 m; a building well inside that band, centred on the bridge span
    building = Polygon([(250.0, -3.0), (350.0, -3.0), (350.0, 3.0), (250.0, 3.0)])
    full_road = road_footprint(res)
    building_clear = road_footprint(res, include_bridges=False)
    assert building.intersection(full_road).area > building.area * .9, \
        "fixture must still reproduce the plan-view overlap that deleted the fort"
    assert building.intersection(building_clear).area < building.area * .01, \
...
def test_no_building_collider_reaches_the_road():
    ...
    # sydney is the harbour-bridge showcase track that replaced goldengate.
    route = load_route("sydney")
    res = build_route("sydney")
    dem = DemSampler()
    y = road_profile(res, dem, route.get("bridgeDeckM"), route.get("bridgeMinLengthM", 200.0))
    res.P[:, 1] = y
    _, boxes = build_buildings(res, y, dem, "sydney", corridor_polygon(res))
    ...
    for b in boxes:
        ...
        # A building under an elevated deck shares its plan position but not its height
        # (Fort Point sat 67 m below the Golden Gate's road; sydney's own bridge deck has the same
        # shape of case). The car can only hit a box whose top reaches the deck, so under a bridge
        # the height decides.
        top = b[1] + b[4]
...
def test_a_side_road_across_a_cross_slope_is_not_buried_in_the_hill():
    ...
    hill: that buried thirty percent of every side road on the old Twin Peaks hill route, by as
    much as 2.8 m (zhangjiajie is the winding mountain road that plays this role now)."""
    import numpy as np
...
def test_no_building_is_drawn_standing_in_the_road():
    ...
    y = road_profile(res, dem, route.get("bridgeDeckM"), route.get("bridgeMinLengthM", 200.0))
    res.P[:, 1] = y
    by_facade, _ = build_buildings(res, y, dem, "sydney", corridor_polygon(res))
    ...
    # A building under an elevated deck (Fort Point below the old Golden Gate; the same
    # shape of case applies to sydney's own harbour bridge deck) is not standing in the road; only
    # what rises to within 3 m of the deck counts there.
...
def test_a_courtyard_is_a_hole_and_not_a_filled_in_block():
    ...
    whole block -- and a slab that size beside the road reads as a wall. Six of them in one old
    Bay Area city corridor alone.
    """
    import numpy as np
    from shapely.geometry import Polygon
...
def test_a_clipped_roof_stays_inside_its_own_outline():
    ...
    which the build's clearance check caught on the old Lombard switchbacks after everything else
    had been fixed (rio and machu-picchu carry that hairpin shape now).
    """
    import numpy as np
    from shapely.geometry import Polygon
...
# The old list hand-picked eight Bay Area routes for shape diversity (loop, hairpins, waterfront,
# highway...). bayshore-101's 64.8 km highway has no equivalent any more, and hand-picking a new
# representative subset would be a guess; P2P_ROUTE_IDS (already computed above from the real
# route files) covers every new p2p track instead, which is strictly more coverage, not less.
@pytest.mark.parametrize("route_id", P2P_ROUTE_IDS)
...
def test_the_other_carriageway_of_a_divided_freeway_stays_level_and_unburied(monkeypatch):
    """A divided freeway's southbound lanes (old US 101) vanished under the terrain. The blend back to natural
...
def test_the_pavement_never_folds_over_the_inside_of_a_hairpin():
    """An offset curve further out than the turn's radius folds back over itself. Twelve of
    the first World Tour's fifteen routes had a bend where the pavement's far edge reached past the centre."""
    import numpy as np

    from sr.roads import FOLD_CLEAR, sidewalks
    from tests.test_terrain import FakeDem, straight_route

    res = straight_route(length=400.0)
    n = len(res.P)
    th = np.linspace(0, 2 * np.pi, n, endpoint=False)
    r = 7.0                                            # tighter than the 7.7 m the pavement reaches
    res.P = np.stack([r * np.cos(th), np.zeros(n), r * np.sin(th)], axis=1)
    res.S = np.linspace(0, 2 * np.pi * r, n, endpoint=False)
    res.T = np.stack([-np.sin(th), np.zeros(n), np.cos(th)], axis=1)
    res.R = np.stack([np.cos(th), np.zeros(n), np.sin(th)], axis=1)
    res.half_width = np.full(n, 5.0)
    res.closed = True
    walk = sidewalks(res, np.zeros(n), FakeDem(res.frame, lambda x, z: np.zeros(np.shape(x))))
    radial = np.hypot(*np.asarray(walk.positions)[:, [0, 2]].T)
    inside = radial[radial < r]
    assert inside.size, "the inner pavement should still be there"
    # Exactly there, not merely "at least": a far edge that folded through the centre lands 0.7 m
    # out on the other side, which a lower bound alone would pass.
    assert inside.min() == pytest.approx(FOLD_CLEAR, abs=.02)
    outside = radial[radial > r]
    assert outside.max() == pytest.approx(r + 5.0 + 0.55 + 2.15, abs=1e-6)   # the outside keeps its width
