...
def test_every_shipping_route_has_a_continuous_dense_second_population():
    ...
    for route_id in route_ids:
        ...
        # Rule in sr/slimes.py: a route that cannot hold its giants apart gets fewer of
        # them, never a pair -- down to none on a switchback mountain road. Demanding two here
        # contradicted that rule and left the suite permanently red on five of the fifteen routes.
        # What must hold is below: the giants there are spread (assert_giants_spread) and the dense
        # population carries exactly as many.
        dense_giants = [item for item in extras if item.kind == "colossus"]
        assert len(dense_giants) == len(giants), route_id
        # Same mix, except the documented stand-in: a boost or a bomb with no safe straight left, in either
        # population, becomes a harmless popper at its station (sr/slimes.py), one for one.
        mix_n = {kind: sum(item.kind == kind for item in normal) for kind in SCALES}
        mix_x = {kind: sum(item.kind == kind for item in extras) for kind in SCALES}
        stood_in = sum(mix_n[k] - mix_x[k] for k in ("boost", "burst", "slick"))
        assert mix_x["popper"] - mix_n["popper"] == stood_in, route_id
        assert all(mix_x[k] == mix_n[k] for k in SCALES if k not in ("boost", "burst", "slick", "popper")), route_id
        # "Many" shows both populations together: no slick of either one within a bomb's throw of a
        # bomb of either one (a light car blown up came down on a dense slick and rolled).
        bombs = [item.s for item in items if item.kind == "burst"]
        # and no two bombs of either population within a thrown car's landing distance
        ordered = sorted(bombs)
        gaps = [b - a for a, b in zip(ordered, ordered[1:])]
        assert not gaps or min(gaps) >= 36.0, (route_id, min(gaps))
        for slick in (item for item in items if item.kind == "slick"):
            near = min((_distance(slick.s, b, res.length, res.closed) for b in bombs), default=1e9)
            assert near >= BURST_CLEAR_M, (route_id, slick.density, slick.s, near)
        ...
        # A declared no-slime zone (a memorial, a place of worship) is not a gap in the infestation:
        # the stretch across it is subtracted before measuring.
        from sr.slimes import forbidden
        zone = forbidden(res, load_route(route_id))
        step = float(res.S[1] - res.S[0])
        def skipped(a, b):
            lo, hi = np.searchsorted(res.S, a), np.searchsorted(res.S, b)
            return float(zone[lo:hi].sum()) * step
        gaps = np.array([b - a - skipped(a, b) for a, b in zip(dense_s[:-1], dense_s[1:])])
        if res.closed:
...
# test_long_route_colossi_are_visibly_different_sizes is deleted, not retargeted: it existed only to
# prove giant colossi read as different sizes over the length of bayshore-101, the 64.8 km highway.
# That route is a named-deleted feature ("there is no long route any more") and load_route/build_route
# both raise FileNotFoundError for it now; no new track is long enough to need this specific check
# (the longest, new-york, is 3.27 km), and test_ordinary_sizes_span_one_to_eight_metres_with_one_height_ratio
# below already covers colossus size variation generically, without depending on any one route.
...
# test_shipped_normal_populations_are_twice_the_pre_247_counts is deleted, not retargeted: its
# baseline dict pins each named route's slime count as it was measured on that specific track before
# Doubled the population. All six named tracks (fishermans-wharf, goldengate, lombard,
# shoreline, twin-peaks, wolfe-pruneridge) are deleted, and there is no way to re-derive a "pre
# count" for any of the 15 new tracks -- they were all built after the doubling was already in the
# pipeline, so no historical baseline for them ever existed to pin against. The doubling formula
# itself (2 * (before + (before + 4) // 5)) is still covered generically by
# test_density_and_starting_mix_are_owned_by_the_pipeline and by
# test_every_shipping_route_has_a_continuous_dense_second_population's len(extras) == len(normal)
# check on every real route, so this specific regression pin is redundant as well as unrecoverable.
