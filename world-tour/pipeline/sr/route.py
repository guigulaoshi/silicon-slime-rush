...
def load_ways(route_id, frame, layer="roads", race_ways=()):
    """Drivable OSM ways. `race_ways` are OSM way ids the route may also race on whatever their class
    -- the one pedestrian street or tram road that is the only way to a landmark's front. Only the
    racing graph gets them; every other footway stays out, so the rest of the town is not paved."""
    extra = {int(i) for i in race_ways}
    ways = []
    for e in load_layer(route_id, layer)["elements"]:
        ...
        if e.get("id") in extra and hw not in DRIVABLE:
            t = {**t, "highway": "living_street", "area": "no"}
            hw = "living_street"
...
def _densify(w, step):
    """Insert vertices so no edge of `w` is longer than `step`. New vertices get ids no OSM node has
    (negative, derived from the way id), so they never join another way."""
    nodes, xy = [w.nodes[0]], [w.xy[0]]
    k = 0
    for i in range(len(w.nodes) - 1):
        a, b = np.asarray(w.xy[i], float), np.asarray(w.xy[i + 1], float)
        n = int(np.ceil(np.linalg.norm(b - a) / step))
        for j in range(1, n):
            k += 1
            nodes.append(-(int(w.osm_id) * 10000 + k))
            xy.append(a + (b - a) * j / n)
        nodes.append(w.nodes[i + 1]); xy.append(b)
    w.nodes, w.xy = nodes, np.array(xy)


def clear_race_ways(ways, race_ways, frame, clear_m):
    """Push raceWays nodes out of a landmark's way: `clear_m` is {landmark id: metres}.

    A promenade mapped along a building's wall is fine for walking and wrong for a twelve metre race
    surface: round the Sydney Opera House the footway runs 0.2 to 6 m from the podium the landmark
    model stands on, so the road's inner half went through the podium. Every node of a raceWay
    closer than `clear_m` to the footprint moves straight out to that distance; ordinary roads never
    move. Each moved node moves in every way through it, so the graph keeps one position per node.
    Distance is taken from the footprint's convex hull. The raceWays are first cut into pieces of at most CLEAR_STEP metres: a straight edge between two
    pushed nodes still cuts across a corner of the podium that sticks out between them."""
    if not clear_m:
        return ways
    from shapely.geometry import Point
    from sr.landmark_data import footprints

    race = {int(i) for i in race_ways}
    for w in ways:
        if w.osm_id in race:
            _densify(w, CLEAR_STEP)
    moved = {}
    for name, metres in clear_m.items():
        polys = footprints(frame, names=[name])
        if not polys:
            raise ValueError(f"raceWayClearM: no footprint for landmark {name}")
        # The convex hull, not the outline: a race line has no business following every notch
        # between two sails, and each one it did follow became a 15 m radius wiggle.
        hull = polys[0].convex_hull
        for w in ways:
            if w.osm_id not in race:
                continue
            for n, p in zip(w.nodes, w.xy):
                pt = Point(float(p[0]), float(p[1]))
                if polys[0].contains(pt):
                    raise ValueError(f"raceWayClearM: way {w.osm_id} node {n} is inside {name}")
                inside = hull.contains(pt)          # in a notch of the outline: push out through the hull
                if not inside and hull.exterior.distance(pt) >= metres:
                    continue
                q = np.asarray(hull.exterior.interpolate(hull.exterior.project(pt)).coords[0])
                out = (q - np.asarray(p, dtype=float)) if inside else (np.asarray(p, dtype=float) - q)
                moved[n] = q + out / np.linalg.norm(out) * float(metres)
    for w in ways:
...
def link_race_ways(G, ways, race_ids, link_m):
    """Join race ways that touch no road to the road network where they pass within `link_m` of it.

    A square in front of a landmark is often mapped as a pedestrian area whose outline shares no node
    with the street beside it, so to the router it is an island and every waypoint on it snapped back
    to the street. Each outline node of a declared race way that is off the main network gets an edge
    to the nearest main-network node within `link_m`; nothing else changes."""
    race_ids = set(int(i) for i in race_ids)
    if not race_ids or link_m <= 0:
        return 0
    giant = max(nx.connected_components(G.to_undirected() if G.is_directed() else G), key=len)
    main = [n for n in giant]
    tree = cKDTree(np.array([G.nodes[n]["xy"] for n in main]))
    added = 0
    for w in ways:
        if w.osm_id not in race_ids:
            continue
        for node, xy in zip(w.nodes, w.xy):
            if node in giant:
                continue
            # The nearest few, not only the nearest: beside a divided road the closer carriageway is
            # not always the one the line arrives on.
            for d, i in zip(*tree.query(xy, k=min(3, len(main)))):
                if d > link_m:
                    continue
                other = main[int(i)]
                if not G.has_edge(node, other):
                    G.add_node(node, xy=xy)
                    G.add_edge(node, other, w=float(d), length=float(d), way=w, i=0)
                    if G.is_directed():
                        G.add_edge(other, node, w=float(d), length=float(d), way=w, i=0)
                    added += 1
    return added
...
def round_corners(P, max_turn=MAX_TURN, passes=180, relax=0.35, closed=False):
    ...
    those corners while leaving the rest of the line exactly where the road is. A circuit's seam is
    a corner like any other."""
    P = np.array(P, dtype=np.float64, copy=True)
    n = len(P)
    for _ in range(passes):
        Q = np.vstack([P[-1:], P, P[:1]]) if closed else P
        d = np.diff(Q[:, [0, 2]], axis=0)
        ln = np.linalg.norm(d, axis=1, keepdims=True)
        ln[ln == 0] = 1.0
        u = d / ln
        ang = np.arccos(np.clip(np.einsum("ij,ij->i", u[:-1], u[1:]), -1.0, 1.0))
        bad = np.where(ang > max_turn)[0] + (0 if closed else 1)
        if not len(bad):
            break
        mid = (P[(bad - 1) % n] + P[(bad + 1) % n]) / 2.0
        P[bad] += (mid - P[bad]) * relax
    return P


def smooth_polyline(xz, sigma_m=3.0, closed=False):
    """Resample at 1 m, gaussian-smooth, round any remaining sharp corner, keeping endpoints.

    A circuit is smoothed round its seam: with "nearest" padding the start/finish of a loop kept
    its raw junction corner while everything else was eased, a kink right on the start line."""
    P = np.column_stack([xz[:, 0], np.zeros(len(xz)), xz[:, 1]])
    P1, _ = resample(P, 1.0, closed=closed)
    sm = gaussian_filter1d(P1, sigma=sigma_m, axis=0, mode="wrap" if closed else "nearest")
    if not closed:
        sm[0], sm[-1] = P1[0], P1[-1]
    sm = round_corners(sm, max_turn=MAX_TURN / 3, passes=400, closed=closed)   # tighter than the
    # target: the relaxation runs at 1 m spacing and the line is resampled to 2 m afterwards
    if not closed:
        sm[0], sm[-1] = P1[0], P1[-1]
    return sm


MIN_RADIUS = 10.0      # metres: set by the longest vehicle in the fleet, not by the road width
EASE_MAX_REACH = 40.0  # how far either side of a tight spot the replacement curve may reach
EASE_HEADROOM = 1.2
REVERSAL_PROBE_M = 30.0  # a tight spot this close to ground the route covers twice is a dead end
REVERSAL_GAP_M = 4.0     # route points this close, far apart along it, are one street driven both ways
KEYHOLE_CLEARANCE_M = 12.0
HAIRPIN_DEG = 150.0      # turns beyond this are opened into keyholes only on routes with "switchbacks"


def _hermite(p0, p1, t0, t1, n):
    """n points of a cubic Hermite curve from p0 to p1 with unit tangents t0, t1 (x/z plane)."""
    chord = float(np.linalg.norm(p1 - p0))
    m0, m1 = t0 * chord, t1 * chord
...
def _bounded_join(p0, t0, p1, t1, turn, k_max, step=1.0, pieces=16):
    """Points (x/z) of a curve from p0 (heading t0) to p1 (heading t1) that turns by `turn` radians
    in all, with curvature nowhere above k_max; None when no such curve exists.

    A hairpin whose legs run closer together than two minimum radii cannot be opened by pulling the
    apex in: the turn needs pi * r of road and the legs leave less. The only curve that fits swings
    out first and comes round in a bulb, the keyhole a bus turns in. Solved as piecewise-constant
    curvature over a free length, so the shape falls out of the constraints instead of a template."""
    ...
    def walk(v):
        k, L = v[:-1], v[-1]
        ds = L / (pieces * sub)
        th = th0 + np.concatenate([[0.0], np.cumsum(np.repeat(k, sub) * ds)])
        mid = (th[:-1] + th[1:]) / 2
        xy = p0 + np.concatenate([[[0.0, 0.0]], np.cumsum(np.c_[np.cos(mid), np.sin(mid)] * ds, axis=0)])
        return xy, th[-1]

    def residual(v):
        xy, th_end = walk(v)
        return np.concatenate([xy[-1] - p1, [(th_end - th0 - turn) * 20.0],
                               np.diff(v[:-1]) * v[-1] * 0.05, [v[-1] * 0.002]])

    L0 = max(chord, abs(turn) / k_max * 1.2)
    lo = np.r_[np.full(pieces, -k_max), max(chord, 1.0)]
    hi = np.r_[np.full(pieces, k_max), chord + 2 * np.pi / k_max + 60.0]
    best = None
    for spread in (1.0, 1.6, 2.4):
        v0 = np.r_[np.clip(np.full(pieces, turn / (L0 * spread)), -k_max, k_max), min(L0 * spread, hi[-1] - 1e-6)]
        fit = least_squares(residual, v0, bounds=(lo, hi))
        xy, th_end = walk(fit.x)
        if np.linalg.norm(xy[-1] - p1) < 0.05 and abs(th_end - th0 - turn) < 2e-3:
            if best is None or fit.x[-1] < best[-1]:
                best = fit.x
    if best is None:
        return None
    xy, _ = walk(best)
    n = max(int(round(best[-1] / step)), 2)
    s = np.linspace(0.0, 1.0, len(xy))
    u = np.linspace(0.0, 1.0, n + 1)
    return np.c_[np.interp(u, s, xy[:, 0]), np.interp(u, s, xy[:, 1])]


def tight_spots(P, S, closed, min_radius=MIN_RADIUS):
    """Arc positions (metres) where the line turns tighter than `min_radius`."""
    k = np.abs(curvature(P, S, closed))
    return [float(S[i]) for i in np.flatnonzero(k > 1.0 / min_radius)]


def ease_tight_spots(P, closed, step=1.0, min_radius=MIN_RADIUS, reach=EASE_MAX_REACH, switchbacks=False):
    """Cut out each stretch tighter than `min_radius` and join the ends with a cubic curve.

    Relaxing the offending points (round_corners) cannot open a U-turn: widening the smoothing drags
    distant road off its street while the hairpin gets sharper. Replacing the stretch keeps the
    tangent continuous at both joints and leaves everything outside `reach` exactly on the map."""
    P = np.array(P, dtype=np.float64, copy=True)
    given_up = np.zeros(len(P), dtype=bool)
    for _ in range(200):
        S = cumulative_lengths(P, closed)[:len(P)]
        k = np.abs(curvature(P, S, closed))
        bad = np.flatnonzero((k > 1.0 / min_radius) & ~given_up)
        if not len(bad):
            return P
        i = int(bad[np.argmax(k[bad])])
        n = len(P)
        fixed = False
        for margin in np.arange(4.0, reach + 1e-6, 2.0):
            w = int(round(margin / step))
            a, b = i - w, i + w
            if not closed and (a < 1 or b > n - 2):
                a, b = max(a, 1), min(b, n - 2)
            idx = np.arange(a, b + 1) % n
            p0, p1 = P[idx[0]][[0, 2]], P[idx[-1]][[0, 2]]
            T = tangents(P, closed)
            t0, t1 = T[idx[0]][[0, 2]], T[idx[-1]][[0, 2]]
            curve = _hermite(p0, p1, t0, t1, len(idx))
            trial = P.copy()
            trial[idx, 0], trial[idx, 2] = curve[:, 0], curve[:, 1]
            lo, hi = max(a - 3, 0), min(b + 3, n - 1)
            window = np.arange(lo, hi + 1) % n if closed else np.arange(lo, hi + 1)
            St = cumulative_lengths(trial, closed)[:n]
            kt = np.abs(curvature(trial, St, closed))
            if kt[window].max() <= 1.0 / min_radius:
                P, fixed = trial, True
                break
        if not fixed:
            joined = _splice_bounded(P, i, closed, step, min_radius, reach, switchbacks)
            if joined is not None:
                P = joined
                given_up = np.zeros(len(P), dtype=bool)
                continue
            # Leave this one for tight_spots() to name (the build refuses it) and ease the rest.
            given_up[max(i - 3, 0):i + 4] = True
    return P


def _splice_bounded(P, i, closed, step, min_radius, reach, switchbacks=False):
    """Replace the stretch round point i with a `_bounded_join` curve; None if none fits in reach.
    The replacement may be longer than what it replaces, so it is spliced, not written in place."""
    n = len(P)
    # A road that bends back on itself has its two legs apart; a line that goes down a street and
    # comes back up it has them on top of each other. The second is a waypoint on the wrong side of
    # a junction, and a keyhole there would draw a loop across the map instead of naming the mistake.
    probe = int(round(REVERSAL_PROBE_M / step))
    near = np.arange(i - probe, i + probe + 1) % n if closed else np.arange(max(i - probe, 0), min(i + probe + 1, n))
    tree = cKDTree(P[:, [0, 2]])
    for j in near:
        others = np.asarray(tree.query_ball_point(P[j][[0, 2]], REVERSAL_GAP_M))
        gap = np.abs(others - j)
        if closed:
            gap = np.minimum(gap, n - gap)
        if len(others) and gap.max() > 2 * probe:
            return None
    T = tangents(P, closed)
    S = cumulative_lengths(P, closed)[:n]
    heading = np.unwrap(np.arctan2(T[:, 2], T[:, 0]))
    for margin in np.arange(10.0, reach * 2 + 1e-6, 5.0):
        w = int(round(margin / step))
        a, b = i - w, i + w
        if a < 1 or b > n - 2:
            if not closed:
                continue
            # Rotate so the stretch sits clear of the seam; the start point stays where it is.
            shift = n // 2 - i
            rolled = _splice_bounded(np.roll(P, shift, axis=0), n // 2, closed, step, min_radius, reach, switchbacks)
            if rolled is None:
                return None
            start = int(np.argmin(np.linalg.norm(rolled - P[0], axis=1)))
            return np.roll(rolled, -start, axis=0)
        turn = float(heading[b] - heading[a])
        # Turning round is only a road's own shape on a declared mountain climb. Anywhere else a
        # U-turn is the line crossing a median at a junction, and the waypoint is what is wrong.
        if abs(turn) > np.radians(HAIRPIN_DEG) and not switchbacks:
            return None
        curve = _bounded_join(P[a][[0, 2]], T[a][[0, 2]], P[b][[0, 2]], T[b][[0, 2]], turn,
                              0.92 / min_radius, step)
        if curve is None:
            continue
        mid = np.c_[curve[1:-1, 0], np.interp(np.linspace(S[a], S[b], len(curve))[1:-1], S, P[:, 1]),
                    curve[1:-1, 1]]
        trial = np.vstack([P[:a + 1], mid, P[b:]])
        m = len(trial)
        St = cumulative_lengths(trial, closed)[:m]
        kt = np.abs(curvature(trial, St, closed))
        j0, j1 = max(a - 3, 0), min(a + len(mid) + 4, m - 1)
        if kt[j0:j1 + 1].max() > 1.0 / min_radius:
            continue
        # The bulb must not swing into another stretch of the route (the next switchback up).
        keep = np.r_[0:max(a - 20, 0), min(b + 20, n):n]
        if len(keep):
            others = cKDTree(P[keep][:, [0, 2]])
            before = others.query(P[a:b + 1][:, [0, 2]])[0].min()
            after = others.query(curve)[0].min()
            if after < min(KEYHOLE_CLEARANCE_M, before):
                continue
        return trial
    return None


MAX_WIDENING = 0.5     # metres of half width per metre of road


def limit_widening(hw, S, closed, rate=MAX_WIDENING):
    """Cap how fast the half width may change along the road, sweeping both ways.

    The corner clamp narrows the road in a bend and lets it spring back at once: 8 m to 14 m inside
    two metres after a hairpin. Neighbouring cross-sections then do not overlap, the tarmac has a
    hole in it and the guardrail wraps round the hole in the middle of the road."""
    hw = np.asarray(hw, dtype=np.float64).copy()
    n = len(hw)
    if n < 2:
        return hw
    ds = np.diff(S) if not closed else np.diff(np.append(S, S[-1] + (S[1] - S[0])))
    loops = 2 if closed else 1
    for _ in range(loops):
        for i in range(1, n + (1 if closed else 0)):
            j, k = i % n, i - 1
            hw[j] = min(hw[j], hw[k] + rate * ds[k])
        for i in range(n - 2, -1 - (1 if closed else 0), -1):
            j, k = i % n, (i + 1) % n
            hw[j] = min(hw[j], hw[k] + rate * ds[j % len(ds)])
    return hw
...
def resolve_waypoints(route, ways, frame):
    """Waypoints are either [lat, lon] or {"road": name | "ref": ref | "way": osm id, "pick": selector}. Selectors: westmost, eastmost,
    ...
    for spec in route["waypoints"]:
        ...
        if "way" in spec:  # one carriageway of a divided road, where the name picks both
            cand = [w for w in ways if w.osm_id == int(spec["way"])]
        elif "ref" in spec:
            cand = [w for w in ways if spec["ref"] in w.refs]
        else:
            cand = [w for w in ways if w.name == spec["road"]]
        if not cand:
            raise ValueError(f"waypoint road not in corridor: {spec.get('road', spec.get('ref', spec.get('way')))}")
...
def build_route(route_id, closed=None):
    ...
    ways = load_ways(route_id, frame, race_ways=route.get("raceWays", ()))
    missing = set(int(i) for i in route.get("raceWays", ())) - {w.osm_id for w in ways}
    if missing:
        raise ValueError(f"{route_id}: raceWays not in the cached roads: {sorted(missing)}")
    clear_race_ways(ways, route.get("raceWays", ()), frame, route.get("raceWayClearM", {}))
    G = build_graph(ways, respect_oneway=route.get("respectOneway", False))
    link_race_ways(G, ways, route.get("raceWays", ()), float(route.get("raceWayLinkM", 0)))
    ...
    # Eased with headroom: the line is eased at 1 m and resampled to 2 m, which sharpens a corner
    # by a few per cent, and the check below is against the plain limit.
    raw = ease_tight_spots(smooth_polyline(xz, closed=closed), closed, min_radius=MIN_RADIUS * EASE_HEADROOM,
                           switchbacks=bool(route.get("switchbacks", False)))
    P, S = resample(raw, STEP, closed=closed)
    tight = tight_spots(P, S, closed)
    if tight:
        raise ValueError(f"{route_id}: turns tighter than {MIN_RADIUS:g} m at s = "
                         + ", ".join(f"{t:.0f}" for t in tight[:8]) + " m; move a waypoint")
    tree = cKDTree(xz)
    ...
    hw = limit_widening(np.minimum(hw, np.maximum(0.7 / np.maximum(curve, 1e-6), 3.0)), S, closed)
