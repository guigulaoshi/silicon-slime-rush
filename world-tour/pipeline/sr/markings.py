...
# What runs down the middle of a two-way road, by route (`centreLine`). North America paints a
# double yellow; most of the world a white line; China a yellow one. Written wrong, it fails.
CENTRE_LINES = {
    "double_yellow": (YELLOW, True),
    "yellow": (YELLOW, False),
    "double_white": (WHITE, True),
    "white": (WHITE, False),
    "none": (None, False),      # an unpaved track: nothing painted at all
}
TRAFFIC = ("right", "left")


def paint_rules(route):
    """(centre material, double?, traffic side) for a route; the original's defaults when unsaid."""
    route = route or {}
    centre = route.get("centreLine", "double_yellow")
    traffic = route.get("traffic", "right")
    if centre not in CENTRE_LINES:
        raise ValueError(f"centreLine {centre!r}: expected one of {sorted(CENTRE_LINES)}")
    if traffic not in TRAFFIC:
        raise ValueError(f"traffic {traffic!r}: expected 'right' or 'left'")
    material, double = CENTRE_LINES[centre]
    return material, double, traffic
...
def offsets(res, i, lane_w):
    ...
    A divided carriageway is drawn as one road with the median on the side the oncoming traffic is:
    on its left where traffic keeps right, on its right where it keeps left. The median edge wears
    the centre line's colour, the kerb edge is white."""
    centre_material, double, traffic = paint_rules(getattr(res, "route", None))
    if centre_material is None:
        return {}
    median = -1 if traffic == "right" else 1
    ...
    if res.oneway[i]:
        ...
        out[("edge", median)] = (median * usable, centre_material, False)   # median side
        out[("edge", -median)] = (-median * usable, WHITE, False)
    else:
        if double:
            out[("centre", -1)] = (-(DOUBLE_GAP + LINE_W) / 2, centre_material, False)
            out[("centre", 1)] = ((DOUBLE_GAP + LINE_W) / 2, centre_material, False)
        else:
            out[("centre", 0)] = (0.0, centre_material, False)
...
def markings(res, road_y, lane_w, junctions=()):
    ...
    # An unpaved track has no stop lines, crossings or arrows either, and no lanes to put an arrow in.
    unpainted = paint_rules(getattr(res, "route", None))[0] is None
    junction_paint = None if unpainted else junction_markings(res, road_y, lane_w, junctions)
