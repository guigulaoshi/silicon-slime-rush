...
def smooth_profile(S, y, window_m=PROFILE_WINDOW, closed=False):
    """Gaussian smoothing with a window measured in meters, not samples. A circuit wraps."""
    step = float(np.median(np.diff(S))) if len(S) > 1 else 1.0
    sigma = max(window_m / 6.0 / max(step, 1e-6), 0.5)
    return gaussian_filter1d(np.asarray(y, dtype=np.float64), sigma=sigma,
                             mode="wrap" if closed else "nearest")


def limit_slope(S, y, max_slope=MAX_SLOPE, passes=3, closed=False, closing=None):
    """Clamp the profile so no segment exceeds max_slope, alternating forward and backward sweeps.

    On a circuit the last sample is followed by the first: sweeping it as an open line left a 0.8 m
    step on the start line, because nothing ever compared the two ends. `closing` is the length of
    the segment from the last sample back to the first."""
    y = np.asarray(y, dtype=np.float64).copy()
    n = len(y)
    if closed and n > 2:
        gap = float(closing if closing is not None else (S[1] - S[0]))
        ds = np.append(np.diff(S), gap)            # ds[i]: from i to i+1 (mod n)
        for _ in range(passes):
            for i in range(1, 2 * n):
                j, k = i % n, (i - 1) % n
                lim = max_slope * ds[k]
                y[j] = min(max(y[j], y[k] - lim), y[k] + lim)
...
def road_profile(res, dem, deck_m=None, bridge_min_length=200.0):
    ...
    c = bool(getattr(res, "closed", False))
    gap = float(np.linalg.norm(res.P[0, [0, 2]] - res.P[-1, [0, 2]])) if c else None
    y = limit_slope(res.S, smooth_profile(res.S, land, closed=c), closed=c, closing=gap)
    ...
    y = limit_slope(res.S, y, closed=c, closing=gap)
    y = limit_slope(res.S, smooth_profile(res.S, y, closed=c), closed=c, closing=gap)
    # A deck may never sit below the ground it crosses: the ground does not make way for a bridge,
    # so a deck under it is simply covered. Smoothing and slope limiting pull short spans back down
    # by a few centimetres, hence a margin, and the fix is applied last with a raise-only sweep.
    if res.bridge.any():
        land_under = res.bridge & (raw >= 0.0)
        floor = np.where(land_under, raw + BRIDGE_GROUND_MARGIN, -np.inf)
        y = _raise_only(res.S, np.maximum(y, floor), c, gap)
    return y


BRIDGE_GROUND_MARGIN = 0.3


def _raise_only(S, y, closed, closing, max_slope=MAX_SLOPE):
    """Restore the slope limit after raising some samples, without lowering any of them."""
    y = np.asarray(y, dtype=np.float64).copy()
    n = len(y)
    ds = np.append(np.diff(S), closing if closed and closing is not None else 0.0)
    for _ in range(2 if closed else 1):
        for i in range(1, n + (1 if closed else 0)):
            j, k = i % n, i - 1
            y[j] = max(y[j], y[k] - max_slope * ds[k])
        for i in range(n - 2 + (1 if closed else 0), -1, -1):
            j, k = i % n, (i + 1) % n
            y[j] = max(y[j], y[k] - max_slope * ds[j])
    return y
...
def _ground_profile(res, road_y, dem, xz, tree=None, water_level=WATER_LEVEL):
    ...
    # Inland lakes and rivers: the bed sits under the surface, and a roadbed beside the water is not
    # levelled out across it -- a wide avenue's parallel carriageways once pushed the flat roadbed
    # tens of metres into the river in front of a city gate.
    level = inland_level(res, xz)
    wet_inland = np.isfinite(level)
    if wet_inland.any():
        on_road = dist <= res.half_width[idx] + 0.5 if len(res.half_width) else np.zeros(len(xz), bool)
        natural = np.where(wet_inland, np.minimum(natural, level - INLAND_BED), natural)
        t = np.where(wet_inland & ~on_road & ~bridge, 1., t)
...
def inland_level(res, xz):
    """Water level at each point that lies inside an inland lake or river, NaN elsewhere."""
    level = np.full(len(xz), np.nan)
    for poly, lvl in getattr(res, "inland_water", ()) or ():
        inside = _contains(poly, xz)
        level = np.where(inside & ~np.isfinite(level), lvl, level)
    return level


def bank_points(res, pad):
    """A ring of samples just inside and just outside every inland shore within the corridor.

    Away from the road the ground is sampled every ten metres, and a forty-metre river between
    banks at road height is then bridged by triangles from one bank to the other: the ground sits
    over the water and the river is gone (one remix drew 29 % of the river in front of a city gate)."""
    waters = getattr(res, "inland_water", ()) or ()
    if not waters:
        return np.zeros((0, 2))
    band = LineString(res.P[:, [0, 2]]).buffer(pad)
    out = []
    for poly, _lvl in waters:
        for offset in (-BANK_OFFSET, BANK_OFFSET):
            ring = poly.buffer(offset).intersection(band)
            for part in getattr(ring, "geoms", [ring]):
                boundary = getattr(part, "exterior", None)
                if boundary is None or part.is_empty:
                    continue
                n = max(int(boundary.length / BANK_SPACING), 4)
                out.append(np.array([boundary.interpolate(i / n, normalized=True).coords[0] for i in range(n)]))
...
def build_terrain(res, dem, road_y, corridor, pad=300.0, water_level=WATER_LEVEL, cover=None, sea=None):
    ...
    banks = bank_points(res, pad)
    if len(banks):
        xz = np.vstack([xz, banks])
    ...
    known = dem.known(*res.frame.to_latlon(xz[:, 0], xz[:, 1])) if hasattr(dem, "known") \
        else np.ones(len(xz), dtype=bool)
    level = inland_level(res, xz)
    lake = np.isfinite(level) & (y < level)
    wet = ((natural < water_level) & (t >= 0.999) & known) | lake
    water_y = np.where(np.isfinite(level), level, water_level)
    ...
    Pw = np.stack([xz[:, 0], water_y, xz[:, 1]], axis=1)
