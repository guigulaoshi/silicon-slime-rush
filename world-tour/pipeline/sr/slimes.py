...
SLICK_GAP_M = 28.0
GIANT_CLEAR_M = 44.0
# A boost is a slime the AI drives *at*, so a bomb near it is not a hazard to avoid but one that will
# be hit. 18 m was 0.7 s at 90 km/h and the AI was blown over at the same metre every lap; 50 m is
# about two seconds -- time for one lane change.
BOOST_BURST_CLEAR_M = 50.0
# Two bombs of either density. A car a bomb throws is in the air one to two seconds at 60-80 km/h, so
# it comes down 20-40 m on; with the dense bombs held only 12-18 m off the others, four stood in 28 m
# on Lhasa and a jeep went up on the first, came down on the next and rolled.
BOMB_SPACING_M = 40.0


def zone_reach(zone, half_width):
    """How far from a no-slime zone's centre a road body still stands in it: the declared radius,
    plus half the road (bodies stand across it) plus the largest body's radius. One function, so the
    placement and its test cannot disagree by the few tens of centimetres that make a test red."""
    largest = max(float(SHAPES[k]["radius"][1]) for k in KINDS)
    return float(zone["radiusM"]) + float(half_width) + largest


def forbidden(res, route):
    """Route stations (s) inside any declared `noSlimes` zone: memorials, cemeteries, places of worship."""
    zones = route.get("noSlimes") or []
    if not zones:
        return np.zeros(len(res.S), dtype=bool)
    mask = np.zeros(len(res.S), dtype=bool)
    for zone in zones:
        x, z = res.frame.to_local(zone["lat"], zone["lon"])
        d = np.hypot(res.P[:, 0] - float(x), res.P[:, 2] - float(z))
        mask |= d <= np.array([zone_reach(zone, hw) for hw in res.half_width])
    return mask
...
def _place_without_overlap(res, kind, preferred_s, legal_sites, rng, size_rng, density, occupied,
    ...
    # The preferred station is tried only when it is itself legal: a gap-filling popper put at a gap's
    # midpoint landed on a bend and stayed there whenever nothing else happened to shove it off.
    legal = set(float(s) for s in legal_sites)
    sites = sorted(legal if legal and float(preferred_s) not in legal else legal | {float(preferred_s)},
    ...
    for site_s in sites:
        if any(float(_distance(site_s, other.s, res.length, res.closed))
               < (BOMB_SPACING_M if kind == other.kind == "burst"
                  else BURST_CLEAR_M if (density == "normal" and other.density == "normal"
                                    and (kind == "burst" or other.kind == "burst"))
                  # "many" shows both populations at once: a bomb and a slick keep a bomb's throw
                  # apart whichever population each belongs to
                  or {kind, other.kind} == {"burst", "slick"}
...
def _fill_largest_gaps(occupied, count, length, closed, blocked=()):
    """Put each dense extra at the midpoint of the largest remaining route gap.

    `blocked` stations (a no-slime zone) take part as occupied points but are never returned."""
    points = sorted([float(s) for s in occupied] + [float(s) for s in blocked])
    out = []
...
def placements(res, route, junctions=()):
    ...
    blocked = forbidden(res, route)
    allowed = (lambda sites: [st for st in sites
                              if not blocked[min(int(np.searchsorted(res.S, st)), len(res.S) - 1)]])
    straight = allowed(_straight_sites(res, curve))
    ...
    fallback = allowed(np.sort(rng.uniform(25.0, max(25.01, res.length - 25.0),
    ...
    fallback_safe = _clear_of(fallback, sharp_turns, res.length, res.closed,
                              turn_anchor_clearance)
    all_sites = allowed([float(s) for s in res.S])
    # Stations inside a no-slime zone count as occupied for the gap filler, so the largest gap is
    # never picked inside one and the bodies are moved elsewhere rather than piled at its edge.
    zone_fill = [float(st) for st in res.S[blocked][::3]]
    ...
    burst_s = _pick(straight_safe, counts["burst"], res.length, res.closed,
                    min_separation=BOMB_SPACING_M, strict_separation=True)
    ...
    fits = (lambda sites: [s for s in sites
                           if res.half_width[min(int(np.searchsorted(res.S, s)), len(res.S) - 1)] <= giant_max])
    # Straights first; a winding route whose few straights the bombs already hold falls back to road
    # clear of sharp bends. Driving through a giant is the game's signature: two savanna and mountain
    # routes had none at all because only the straight pool was ever tried.
    pools = [fits(straight_safe or straight or fallback), fits(fallback_safe)]
    colossus_pool = pools[0]
    ...
    for giants in range(counts["colossus"], 0, -1):
        ...
        for pool in pools:
            colossus_s = _pick(pool, giants, res.length, res.closed,
    ...
    boost_s = _pick(boost_candidates, boost_target, res.length, res.closed,
                    exclude=burst_s, clearance=BOOST_BURST_CLEAR_M,
    ...
    hazard_sites = allowed(hazard_sites)
    if not hazard_sites:
    ...
    if missing > 0:
        ...
        if len(filler) < missing:
            popper_s.extend(_fill_largest_gaps(
                accepted_s, missing - len(filler), res.length, res.closed, zone_fill))
    ...
    fixed_many_sites = []
    volatile_pool = _clear_of(straight_safe, normal_bursts, res.length, res.closed, 12.0)
    volatile_pool = _clear_of(volatile_pool, normal_boosts, res.length, res.closed, BOOST_BURST_CLEAR_M)
    ...
    boost_pool = _clear_of(straight_safe, [*normal_bursts, *many_sites["burst"]],
                           res.length, res.closed, BOOST_BURST_CLEAR_M)
    ...
    # The original kept only the normal population's slicks off its bombs. On "many" -- the default --
    # both populations drive together, and a light car thrown by a bomb came down on a dense slick and
    # rolled (Lhasa, every third lap). The dense bombs are re-picked clear of every slick station, and
    # the dense slicks clear of every bomb. The boosts and giants above keep the stations they were
    # given from the first pick: re-deriving them from the new bombs moved a dense giant onto a normal
    # slick, which then slid along the road in the normal setting too.
    first_bursts = set(many_sites["burst"])
    occupied = [s for s in occupied if s not in first_bursts]
    burst_pool = _clear_of(volatile_pool, normal_bursts, res.length, res.closed, BOMB_SPACING_M)
    burst_pool = _clear_of(burst_pool, normal_sites["slick"], res.length, res.closed, BURST_CLEAR_M)
    burst_pool = _clear_of(burst_pool, many_sites["boost"], res.length, res.closed, BOOST_BURST_CLEAR_M)
    burst_pool = _clear_of(burst_pool, many_sites["colossus"], res.length, res.closed, BURST_CLEAR_M + 12.0)
    many_sites["burst"] = _pick(burst_pool, actual["burst"], res.length, res.closed,
                                exclude=fixed_many_sites, clearance=.1, min_separation=BOMB_SPACING_M,
                                strict_separation=True)
    occupied.extend(many_sites["burst"])
    slick_pool = _clear_of(all_sites, [*normal_bursts, *many_sites["burst"]], res.length, res.closed,
                           BURST_CLEAR_M)
    many_sites["slick"] = _pick(slick_pool, actual["slick"], res.length, res.closed,
                                exclude=(), clearance=0.0, min_separation=10.0)
    occupied.extend(many_sites["slick"])
    slick_short = actual["slick"] - len(many_sites["slick"])
    # Bombs need a clear 60 m window each, kept apart from the normal population's. A winding road can
    # hold its normal share and nothing more; the dense setting then takes harmless bodies instead,
    # the same trade the normal population makes, rather than putting a bomb on a blind bend.
    burst_short = actual["burst"] - len(many_sites["burst"])
    for kind in KINDS[1:]:
        if kind not in ("burst", "slick") and len(many_sites[kind]) != actual[kind]:
            raise ValueError(
                f'{route["id"]}: dense {kind} population needs {actual[kind]}, got {len(many_sites[kind])}')
    many_sites["popper"] = _fill_largest_gaps(occupied, actual["popper"] + burst_short + slick_short, res.length, res.closed,
                                              zone_fill)

    normal = []
    extras = []
    many_pools = {"popper": all_sites, "slick": slick_pool,
                  "burst": burst_pool, "boost": boost_pool, "colossus": giant_pool}
    def place_many(kind):
        for site_s in many_sites[kind]:
            try:
                ...
                # The dense boost pool is the few long straights, and the normal population's bodies
                # may already fill them in world space (a reversed Rome did). A boost on a bend is
                # worse than no boost, so that station takes a harmless body, as a bomb does below.
                try:
                    if kind not in ("boost", "burst", "slick"):
                        raise
                    extras.append(_place_without_overlap(res, "popper", site_s, all_sites, many_rng,
                                                          size_rng, "many", [*normal, *extras]))
                except ValueError:
                    if res.length >= 600:
                        raise
    ...
    for kind in ("colossus", "burst", "boost", "slick", "popper"):
        for site_s in normal_sites[kind]:
            gap = 12.0 if kind == "popper" and res.length >= 600 else .5
            try:
                normal.append(_place_without_overlap(res, kind, site_s, normal_pools[kind], rng,
                    size_rng, "normal", [*normal, *extras], gap))
            except ValueError:
                # A mountain road is bend after bend: its few clear stretches fill up, and a harmless
                # popper may then sit on a bend rather than the build refusing the route.
                try:
                    # A bomb that finds no clear window becomes a harmless body at the same station.
                    stand_in = "popper" if kind in ("popper", "burst", "slick") else None
                    if stand_in is None:
                        raise
                    normal.append(_place_without_overlap(res, stand_in, site_s, all_sites, rng,
                        size_rng, "normal", [*normal, *extras], 12.0 if res.length >= 600 else .5))
                except ValueError:
                    if res.length >= 600:
                        raise
        if kind == "colossus":
            place_many(kind)
    for kind in ("burst", "boost", "slick", "popper"):
        place_many(kind)
    if res.length < 600:
