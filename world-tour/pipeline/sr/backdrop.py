...
def terrain(res, dem, radius, corridor_pad, water_level=0.0, cover=None, fallback="terrain"):
    ...
    known = dem.known(lat, lon) if hasattr(dem, "known") else np.ones(len(xz), dtype=bool)
    from sr.terrain import INLAND_BED, inland_level
    level = inland_level(res, xz)                   # the same lakes and rivers as the near ground
    lake = np.isfinite(level)
    y = np.where(lake, np.minimum(y, level - INLAND_BED), y)
    wet = ((y < water_level) & known) | lake        # a tile never downloaded reads 0 m: that is not sea
    ...
    Pw = np.stack([xz[:, 0], np.where(lake, level, water_level) - WATER_SINK, xz[:, 1]], axis=1)
...
def skyline(res, dem, route_id, corridor_pad, water_level=0.0, keep_clear=None, landmark_names=None):
    ...
    try:
        local = localise(local_types(load_route(route_id)), res.frame)
    except FileNotFoundError:          # a synthetic or test route with no file
        local = []
    try:
    ...
    for e in elements:
        ...
        if not e.get("tags", {}).get("building") or underground(e.get("tags", {})):
            continue
        geom = e.get("geometry")
        ...
        far = far_style(local, height / LEVEL_HEIGHT, poly.area, (cx, cz)) if local else None
        if far:
            material = far[0]       # the city's own walls on its skyline too (sr/local_style.py)
...
def arterial_roads(res, dem, route_id, corridor_pad, water_level=0.0):
    ...
    # Nothing is drawn past the horizon mesh, so a road draped beyond it would float over nothing.
    from sr.horizon import radius_for
    spec = getattr(res, "route", None) or {}
    reach = LineString(res.P[:, [0, 2]]).buffer(max(float(spec.get("backdropM", 3000)),
                                                    radius_for(spec, res.frame)), quad_segs=8)
    for element in elements:
        ...
        visible = LineString(np.c_[x, z]).simplify(12.0).intersection(reach).difference(corridor)
...
def build(res, route_id, dem, corridor_pad=280.0, water_level=0.0, keep_clear=None, cover=None):
    ...
    # Landmarks too: the low-rise filler stayed off roads only, and stood a 25 m house against a
    # bridge tower and more on the river flat under the span (Brooklyn Bridge model report).
    from sr.landmark_data import footprints as landmark_footprints
    marks = [poly.buffer(20.0) for poly in landmark_footprints(res.frame, names=route.get("landmarks"))]
    all_clear = unary_union([shape for shape in (keep_clear, road_clear, *marks) if shape is not None])
    ...
    # A route's `pillarField`: the sandstone pillars the elevation grid is too coarse to keep.
    from sr.pillars import build as build_pillars
    out.update(build_pillars(res, route, dem, water_level, keep_clear=all_clear))
