...
from sr.buildings import build_buildings, load_landmark_footprints, oriented_box, road_footprint
...
from sr.mesh import merge, yaw_for_z_axis
from sr.markings import markings
from sr.preview import render_preview
from sr.roads import (barrier_lines, barrier_mesh, guardrail_shield, guardrails, junction_layout, race_ribbons,
...
from sr.vegetation import add_to_tiles as add_trees, positions as tree_positions
...
def road_markings(res, road_y, route_id):
    ...
    return markings(res, road_y, LANE_W, layout.junctions), layout
...
def build(route_id, stage="greybox", out_root=None, preview_root=None, compress=True, log=print):
    ...
    # A cache that stops short of the corridor builds a stretch of bare default ground: no streets,
    # no buildings, no trees, no park -- and nothing else says so.
    from sr.fetch_osm import coverage
    short = {k: v for k, v in coverage(route_id, res).items()} if not route.get("syntheticCache") else {}
    if short:
        raise ValueError(f"{route_id}: the OSM cache stops short of the built corridor ("
                         + ", ".join(f"{k} by {v:.0f} m" for k, v in short.items())
                         + f"); run `python -m sr.cli fetch {route_id}`")
    # Every reader below -- road profile, terrain, buildings, props, backdrop -- sees the ground,
    # not the roofs: see sr/bare_earth.py.
    from sr.bare_earth import for_route as bare_earth
    dem = bare_earth(dem, res, route_id, float(route.get("terrainPadM", 300)), float(route.get("waterLevelM", 0.0)))
    ...
    res.inland_water = landcover.inland_water(route_id, res.frame, dem, res, road_y,
                                              float(route.get("waterLevelM", 0.0)))
    if res.inland_water:
        log(f"  inland water: {len(res.inland_water)} surfaces, "
            + ", ".join(f"{p.area / 1e4:.1f} ha at {lvl:.1f} m" for p, lvl in res.inland_water[:4]))
    ...
    shield = guardrail_shield(res, road_y) if rail is not None else None
    ts.add_mesh("guardrail_shield", shield)          # collision only: the runtime hides it
    log(f"  guardrail {rail.triangle_count if rail else 0} tris, shield {shield.triangle_count if shield else 0}")
    ...
    # Buildings are deliberately built first: their real exported boxes decide where a face can be
    # read from the road, ordinary bridges carry no boards.
    landmark_boxes = []
    ...
    # The model's own plan too: a landmark often brings more than its map outline -- the paved island
    # round the Arc de Triomphe, a palace's stairs and terraces, a pyramid's apron. Trees planted by the
    # outline alone grew up through the Arc's paving.
    landmark_boxes += landmark_model_boxes(route)
    boards = billboard_placements(
        res, road_y, dem, obstacles=[*visual_boxes, *landmark_boxes], terrain=terrain.terrain,
    )
    ...
    # prayer flags on the hills and at the shrines (routes/README.md `prayerFlagFields`)
    from sr.local_style import flag_fields
    from scipy.spatial import cKDTree as _Tree
    _tree = _Tree(res.P[:, [0, 2]])
    flags = flag_fields(route, res.frame, lambda pts: ground_height(res, road_y, dem, pts, _tree))
    if flags:
        from sr.mesh import merge as _merge
        by = {}
        for m in flags:
            by.setdefault(m.material, []).append(m)
        for material, parts in by.items():
            ts.add_mesh("scenery_flags" if material == "local_flags" else "scenery_poles", _merge(parts, material))
        log(f"  prayer flags {sum(m.triangle_count for m in flags)} tris")
    from sr.vegetation import check_density
    check_density(route_id, trees, res.length)
    ...
    # The tiled road must have no holes: sweep what the tiles carry, every metre, every metre across.
    from sr.roads import ROAD_LIFT
    from sr.surface_check import holes, road_triangles
    gaps = holes(res, road_y, road_triangles(ts), ROAD_LIFT)
    if gaps:
        worst = ", ".join(f"s={s:.0f} m lateral {l:+.0f} m" for s, l in gaps[:5])
        raise ValueError(f"{route_id}: {len(gaps)} points of the racing surface have no road under them -- {worst}")
    log("  road surface continuous (tile sweep)")
    ...
    for name in route.get("landmarks", []):
        ...
        y = landmark_base(res, road_y, dem, entry, float(x), float(z), lat, lon)
        # Collision is on unless an entry says why not: a route that drives up to a landmark drove
        # straight through a gateway's pier when only 12 of 46 landmarks had remembered to switch it on.
        models.append({"id": name, "file": entry["file"], "pos": [float(x), y, float(z)],
                       "yaw": float(entry.get("yaw", 0)), "loadRadius": entry["loadRadius"],
                       "collision": bool(entry.get("collision", True))})
    for model in models:
        path = os.path.normpath(os.path.join(ROOT, "game", "public", "tracks", route_id, model["file"]))
        if not os.path.exists(path):
            continue
        from sr.landmark_clearance import conflicts, load_placed
        hits = conflicts(load_placed(path, model["pos"], model["yaw"]), res.P, road_y, res.half_width)
        if hits:
            worst = ", ".join(f"{a:.1f} m2 at y={h:.1f} ({x:.0f}, {z:.0f})" for h, a, x, z in hits[:3])
            raise ValueError(f"{route_id}: landmark {model['id']} is solid on the road at car height -- {worst}")
    if models:
    ...
    far = build_backdrop(res, route_id, dem.over(DemSampler(BACKDROP_ZOOM, require_complete=True)), water_level=water_level,
    ...
    for name, mesh in (far or {}).items():
        if "building" not in name and "landmark" not in name and "pillars" not in name:
            continue
        ...
        # every visible side road so they cannot cut a street in half; so must generated pillars.
    ...
    if far:
        ...
        from sr.horizon import radius_for
        track["backdrop"] = {"file": "backdrop.glb", "radiusM": float(route.get("backdropM", 3000)),
                             "horizonRadiusM": radius_for(route, res.frame)}
    ...
    # the city's own ordinary buildings: their textures travel with the track (sr/local_style.py)
    from sr.local_style import write_textures
    local_dir = os.path.join(out_dir, "textures")
    if os.path.isdir(local_dir):
        import shutil
        shutil.rmtree(local_dir)
    local = write_textures(route, local_dir)
    if local:
        track["localTextures"] = True
        log(f"  local textures {len(local['textures'])}, {sum(e['bytes'] for e in local['textures']) / 1e3:.0f} kB")
...
def landmark_model_boxes(route):
    """[cx, 0, cz, hx, 1e4, hz, 0] per landmark model: its plan extent round its anchor, in track xz."""
    from sr.landmark_data import entries as landmark_entries, model_anchor
    ...
    for name in route.get("landmarks", []):
        ...
        if entry.get("kind") != "glb":
            continue
        path = os.path.normpath(os.path.join(ROOT, "game", "public", "tracks", route["id"], entry["file"]))
        if not os.path.exists(path):
            continue
        # The smallest rectangle round the model's plan, turned with it: a north-aligned box round a
        # long building set diagonally is half as big again, and cleared the tree rows along the road.
        from shapely.geometry import MultiPoint
        from sr.buildings import oriented_box
        scene = trimesh.load(path, force="scene")
        plan = np.concatenate([g.vertices for g in scene.dump()])[:, [0, 2]] if len(scene.geometry) else np.zeros((0, 2))
        x, z = frame.to_local(*model_anchor(entry))
        if len(plan) < 3:
            continue
        hull = MultiPoint(plan + [float(x), float(z)]).convex_hull
        if hull.geom_type != "Polygon":
            continue
        (cx, _cy, cz), (hx, _hy, hz), yaw = oriented_box(hull, 0.0, 1.0)
        boxes.append([cx, 0.0, cz, hx, 10_000.0, hz, yaw])
    return boxes


def landmark_base(res, road_y, dem, entry, x, z, lat, lon):
    """Where a landmark model's ground datum sits: the lowest finished ground round its footprint.

    The original read the raw elevation at the anchor point, which in a city is often a roof: one
    remix hung a hall 6-26 m in the air. A footprint-less entry (a parked aircraft) keeps a point."""
    ring = entry.get("footprint")
    if not ring:
        return float(ground_height(res, road_y, dem, np.array([[x, z]]))[0])
    from shapely.geometry import Polygon
    from sr.buildings import _building_base
    fx, fz = res.frame.to_local([p[0] for p in ring], [p[1] for p in ring])
    poly = Polygon(np.c_[fx, fz])
    if not poly.is_valid:
        poly = poly.buffer(0)
    return _building_base(res, road_y, dem, poly, None)
...
def _track_document(route, res, road_y):
    ...
    if route.get("sky"):
        doc["sky"] = route["sky"]       # the place's own daylight sky (see routes/README.md)
    from sr.local_style import tints as local_tints
    tint = {**((route.get("architecture") or {}).get("tint") or {}), **local_tints(route)}
    if tint:
        doc["tint"] = tint              # the city's own wall and roof colours (see routes/README.md)
    if route.get("mist"):
        doc["mist"] = route["mist"]     # valley cloud sheets (see routes/README.md)
    if route.get("lampStyle") is not None:
        doc["lampStyle"] = route["lampStyle"]
    return doc
