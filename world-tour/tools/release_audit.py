...
def addresses(body: str) -> set[str]:
    ...
    while hit := URL.search(body, at):
        ...
        # A closing bracket is part of the address when it closes one the address opened
        # (`.../wiki/Christ_the_Redeemer_(statue)`); otherwise it is the prose around it.
        while True:
            trimmed = text.rstrip(".,;'\"`")
...
def _expected_404(url: str) -> bool:
    ...
    return url.rstrip("/") in {
        ...
        "https://guigulaoshi.itch.io/silicon-slime-rush-world-tour/purchase",
    }
