...
def main() -> int:
    ...
    with open(os.path.join(GAME, "package.json"), encoding="utf-8") as f:
        version = json.load(f)["version"]        # the one owner of the version (game/build/credits.ts reads it too)
    ...
    # The store page is written at upload time (the remix makes the game, not the store page); once it
    # exists under docs/itch/, pass its text here with --extra so the audit reads it too.
    ...
    refused = []
    # named after the game's own page (PLANNED_GAME_URL), so a remix's zip is not called the original's
    slug = home.rstrip("/").rsplit("/", 1)[-1]
    name = "%s-%s%s.zip" % (slug, version, "-embed" if args.asset_origin else "")
    ...
    print(json.dumps({
        ...
        "then": [
            "upload %s to itch.io as the HTML build of %s (your account, your click)" % (name, home),
