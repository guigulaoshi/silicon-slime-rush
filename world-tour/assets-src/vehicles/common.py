...
def prepare_glazing(body, spec):
    ...
    # A catalogue entry may carry its own cabin (`cabin: {layout, footwell, rows}`); a new body no
    # longer needs its id added to three tables here before it can be built.
    cabin = spec.get("cabin", {})
    front,rear,floor,top,half = cabin.get("layout") or layout.get(spec["id"], (-.15,.28,.635,.88,.28))
    ...
    floor_front,floor_rear,floor_width = cabin.get("footwell") or footwell.get(spec["id"], (-.235,.355,.80))
    ...
    if spec["id"] == "travel-trailer":
        ...
        return
    rows = cabin.get("rows") or (8 if spec["id"] == "school-bus" else 3 if spec["id"] == "retro-van" else 2 if spec["id"] in ("micro-hatch","jeep") else 1)
