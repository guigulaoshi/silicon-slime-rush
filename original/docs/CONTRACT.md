...
| `trees` | 树实例。`trees_<种类>_trunk` 和 `_foliage*` 两个节点；种类是 `broadleaf`、`conifer`、`palm`、`orchard` 或 `scrub` | `none` | `halfExtents` |
...
`airfieldCircuit` 是唯一的测绘路面例外：它的每一段直接携带公开地图和机场图核过的 `widthM`，
所以跑道可以保留真实宽度，不被压进普通道路的二十八米上限。这个例外只由机场路线生成器读取；
图路由出来的普通街道仍只认上面两个常量。
