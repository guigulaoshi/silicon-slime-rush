...
def build_id(track, pipeline=None):
    ...
    for name in route.get("landmarks", []):
        ...
        h.update(json.dumps(landmark_entries.get(name), sort_keys=True,
                            separators=(",", ":")).encode())
    _hash_tree(h, os.path.join(PIPELINE, "cache", track))
    coast = os.path.join(PIPELINE, "cache", "bayarea", "coast.json.gz")
    h.update(b"horizon-coast\x00")
...
def ensure_track(track, pipeline=None, log=say):
...
def menu_map_id(module):
    h = hashlib.sha256()
    paths = [module.__file__, module.REGION_CACHE, module.ROADS_CACHE, module.PLACES_CACHE]
