...
def test_ring_roof_fascia_does_not_fill_the_occupied_roof():
    ...
    assert np.allclose(fascia.uvs, .02), "Apple Park's top band must stay free of windows"
...
def test_actual_building_producers_do_not_overlap_windowed_walls_and_roof_bands(monkeypatch, producer):
    ...
    if producer == "street":
        ...
        meshes, stats = vistas.city_massing(res, dem, "twin-peaks")
        ...
        meshes = backdrop.skyline(res, dem, "twin-peaks", corridor_pad=300)
...
def test_a_road_clipped_house_does_not_regrow_a_rectangular_roof_over_the_road(monkeypatch):
    ...
    by_material, _ = buildings.build_buildings(
        ...
        "twin-peaks", corridor, keep_clear=road)
...
def test_build_buildings_connects_hillside_base_to_mesh_and_collider(monkeypatch):
    from scipy.spatial import cKDTree
    import sr.buildings as buildings
    from tests.test_terrain import FakeDem, straight_route
    ...
    by_facade, boxes = buildings.build_buildings(
        res, road_y, dem, "goldengate", corridor, keep_clear=road_clear)

    edge = np.asarray(poly.exterior.coords)
    ground = buildings.ground_height(res, road_y, dem, edge, cKDTree(res.P[:, [0, 2]]))
    mesh = next(iter(by_facade.values()))
    assert mesh.positions[:, 1].min() <= ground.min() - buildings.SINK + 1e-6
    assert len(boxes) == 1
    _, cy, _, _, hy, _, _ = boxes[0]
    assert cy - hy == pytest.approx(mesh.positions[:, 1].min())
    assert cy + hy == pytest.approx(mesh.positions[:, 1].max())
...
def test_visual_boxes_include_buildings_too_far_away_to_need_collision(monkeypatch):
    ...
    by_facade, colliders, visual = buildings.build_buildings(
        res, np.zeros(len(res.P)), FakeDem(res.frame, lambda x, z: np.zeros(np.shape(x))),
        "goldengate", corridor, collider_radius=1.0, return_visual_boxes=True,
    )
    assert by_facade
    assert colliders == []
    assert len(visual) == 1
...
def test_every_cached_split_building_relation_emits_only_closed_rings():
    ...
    golden = next(element for element in load_layer("goldengate", "buildings")["elements"]
                  if element.get("id") == 19109535)
    assert _rings(golden) == ([], []), "50 Golden Gate structure parts are not 50 generic buildings"
    wharf = next(element for element in load_layer("fishermans-wharf", "buildings")["elements"]
                 if element.get("id") == 19651252)
    outer, inner = _rings(wharf)
    assert len(outer) == 1 and not inner, "the outline owns the building; its part must not duplicate it"
...
def test_v_is_height_above_the_building_and_not_above_sea_level():
    ...
    they drift against any building on a hill -- shoreline sits at 3.6-7.3 m, Twin Peaks far
    higher -- and the pavement band ends up underground."""
...
def test_nothing_gives_building_geometry_a_scale_of_its_own():
    ...
    until it covered whole buildings: Alcatraz's 16 m cellhouse spans only 2.7 uv at scale 6."""
    offenders = []
...
def test_the_five_facades_are_told_apart_by_something_a_street_actually_has():
    """A guard against the mapping collapsing to one kind: the tags a Bay Area corridor is full of
...
def test_the_untagged_majority_gets_a_facade_too():
    """`building=yes` is not an edge case, it is most of the map: 2813 of lombard's 2967 footprints,
    4457 of twin-peaks' 4540, 6635 of sand-hill's 7423. Mapping only the tags that say something
    left 80.7% of every wall in the game on the untextured fallback -- whole city blocks still the
    one beige with one window grid, which is the thing this was meant to end."""
    from collections import Counter
...
def test_the_golden_gate_tower_piers_are_not_windowed_buildings_in_the_strait():
    ...
    drawn = {tags["sr:facade-id"] for _, tags in footprints("goldengate", frame)}
    assert drawn, "the real Golden Gate cache has buildings"
    assert not {"way:1330586852", "way:1330832666"} & drawn
