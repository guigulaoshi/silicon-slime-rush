...
def test_a_deck_below_the_ground_still_cuts_it():
    """The other side of the same rule, and the one that bites: a span whose deck ends up *under* the
    ground beside it must still be cut out, or the ground closes over the tarmac the car drives on.

    bayshore-101 has one at s=1454 m: sixty metres, too short for `bridge_deck` to lift, so smoothing
    and the grade clamp leave the deck four metres under the natural ground while `_ground_profile`
    still holds the blend at 1 over it."""
    ...
    road_y = road_profile(res, dem, deck_m=67.0, bridge_min_length=200.0)
    middle = len(road_y) // 2
    # Smoothing and the grade clamp leave the deck about eleven metres under the knoll it crosses.
    assert road_y[middle] < 40.0 - 1.5, "the fixture must put the deck under the ground beside it"
    tr = build_terrain(res, dem, road_y, corridor_polygon(res, 150.0), pad=150.0)
    tris = tr.terrain.positions[tr.terrain.indices.reshape(-1, 3)]
    centroids = tris.mean(axis=1)
...
def test_goldengate_terrain():
    ...
    # sea level up to the Conzelman viewpoint; the summit at 248 m is no longer on the route
    assert 0 < y.min() and 120 < y.max() < 220, (y.min(), y.max())
    assert (np.abs(np.diff(y) / np.diff(res.S)) <= 0.15 + 1e-6).all()
    deck = y[res.bridge]
    assert abs(np.median(deck) - route["bridgeDeckM"]) < 6, np.median(deck)

    tr = build_terrain(res, dem, y, corridor_polygon(res))
    assert tr.water_area > 800_000, tr.water_area          # the strait crossing
    ...
    fins = near_bridge & (triangles[:, :, 1].min(axis=1) <= SHORE_CLAMP + 1e-5) \
        & (triangles[:, :, 1].max(axis=1) > 40.0)
    assert not fins.any(), f"{fins.sum()} Golden Gate terrain faces join bridge height to the sea"
