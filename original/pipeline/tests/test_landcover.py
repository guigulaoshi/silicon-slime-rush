...
def test_zoning_is_not_a_surface():
    ...
    draws it enormous: the salt works behind the Dumbarton is one industrial polygon of fifteen
    square kilometres. Treating it as tarmac painted that whole route -- ponds included -- grey."""
...
def test_a_patch_too_small_to_see_from_the_road_is_not_a_patch():
    """Measured, not guessed: the sand-hill corridor carries 745 `leisure=swimming_pool` polygons,
    almost all a few square metres in a back garden. Each would cost a triangle group."""
...
def test_a_route_can_ask_for_green_hillsides_without_losing_its_specific_ground(monkeypatch):
    """The Golden Gate's
    unmapped hillside and its mapped scrub turn green; a beach or a car park on it stays what it is."""
    ...
    assert landcover.route_ground("goldengate") == "terrain_grass"
    green = landcover.patches("goldengate", frame)
    assert list(landcover.classify(points, green)) == ["terrain_grass", "terrain_sand", "terrain_grass"]
    dry = landcover.patches("lombard", frame)
