...
def test_all_routes_load_and_have_waypoints():
    ...
    assert len(ids) >= 3 and "goldengate" in ids
    for rid in ids:
        r = load_route(rid)
...
def test_tile_xy_matches_known_tile():
    # Apple Park at z15 lands on the tile that contains it, and its bounds bracket the point
    x, y = tile_xy(37.3349, -122.0090, 15)
    s, w, n, e = tile_bounds_deg(x, y, 15)
    assert s <= 37.3349 <= n and w <= -122.0090 <= e
