        ...
        for path in glob.glob(os.path.join(ROOT, "game", "public", "tracks", track, "tiles", "*.glb")):
            for node in pygltflib.GLTF2().load(path).nodes:
                if node.name.startswith("terrain"):
                    kinds.add(node.name)
        assert len(kinds) > 1, f"{track}: the ground is still one flat colour ({kinds})"
...
def test_a_bridge_support_the_elevation_reads_as_ground_is_sea():
    ...
    terrain drew a strip of grass under the Golden Gate that the pier's own outline did not match."""
...
def test_only_mapped_bridge_supports_become_sea():
    from sr import landcover
    ...
    supports = landcover.bridge_supports("goldengate", frame)
    # The south tower's pier, under the span, a third of a kilometre off the San Francisco shore.
    assert [(round(p.centroid.x / 10), round(p.centroid.y / 10)) for p in supports] == [(4, 65)]
