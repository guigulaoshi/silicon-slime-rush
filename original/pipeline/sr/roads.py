...
def sidewalks(res, road_y, dem):
    ...
    for a, b in _runs(city):
        ...
        for side, flip in ((-1.0, False), (1.0, True)):
            inner = P[sl] + res.R[sl] * (side * (res.half_width[sl] + WALK_INNER))[:, None]
            outer = P[sl] + res.R[sl] * (side * (res.half_width[sl] + WALK_INNER + WALK_WIDTH))[:, None]
            mid = (inner + outer) / 2
            hw = np.full(b + 1 - a, WALK_WIDTH / 2)
