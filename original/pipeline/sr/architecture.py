...
def house_style(route_id, roof, height):
    """SF's narrow bay fronts versus the valley's low porches; roof choice stays in vistas."""
    area = profile(route_id).source_key
    if area in ("twin-peaks", "san-francisco-waterfront", "golden-gate") and height >= 6:
        return "bay-front"
    if roof == "hip":
        return "courtyard"
    if roof == "gable":
