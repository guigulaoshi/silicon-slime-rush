...
const run: Challenge = { trackId: 'goldengate', direction: 'reverse', time: 151.37, rating: 3, score: 15562,
...
describe('challenge codes', () => {
  it('round-trips every field, including a name with separators, spaces and CJK', () => {
    ...
    const bare = encodeChallenge({ trackId: 'lombard', direction: 'forward', time: 60, rating: 1, score: 0 });
    expect(decodeChallenge(bare, known)).toEqual({ trackId: 'lombard', direction: 'forward', time: 60, rating: 1, score: 0 });
  });
  ...
  it('links the plain game page, never a ?challenge= code itch cannot pass into the game', () => {
    expect(challengeUrl(null)).toBe('https://guigulaoshi.itch.io/silicon-slime-rush');
  ...
  it('puts the dare and the link into both share texts, and the plain URL only once', () => {
    const code = encodeChallenge(run);
    const result: ResultFacts = { trackId: 'goldengate', time: 151.37, score: 15562, slimeHits: 35, best: null, isBest: false,
      rating: 3, challengeCode: code, challengeUrl: challengeUrl(null), challengeVehicleId: 'sports-car' };
    for (const lang of ['zh', 'en'] as const) {
      const t = new I18n(lang);
      for (const text of [shareLines(t, result, 'https://guigulaoshi.itch.io/silicon-slime-rush').join('\n'),
        textCard(t, result, 'https://guigulaoshi.itch.io/silicon-slime-rush')]) {
        // Cars differ in pace: the shared dare names the car, the route and the time.
        expect(text).toContain(t.t('challenge.shareLine', { car: t.t('car.sports-car.name'), route: t.t('track.goldengate.name'), time: '2:31.37' }));
        expect(text).toContain(result.challengeUrl);
        expect(text.split('https://').length - 1, 'one link, the challenge link').toBe(1);
      }
    }
    expect(challengeLines(new I18n('en'), { trackId: 'goldengate', time: 1 })).toEqual([]);
  });
});
