...
// built by `npm run synth` / `sr build goldengate`; a fresh clone has no tracks yet
const TRACK_PATH = resolve(process.cwd(), 'public/tracks/goldengate/track.json');
...
describe.skipIf(!HAS_TRACK)('streaming the real Golden Gate track', () => {
  it('has tiles, and the off-route ones carry no arc-length interval', () => {
    expect(track.tiles.length).toBeGreaterThan(50);
  ...
  it('keeps the working set bounded end to end and back', () => {
    ...
    // The window is 300 m behind, 1500 m ahead and a kilometre either side, so on a five kilometre
    // route it legitimately covers most of the track: what has to stay bounded is the count, not
    // its share. Measured peak is 52 tiles, about 1.4 MB compressed.
    checkMaximum(run.peak, 'max_loaded_tiles', 'Golden Gate streaming working set');
  ...
  it('drops every tile that is neither ahead on the road nor near the car', () => {
    ...
    // the window reaches 1500 m ahead and 300 m back, which on a five kilometre route is a third of
    // it, so a little over half the track is dropped rather than the two thirds a long route drops
    expect(plan.unload.length).toBeGreaterThan(track.tiles.length * 0.5);
  });
});
