...
for (const id of [...SYNTHETIC, ...CATALOGUE.map(t => t.id)]) it(id + ' reverses the same physical road, gates and streaming ranges without mutating the source', () => {
  ...
  const reverse = directedTrack(source, id === 'twin-peaks' ? 'forward' : 'reverse');
  if (id !== 'twin-peaks') expect(directedTrack(source)).toBe(source);
...
it('swaps the player-facing Twin Peaks directions while keeping their record labels', () => {
  const source: TrackData = JSON.parse(readFileSync('public/tracks/twin-peaks/track.json', 'utf8'));
  const forward = directedTrack(source, 'forward');
  const reverse = directedTrack(source, 'reverse');
  expect(reverse).toBe(source);
  expect(forward.spline.points[0]).toEqual(source.spline.points.at(-1));
  expect(forward.spline.points.at(-1)).toEqual(source.spline.points[0]);
  expect(raceKey(source.id, 'forward')).toBe('twin-peaks');
  expect(raceKey(source.id, 'reverse')).toBe('twin-peaks:reverse');
});
