...
describe('the file the pipeline actually writes', () => {
  ...
  it('parses, and holds every route the game can play', () => {
    ...
    expect(map.land.length).toBeGreaterThan(10);        // the islands in the bay
    expect(map.roads.length).toBeGreaterThan(50);       // the freeway skeleton under the routes
    expect(map.places.length).toBeGreaterThan(4);       // the towns a local locates a route by
  ...
  it('has the bay wet and the valley dry', () => {
    // The failure this catches is the polygon assembly coming out inverted -- a map that is all
    // water with a lake where San Francisco should be, which throws nothing at all.
    const wet = (p: [number, number]): boolean => map.water.some((r) => inside(r, p));
    expect(wet([-122.30, 37.72]), 'middle of the bay').toBe(true);
    expect(wet([-122.60, 37.60]), 'ocean off Pacifica').toBe(true);
    expect(wet([-122.38, 38.07]), 'San Pablo Bay').toBe(true);
    expect(wet([-121.90, 37.34]), 'San Jose').toBe(false);
    expect(wet([-122.18, 37.83]), 'the Oakland hills').toBe(false);
    expect(wet([-122.42, 37.77]), 'downtown San Francisco').toBe(false);
  });

  it('lands every route in the Bay Area, not in the middle of the ocean', () => {
    for (const r of map.routes) {
      for (const [lon, lat] of r.line) {
        expect(lon, r.id).toBeGreaterThan(-123.2);
        expect(lon, r.id).toBeLessThan(-121.5);
        expect(lat, r.id).toBeGreaterThan(36.4);
        expect(lat, r.id).toBeLessThan(38.1);
      }
    }
  });
});
