...
import { musicSamples, RACE_MUSIC_RATE, RACE_PIECES } from '../src/audio/music';
...
it('renders three distinct race loops, each longer than the menu loop, with headroom and a join that does not click', () => {
  expect(RACE_PIECES.map(p => p.id)).toEqual(['rush', 'coast', 'night']);
