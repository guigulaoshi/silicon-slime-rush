...
describe('Save', () => {
  it('migrates old saves without inventing a day choice and persists either explicit time', () => {
    ...
    store.setItem('silicon-rush.save.v1', JSON.stringify({ version: 2, language: 'zh',
      best: { shoreline: 123 }, slimeDensity: 'many' }));
    expect(new Save(store).all).toMatchObject({ version: SAVE_VERSION, timeOfDay: null,
      language: 'zh', best: { 'shoreline@micro-hatch': 123 }, slimeDensity: 'many' });
  ...
  it('remembers each track vehicle independently and rejects retired ids on migration', () => {
    ...
    save.rememberVehicle('goldengate', 'school-bus');
    save.rememberVehicle('shoreline', 'pickup-travel-trailer');
    expect(new Save(store).all.vehicles).toEqual({ goldengate: 'school-bus', shoreline: 'pickup-travel-trailer' });
    expect(migrate({ vehicles: { goldengate: 'riot-truck', shoreline: 'retro-van' } }).vehicles)
      .toEqual({ shoreline: 'retro-van' });
  });
  ...
  it('lets an embedded performance run override this page without changing stored preferences', () => {
    ...
    const before = store.getItem('silicon-rush.save.v1');
    save.useForSession({ quality: 'low', slimeDensity: 'many' });
    expect(save.all).toMatchObject({ quality: 'low', slimeDensity: 'many' });
    expect(store.getItem('silicon-rush.save.v1')).toBe(before);
  });

  it('records a best time only when it is actually better', () => {
    const s = new Save(new Memory());
    expect(s.best('goldengate')).toBeNull();
    expect(s.record('goldengate', 300)).toBe(true);
    expect(s.record('goldengate', 320)).toBe(false);
    expect(s.best('goldengate')).toBe(300);
    expect(s.record('goldengate', 290)).toBe(true);
    expect(s.best('goldengate')).toBe(290);
  });

  it('survives a corrupt or hostile save rather than half-reading it', () => {
    const store = new Memory();
    store.setItem('silicon-rush.save.v1', '{not json');
...
describe('catalogue', () => {
  it('lists every track and only lets the built ones be driven', () => {
    ...
    expect(playable('goldengate', false)).toBe(true);
    expect(playable('cupertino-ring', false)).toBe(false);  // no data built yet
