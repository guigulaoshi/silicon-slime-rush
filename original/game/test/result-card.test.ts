...
const base: ResultFacts = { trackId: 'fishermans-wharf', time: 83, best: null, isBest: false,
...
it('puts five stars where the old slogan was, a new footer, and the place only with AI, in both languages', () => {
  ...
  expect(new I18n('en').t('replica.realRoads')).toBe('DRIVE REAL BAY AREA ROADS · FREE IN YOUR BROWSER');
});
...
it('draws the shared image with a large title, stars, and the place on its own route line for two players', () => {
  ...
  const title = texts.find(entry => entry.text === 'SILICON SLIME RUSH')!;
  ...
  expect(routeLines).toEqual([`Fisherman's Wharf · 3 / 3 PLACE`, `Fisherman's Wharf · 1 / 3 PLACE`]);
