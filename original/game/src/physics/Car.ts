...
// Breakout is based on velocity into the rail, not total speed. Ten metres per second means a
// genuine high-speed, fairly direct strike; a car doing motorway speed almost parallel to the rail
// still has only a small normal component and remains contained.
const GUARDED_WALL_CLOSING_SPEED = 10;
...
export class Car {
  ...
  constructor(
    ...
    const desc = this.api.RigidBodyDesc.dynamic()
      .setTranslation(spawn.pos[0], spawn.pos[1], spawn.pos[2])
    ...
    this.lastPosition.set(...spawn.pos);
  }
  ...
  private tryGuardrailBreakout(t: CarTuning, dt: number): boolean {
    ...
    if (Math.abs(normal.y) > .7 || closingSpeed < GUARDED_WALL_CLOSING_SPEED) return false;
  ...
  update(dt: number, input: CarInput): void {
    ...
    for (let i = 0; i < wheelCount; i++) {
      ...
      const load = Math.max(w.compression * t.suspensionStiffness + closingSpeed * t.suspensionDamping, 0);
      w.load = load;
      totalLoad += load;
