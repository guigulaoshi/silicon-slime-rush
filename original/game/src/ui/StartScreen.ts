...
import { VEHICLES, vehicleFor, vehicleTuning } from '../vehicles/catalogue';
...
const MAP_W = 1000;
const LOC = 300;
const PREVIEW_FALLBACK = 'goldengate';
...
export function startScreen(i18n: I18n, deps: StartDeps): StartScreen {
  ...
  const landing = home(i18n, () => {
    ...
    detail: () => {
      ...
      const r = routes[pick];
      if (!r) return '';
      const left = i18n.t(`car.${CARS[carAt]}.name`);
      const cars = playerCount === 2 ? `${left} + ${i18n.t(`car.${CARS[rightCarAt]}.name`)}` : left;
  ...
  function buildLocator(): void {
    ...
    project = null;
    if (!map) return;
    const frame = frameOf(map.region, 1);
    ...
    const g = svgEl('g');
    // The whole card is land, and the water is painted on it, so no page shows through the gaps.
    g.appendChild(svgEl('rect', { class: 'sm-locland', x: '0', y: '0', width: String(LOC),
      height: String(LOC) }));
    for (const w of map.water) g.appendChild(svgEl('path', { class: 'sm-water', d: `${pathFor(w, at)}Z` }));
    for (const l of map.land) g.appendChild(svgEl('path', { class: 'sm-land', d: `${pathFor(l, at)}Z` }));
    ...
    for (const place of map.places) {
      ...
      const w = text.length * 5.6 + 4, h = 12;
  ...
  function buildList(): void {
    ...
    routes.forEach((r, i) => {
      ...
      li.append(routeThumbnail(r), el('span', 'sm-nm', i18n.t(`track.${r.id}.name`)),
  ...
  function routeThumbnail(route: MenuRoute): HTMLImageElement {
    ...
    image.addEventListener('error', () => { image.src = './home/goldengate.webp'; }, { once: true });
    return image;
  }
  ...
  function buildStatic(): void {
    ...
    CARS.forEach((c, i) => {
      ...
      b.dataset.vehicle = c;
      const speed = i18n.speed(tuning(c).maxSpeed * 3.6);
      const drive = i18n.t(`drive.${tuning(c).drive}.short`);
      b.append(carThumbnail(c), el('span', 'sm-carnm', i18n.t(`car.${c}.name`)),
  ...
  function paint(): void {
    ...
    rightName.textContent = i18n.t(`car.${CARS[rightCarAt]}.name`);
    rightPreview.show(vehicleFor(CARS[rightCarAt]!)!, active && step === 2 && playerCount === 2);
    ...
    const chosen = [
      ...
      i18n.t(`car.${CARS[carAt]}.name`) + (playerCount === 2 ? ` + ${i18n.t(`car.${CARS[rightCarAt]}.name`)}` : ''),
    ];
    ...
    preview.show(vehicleFor(car)!, active && step === 2);
    carName.textContent = i18n.t(`car.${car}.name`);
    [car, CARS[rightCarAt]!].forEach((id, player) => {
      const specs = carSpecs[player]!;
      specs.drive.textContent = `${i18n.t(`drive.${tuning(id).drive}.name`)} · ${i18n.t(`car.${id}.drive`)}`;
      specs.desc.textContent = i18n.t(`car.${id}.desc`);
