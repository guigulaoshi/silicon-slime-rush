...
import { trailerRestOffset, type VehicleDefinition } from '../vehicles/catalogue';
...
export class GaragePreview {
  ...
  show(vehicle: VehicleDefinition, active: boolean): void {
    const changed = this.selected?.id !== vehicle.id;
  ...
  private async load(): Promise<void> {
    ...
    try {
      ...
      for (const [index, body] of bodies.entries()) {
        loaded.push(await VehicleModel.load(body, request.signal, undefined, fraction => {
