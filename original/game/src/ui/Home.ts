...
export function home(i18n: I18n, enter: () => void, language: () => void,
  ...
  image.src = './home/goldengate.webp';
  ...
  body.append(place, name, slogan, story, quickButton, go, promise);
  ...
  function render() {
    ...
    if (i18n.lang === 'zh') {
      name.replaceChildren(document.createTextNode('史莱姆赛车'), el('span', '', '硅谷'));
    } else {
      const secondLine = el('span', '', '');
      secondLine.append(el('span', 'brand-word-slime', 'SLIME'), document.createTextNode(' '), el('span', 'brand-word-rush', 'RUSH'));
      name.replaceChildren(document.createTextNode('SILICON'), secondLine);
    }
