...
export function resultsScreen(t: I18n, facts: () => ResultFacts, list: MenuList,
  ...
  function render() {
    ...
    cards.replaceChildren(...people.map((person, index) => {
      ...
      if (background) card.style.backgroundImage = `linear-gradient(90deg,#102332d9,#10233255),url(${background})`;
      ...
      card.append(branding, rating, track, score, stats, bottom, accessibleScore);
