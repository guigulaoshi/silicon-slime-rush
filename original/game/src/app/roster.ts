...
import { VEHICLES, vehicleTuning, type VehicleDefinition } from '../vehicles/catalogue';
...
export function raceRoster(humans: readonly VehicleDefinition[], ai: boolean, limit = Infinity,
  pace: (vehicle: VehicleDefinition) => number | undefined = () => undefined): RosterEntry[] {
  const selected = new Set(humans.map(vehicle => vehicle.id));
  let rivals = ai ? VEHICLES.filter(vehicle => !selected.has(vehicle.id)) : [];
