...
export class Racer {
  ...
  constructor(readonly id: string, readonly vehicle: VehicleDefinition,
    ...
    this.bot = new Autopilot(spline, role === 'ai' ? aiSettingsFor(tuning, difficulty) : autopilotSettingsFor(tuning));
  ...
  driveAs(tier: AiDifficulty): void {
    this.bot = new Autopilot(this.spline, aiSettingsFor(this.car.tuning, tier));
  }
