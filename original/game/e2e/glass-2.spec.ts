...
test('all nine garage cars show the same fixed glass viewpoints',async({page})=>{
 ...
 for(const vehicle of VEHICLES){
  ...
  await expect(hero).toHaveAttribute('data-model-vehicle',vehicle.id);await expect(hero).toHaveAttribute('data-state','ready');
...
test('glass remains reflective in the chase view and after changing render quality',async({page})=>{
 ...
 await page.goto('/?track=shoreline&bot=1&dev=1&time=day&vehicle=micro-hatch&slimes=none');
 await page.waitForFunction(()=>window.game);
 await page.evaluate(async()=>{const g=window.game as any;g.save.useForSession({slimeDensity:'none'});await g.autoRun('shoreline','day','micro-hatch');});
