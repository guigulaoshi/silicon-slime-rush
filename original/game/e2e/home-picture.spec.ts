...
/**
 * The home page's shipped picture, `public/home/goldengate.webp`, is a frame of the live orbit itself.
 * It is what a phone, reduced motion and a low-quality machine get instead of the orbit, what the home
 * page's Share hands out, and what `build/og-image.mjs` turns into the link preview. The first one was
...
/**
 * The orbit angle of the shipped picture: the tower left of the copy, the deck curving into the Presidio. An
 * angle rather than seconds after the opening, so the picture stays the same frame when the opening moves
 *.
 */
const HOME_PICTURE_ANGLE = Math.PI + .36;
const SURVEY_OUT = resolve(HERE, '..', 'test-results', 'evidence', 'home-picture');

async function openOrbit(page: Page) {
  await page.setViewportSize({ width: 1280, height: 720 });
  await page.addInitScript(() => localStorage.setItem('silicon-rush.save.v1', JSON.stringify({ version: 1, language: 'en', muted: true })));
...
test('home picture', async ({ page }) => {
  ...
  await sharp(png).resize(1280, 720).webp({ quality: 80 }).toFile(resolve(HERE, '..', 'public', 'home', 'goldengate.webp'));
