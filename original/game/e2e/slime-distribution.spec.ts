...
for (const density of ['none', 'normal', 'many'] as const) test(`211 ${density} road and scenery`, async ({ page }) => {
  await page.addInitScript(value => localStorage.setItem('silicon-rush.save.v1', JSON.stringify({
    version: 5, language: 'en', quality: 'high', muted: true, slimeDensity: value,
  })), density);
  await page.goto('/?track=shoreline&bot=1&dev=1&speed=3&time=day');
  ...
  while (report.state !== 'finished' && report.time < budget) {
    ...
    samples.push(report);
    for (const distance of [600, 1400, 2200]) {
