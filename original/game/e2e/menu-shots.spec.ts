...
const PLACES: Record<string, { at: number; side: number; back: number; up: number; look: number; car?: string }> =
  JSON.parse(readFileSync(resolve(HERE, 'menu-shots.json'), 'utf8'));
const ONLY = process.env.MENU_TRACK;
const TRACKS = CATALOGUE.map(track => track.id).filter(id => !ONLY || id === ONLY);
...
async function frame(page: Page, place: { at: number; side: number; back: number; up: number; look: number }) {
  await page.evaluate(place => {
    ...
    s.racers[0].chase.update = () => {
      ...
      cam.lookAt(p[0] + t[0] * place.look, p[1] + 1.2, p[2] + t[2] * place.look);
...
for (const track of TRACKS) test(`menu shots ${track}`, async ({ page }) => {
  ...
  if (MODE === 'preview') {
    mkdirSync(SURVEY_OUT, { recursive: true });
    await open(page, track, variants.find(([name]) => name === 'day-clear')![1], place.car);
  ...
  for (const [name, choice] of variants) {
    ...
    if (name === 'day-clear') await webp(page, resolve(HERE, '..', 'public', 'menu', 'tracks', `${track}.webp`), 960);
  }
});
