...
test('articulated trailer: sharp turns, reverse, impact, reset and unload on a real road', async ({ page }) => {
  await page.goto('/?track=shoreline&bot=1&dev=1&time=day&vehicle=pickup-travel-trailer');
  ...
  // From rest in the middle of Shoreline's 22 m roadway the rig needs about 3.2-3.5 s to reach the
  // rail, so a fixed 3.5 s sleep raced the frame rate. Wait for the real scrape instead.
...
// Full Shoreline completion and the <.15m hitch check share the per-vehicle drive in vehicle-driving.spec.ts.
