...
test('fog weather renders the layered fog model on a streamed city drive', async ({ browser }) => {
  ...
  try {
    ...
    await page.goto('/?dev=1&track=fishermans-wharf&bot=1&speed=1&time=day&weather=fog');
    await page.waitForFunction(() => window.game?.report().phase === 'racing', null, { timeout: 90_000 });
    await expectWorldLoaded(page, 'fishermans-wharf fog');
    ...
    await page.screenshot({ path: resolve(OUT, 'fishermans-wharf-day-start.png') });
    await page.waitForFunction(() => window.game.report().progress > 350, null, { timeout: 120_000 });
    await page.screenshot({ path: resolve(OUT, 'fishermans-wharf-day-350m.png') });
    ...
    await context.close();
    renameSync(await video.path(), resolve(OUT, 'fishermans-wharf-fog-drive.webm'));
  }
});
