...
const ROUTES = ['fishermans-wharf', 'goldengate', 'lombard',
  'shoreline', 'twin-peaks', 'wolfe-pruneridge'];
...
test('boards along the road carry a face, and the face follows the language', async ({ page }) => {
  ...
  // Every platform once, and the same social face whichever language is showing.
  const social = manifest.filter((face) => face.kind === 'social');
  expect(social.every((face) => face.zh === face.en && face.zhPlatform === face.enPlatform)).toBe(true);
  expect(social.map((face) => face.zhPlatform).sort()).toEqual(
