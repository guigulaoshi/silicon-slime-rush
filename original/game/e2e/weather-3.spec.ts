...
test('actual weather contacts produce water and snow sprays, ice stays clean, and a new clear race resets effects',async({page})=>{
 ...
 for(const [weather,kind,speed]of cases){
  const result=await page.evaluate(async({weather,kind,speed})=>{
   ...
   await g.startRace({trackId:'shoreline',vehicleId:kind==='deepSnow'?'monster-truck':'micro-hatch',weather,timeOfDay:'day',ai:false,slimeDensity:'none'});g.phase='paused';cancelAnimationFrame(g.raf);g.uiHost.style.display='none';
 ...
 const clear=await page.evaluate(async()=>{const g=window.game as any;await g.startRace({trackId:'shoreline',weather:'clear',ai:false,slimeDensity:'none'});return g.session.world.weatherEffects.stats();});
...
test('stationary water receives animated ripples without wheel spray',async({page})=>{
 ...
 const result=await page.evaluate(async()=>{
  const g=window.game as any;await g.startRace({trackId:'shoreline',weather:'rain',ai:false,slimeDensity:'none'});g.phase='paused';cancelAnimationFrame(g.raf);g.uiHost.style.display='none';
