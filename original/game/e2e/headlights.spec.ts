...
test('capture each vehicle lighting on the same real bridge road',async({page})=>{
 ...
 for(const vehicle of VEHICLES){
  ...
  await page.evaluate(async id=>{
   const g=window.game as any;await g.startRace({trackId:'goldengate',vehicleId:id,timeOfDay:'night',weather:'clear',slimeDensity:'none',ai:false});
   const s=g.session,w=s.world,i=w.spline.indexAt(1450),p=w.spline.point(i),t=w.spline.tangent(i),y=p[1]-s.vehicle.anchorY+s.vehicle.suspensionRest;
...
test('roof lights follow real hills and a full night split race stays playable',async({page})=>{
 ...
 await page.evaluate(async()=>{
  const g=window.game as any;await g.startRace({trackId:'twin-peaks',vehicleId:'monster-truck',timeOfDay:'night',slimeDensity:'none',ai:false});
  const s=g.session,w=s.world,i=186,p=w.spline.point(i),t=w.spline.tangent(i),at=w.spline.s[i];
 ...
 const shot=await page.evaluate(()=>new Promise<string>(resolve=>{(window.game as any).captureNextFrame=(canvas:HTMLCanvasElement)=>resolve(canvas.toDataURL('image/png'));}));
 writeFileSync(resolve(out,'twin-peaks-slope.png'),Buffer.from(shot.split(',')[1]!,'base64'));
 expect(await page.evaluate(()=>window.game.report().vehicleLights.every(r=>r.pools>0))).toBe(true);
 if(process.env.PERF_REAL_GPU==='1'){
  await page.evaluate(()=>window.game.startRace({trackId:'twin-peaks',playerVehicles:['monster-truck','jeep'],timeOfDay:'night',slimeDensity:'normal',ai:true},true));
