...
test('shows day and night snow and completes curved Lombard Street with reduced grip', async ({ page }) => {
  mkdirSync(OUT, { recursive: true });
  const reports: Record<string, unknown> = {};
  const open = async (time: TimeOfDay) => {
    await page.goto(`/?dev=1&track=lombard&bot=1&speed=12&time=${time}&weather=snow`);
