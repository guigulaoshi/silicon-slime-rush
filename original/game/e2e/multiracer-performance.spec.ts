...
const scenarios = [1, 2].flatMap(humans => [false, true].flatMap(ai => ['day', 'night'].map(time => ({ humans, ai, time, track: 'bayshore-101' }))));
scenarios.push(...['day', 'night'].map(time => ({ humans: 2, ai: true, time, track: 'twin-peaks' })));
...
for (const scenario of scenarios) test(`whole frame ${scenario.track} ${scenario.humans} humans AI ${scenario.ai} ${scenario.time}`, async ({ browser }) => {
  ...
  await context.addInitScript(() => localStorage.setItem('silicon-rush.save.v1', JSON.stringify({ quality: 'high', language: 'en', muted: true })));
  ...
  try {
    ...
    const samples = [];
    for (const spread of scenario.track === 'bayshore-101' && roster.length > 1 ? [false, true] : [false]) {
      ...
      if (spread) expect(Math.max(...state.players.map(r => r.x)) - Math.min(...state.players.map(r => r.x))
        + Math.max(...state.players.map(r => r.z)) - Math.min(...state.players.map(r => r.z))).toBeGreaterThan(4000);
...
test('full roster releases resources across repeated separation, restart and exit', async ({ page }) => {
  ...
  for (let run = 0; run < 3; run++) {
    expect(await page.evaluate(() => window.game.startRace({ trackId: 'bayshore-101', playerVehicles: ['pickup-travel-trailer', 'pickup-travel-trailer'],
