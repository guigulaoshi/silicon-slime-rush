...
/**
 * A whole snow race on Shoreline with every AI. Ruts laid on the opening straight are
 * photographed from one fixed camera early and again at the finish, more than 60 s of race later,
 * and the race reports what the kept marks cost. TRAIL_LABEL names the build (before/after).
 */
...
async function driveUntil(page: Page, done: (state: Progress) => boolean): Promise<void> {
  for (const started = Date.now(); ; ) {
    ...
    // The robot can wedge itself (seen at Shoreline ~830 m). Marks are what is under test,
...
async function race(page: Page): Promise<void> {
  ...
  expect(await page.evaluate(() => window.game.startRace({ trackId: 'shoreline', car: 'sedan', timeOfDay: 'day',
