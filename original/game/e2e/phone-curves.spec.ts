...
test('all vehicle models report low-quality phone frame rate', async () => {
  ...
  try {
    ...
    for (const { id } of vehicles) {
      await page.goto(`http://localhost:${PORT}/?phonecheck=189&track=${process.env.PHONE_TRACK ?? 'shoreline'}&bot=1&dev=1&vehicle=${id}&time=${process.env.PHONE_TIME ?? 'day'}&perf=1&perfQuality=low&perfSlimes=${process.env.PHONE_DENSITY ?? 'normal'}`);
