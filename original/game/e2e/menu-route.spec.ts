...
for (const language of ['en', 'zh'] as const) for (const [width, height] of [[1440, 900], [844, 390]] as const)
  test(`route and world steps ${language} ${width}`, async ({ page }) => {
    ...
    await page.locator('[data-track="moffett-field"]').click();
    await expect(page.locator('.sm-item.sel')).toHaveAttribute('data-track', 'moffett-field');
    ...
    for (const track of ['moffett-field', 'goldengate']) {
