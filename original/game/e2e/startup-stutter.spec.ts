...
test('first thirty seconds after loading avoid a startup stall', async ({ page }) => {
  ...
  await page.addInitScript(() => {
    localStorage.setItem('silicon-rush.save.v1', JSON.stringify({ language: 'en', quality: 'high', muted: true }));
  ...
  await page.goto('/?track=moffett-field&bot=1&dev=1&speed=1&time=day');
