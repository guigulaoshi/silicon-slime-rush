 ...
 */
// Re-anchored to the route moved (start in the Presidio, 5.40 km): the south tower
// is nearest the road at ~1190 m, the north tower at ~2520 m, and the deck runs straight from ~1000 m to
// ~3200 m; the sharpest turns of the Marin climb are around 3900-4000 m. The old numbers put
// "bridge-middle" past the north tower.
const MILESTONES: { at: number; name: string }[] = [
  { at: 40, name: '01-start' },
  { at: 900, name: '02-fort-point' },
  { at: 1130, name: '03-south-tower' },
  { at: 1500, name: '04-bridge-south' },
  { at: 1855, name: '05-bridge-middle' },
  { at: 2450, name: '06-bridge-north' },
  { at: 3950, name: '07-conzelman-hairpins' },
  { at: 5300, name: '08-viewpoint' },
];
...
async function settle(page: Page, at: number): Promise<void> {
  ...
  for (let round = 0; round < 40; round++) {
    ...
    if (stable >= TILE_RETRIES) {
      expect(loaded, `goldengate at ${at} m: the complete planned tile window`).toBe(required);
      return;
    }
  }
...
async function pinnedFrame(page: Page, at: number): Promise<{ png: Buffer; scene: Scene }> {
  const shot = await page.evaluate(([s, back, up, ahead, water, fov]: number[]) => {
    ...
    // dimmed one on the next. 03-toll-plaza is 50 m from the gate at 1200 m, and the bot overshoots
    // a 500 ms poll by more than that. Pin it to the arc length instead of to where the bot got to.
...
test('the Golden Gate baseline still looks like the Golden Gate baseline', { tag: ['@version', '@pixel'] }, async ({ page }) => {
  mkdirSync(resolve(OUT, 'goldengate'), { recursive: true });
  await page.goto('/?track=goldengate&bot=1&dev=1');
  ...
  await expectWorldLoaded(page, 'goldengate at the start');
  ...
  for (const target of MILESTONES) {
    ...
    await expectWorldLoaded(page, `goldengate at ${target.name}`);
  ...
  await expectWorldLoaded(page, 'goldengate at the finish');
  ...
  const record = resolve(OUT, 'goldengate.json');
  if (updating()) {
