...
for (const [name, viewport, mobile] of [['desktop', { width: 1280, height: 720 }, false], ['phone', { width: 915, height: 412 }, true]] as const) {
  test(`439 ${name}: a taller-than-before map card keeps every selected route in view`, async ({ browser }) => {
    ...
    await page.addInitScript(() => localStorage.setItem('silicon-rush.save.v1', JSON.stringify({ language: 'zh', muted: true })));
    ...
    for (const track of CATALOGUE) {
      ...
      if (track.id === 'goldengate' || track.id === 'wolfe-pruneridge') await page.screenshot({ path: resolve(OUT, `${name}-${track.id}.png`) });
    }
    // A resize rebuilds the map's paths; the selected one keeps its highlight. (A mobile context cannot resize.)
    // Twin Peaks, because this resize does not pan the map there, and a pan rebuilt with the highlight anyway.
    if (!mobile) {
      await page.locator('[data-track="twin-peaks"]').click();
