...
test('a lost 3D context mid-race shows the recovery notice and the race carries on once restored', async ({ page }) => {
  ...
  await page.addInitScript(() => localStorage.setItem('silicon-rush.save.v1', JSON.stringify({ language: 'en', muted: true, quality: 'high' })));
...
test('a context that does not come back tells the player to restart the browser', async ({ page }) => {
  await page.addInitScript(() => localStorage.setItem('silicon-rush.save.v1', JSON.stringify({ language: 'zh', muted: true, quality: 'high' })));
  await page.goto('/?track=synth-p2p&bot=1&dev=1');
  await page.waitForFunction(() => window.game?.report().phase === 'racing');
...
test('a phone on auto quality starts its race at low', async ({ browser }) => {
  ...
  try {
    ...
    await page.addInitScript(() => localStorage.setItem('silicon-rush.save.v1', JSON.stringify({ language: 'en', muted: true, quality: 'auto' })));
