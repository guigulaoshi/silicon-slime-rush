...
test('the dense real window keeps every road slime and shows the full ordinary size spread', async ({ page }) => {
  ...
  await expectWorldLoaded(page, '233 Golden Gate dense window');
  ...
  writeFileSync(resolve(OUT, 'goldengate-window.json'), JSON.stringify(evidence, null, 2));
  await page.screenshot({ path: resolve(OUT, 'goldengate-many.png') });
});
