...
for(const option of ['none','relaxed','rush'] as const)test(`AI choice ${option} reaches the actual race`,async({page})=>{
 ...
 await page.addInitScript(()=>{if(!localStorage.getItem('silicon-rush.save.v1'))localStorage.setItem('silicon-rush.save.v1',JSON.stringify({language:'en',muted:true,reducedMotion:true}));});
...
test('phone Chinese AI selector fits and retains three visible choices',async({browser})=>{
 ...
 try{
  const page=await context.newPage();await page.addInitScript(()=>localStorage.setItem('silicon-rush.save.v1',JSON.stringify({language:'zh',muted:true,reducedMotion:true})));
