...
test('?time= changes the time of day and nothing else', async ({ page }) => {
  ...
  // carry the time, which also picked the car: `?time=day` turned Golden Gate's convertible into a
  // sedan, so a picture taken with it was a different drive from one taken without.
  await page.goto('/?track=goldengate&bot=1&speed=4');
  ...
  await page.goto('/?track=goldengate&bot=1&time=day&speed=4');
