...
test('startup language survives denied storage', async ({ page }) => {
  ...
  await expect(page.locator('.home-title')).toHaveAttribute('aria-label', language === 'zh' ? '史莱姆赛车：硅谷' : 'SILICON SLIME RUSH');
  await expect(page.locator('.home-title .brand-wordmark')).toHaveText('Silicon Slime Rush');
});
...
test('phone help uses touch controls and saved motion preference covers loading', async ({ browser }) => {
  ...
  await page.addInitScript(() => localStorage.setItem('silicon-rush.save.v1', JSON.stringify({ reducedMotion: true })));
  let release!: () => void; const held = new Promise<void>(r => { release = r; });
