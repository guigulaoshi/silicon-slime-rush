...
for (const quality of ['high', 'low'] as const) {
  test(`${quality} keeps the full pole-to-ground light chain`, async ({ page }) => {
    ...
    await page.addInitScript((selected) => localStorage.setItem('silicon-rush.save.v1', JSON.stringify({
      version: 1, language: 'en', quality: selected, volume: 0, muted: true, best: {},
    })), quality);
    await page.goto('/?track=twin-peaks&bot=1&dev=1&time=night&inspect=lights');
