  ...
  for (const track of TRACKS) {
    for (const time of ['day', 'night']) {
      ...
      const data = await page.evaluate(async ({ route }) => {
        ...
        const at = world.spline.length * (route === 'wolfe-pruneridge' ? .18 : .36);
        ...
        if (route === 'wolfe-pruneridge') {
          world.camera.position.set(0, point[1] - 17, 0);
          world.camera.lookAt(point[0], point[1] - 1.2, point[2]);
        } else {
        ...
        world.camera.fov = route === 'wolfe-pruneridge' ? 62 : 68;
        ...
        const inspect = (root: any, backdrop: boolean) => root.traverse((object: any) => {
          ...
          if (object.name === 'roof_top_band') topBandMeshes++;
        });
        inspect(world.streamer.root, false);
        inspect(world.backdrop.root, true);
        return { route, at, buildingMeshes, streamFasciaVertices, backdropFasciaVertices, topBandMeshes,
      ...
      expect(data.streamFasciaVertices, `${track}: streamed no-window fascia reaches the browser`)
        .toBeGreaterThan(0);
      expect(data.backdropFasciaVertices, `${track}: backdrop no-window fascia reaches the browser`)
        .toBeGreaterThan(0);
      if (track === 'wolfe-pruneridge') {
        expect(data.topBandMeshes, 'Big Tech Rooftop has its dedicated no-window top band').toBeGreaterThan(0);
      }
    }
  }
});
