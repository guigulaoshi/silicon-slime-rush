...
test('desktop views, HUDs and giant immersion remain independent and return to single', async ({ page }) => {
  ...
  expect(await page.evaluate(() => window.game.startRace({ trackId: 'shoreline', car: 'sedan',
...
test('night views keep both headlights and distant viewpoints', async ({ page }) => {
  ...
  await page.setViewportSize({ width: 1280, height: 720 });
  await page.goto('/?dev=1'); await page.waitForFunction(() => window.game);
  expect(await page.evaluate(() => window.game.startRace({ trackId: 'bayshore-101', car: 'sedan',
  ...
  await page.evaluate(() => {
    ...
    const i = s.world.spline.indexAt(8000), p = s.world.spline.point(i), t = s.world.spline.tangent(i);
