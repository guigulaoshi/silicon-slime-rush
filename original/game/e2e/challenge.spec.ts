...
test('a challenge link lands on the route, the run is compared, and the copied text carries the next dare', async ({ page, context }) => {
  ...
  await page.addInitScript(() => { if (!sessionStorage.getItem('seeded')) { sessionStorage.setItem('seeded', '1');
    localStorage.setItem('silicon-rush.save.v1', JSON.stringify({ version: 1, language: 'en', volume: 0, muted: true, best: {} })); } });
  ...
  await page.evaluate(() => { const save = JSON.parse(localStorage.getItem('silicon-rush.save.v1')!); save.language = 'zh'; localStorage.setItem('silicon-rush.save.v1', JSON.stringify(save)); });
